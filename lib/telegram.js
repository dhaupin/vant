...
/**
 * Telegram Bot - Simple bot wrapper for VANT (v0.8.6)
 * WITH EVENT EMISSIONS - bot events emit globally
 * 
 * Usage:
...
async function api(method, params = {}) {
    ...
    if (!BOT_TOKEN) {
        throw new errors.VantError('TELEGRAM_BOT_TOKEN not set', { code: errors.CODES.UNKNOWN });
    }
    
    const data = await network.fetchJson(`${API_BASE}/${method}`, {
        method: 'POST',
    ...
    if (!data.ok) {
        throw new errors.VantError('Telegram API error', { code: errors.CODES.API_RESPONSE_ERROR });
    }
...
async function send(chatId, text, options = {}) {
    ...
    // EVENT: message sent (bot communicated)
    _emit('telegram:sent', { 
        chatId, 
...
async function startPolling(timeout = 60) {
    ...
    audit.info('[Telegram] Starting polling...');
    ...
    while (true) { // eslint-disable-next-line no-constant-condition
        try {
            ...
            audit.error('[Telegram] Poll error:', e.message);
