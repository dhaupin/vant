...
class Relay {
  ...
  async connect(targetUrl) {
    ...
    restoreState,
        ...
        _emit('relay:connect', { name: response.identity.name, url: targetUrl });
...
module.exports = {
  ...
  setBrainRelayConfig,
    gatherState,
    restoreState,
  getStackRelayConfigs
};

// ==================== MULTIBRAIN SUPPORT ====================

const _brainRelayConfigs = {};

function getBrainRelayConfig() {
    const brain = require('./brain');
    const brainName = brain.getCurrentBrain();
    return _brainRelayConfigs[brainName] || { timeout: 5000 };
}

function setBrainRelayConfig(config) {
    const brain = require('./brain');
    const brainName = brain.getCurrentBrain();
    _brainRelayConfigs[brainName] = config;
    return true;
}

function getStackRelayConfigs() {
    const brain = require('./brain');
    ...
    for (const brainName of stack) {
        try {
            ...
            results.byBrain[brainName] = getBrainRelayConfig();
        } catch (e) {
...
function gatherState() {
    return { configs: Object.assign({}, _brainRelayConfigs), count: Object.keys(_brainRelayConfigs).length, gatheredAt: Date.now() };
}
function restoreState(data) {
    if (data && data.configs) Object.assign(_brainRelayConfigs, data.configs);
    return { restored: true, configs: Object.keys(_brainRelayConfigs).length };
}
