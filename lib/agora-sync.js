...
function install(bus, opts = {}) {
    ...
    // (pass 59 / Wave D) MSG SYNC — the town's postal service. Same trust
    // posture as every agora leg: registered peers only, scope resolves
    // OWNER-SIDE on the request leg (a scoped conversation is never named
    // to a non-member), reply legs sender-bound (pass-51 rule), and the
    // receiving node merges via msg.mergeSnapshot which adopts only what
    // it has never seen (the wire never edits local history).
    //
    //   A -> B  crew.msg.request  { convId, reqId }   (registered peers)
    //   B -> A  crew.msg          { snapshot, from }  (scope-gated reply)
    //   B -> A  crew.msg.push     { snapshot, from }  (receiver-side gate)
    //
    // Scope rides the payload so crew-bus's pass-40 gate ALSO drops a
    // scoped envelope for a non-member before any dispatcher sees it —
    // two independent gates on one boundary (belt and suspenders, the
    // genesis-lesson pattern).
    bus.onDispatch('msg.request', (env) => {
        const from = env && env.from;
        const p = env && env.payload;
        if (!p || typeof p.convId !== 'string' || p.convId.length < 1 || p.convId.length > 100) {
            return; // malformed ask: drop silently
        }
        let peers;
        try { peers = bus.nodes(); } catch (e) { peers = []; }
        if (!peers.some((n) => n.name === from)) return; // registered peers only
        // OWNER-SIDE SCOPE GATE: resolve the asker's PRINCIPAL through the
        // node-registry — the same vetting anchor genesis writes and
        // consensus's requireRegistry reads (no new trust path).
        // resolvePrincipal is registry-first (pass 59 live-fire fix): the
        // vetted agent identity beats the crew_ transport self-registration
        // when both carry the node name; the fallback is the node name
        // itself (the pass-40 convention where node name = principal when
        // no agentId was vetted).
        let visible = true;
        try {
            const scopeMod = require('./scope');
            const convScope = require('./msg').getScope(p.convId);
            if (convScope) {
                let principal = from;
                try {
                    principal = require('./node-registry').resolvePrincipal(from) || from;
                } catch (e2) { /* registry unavailable: node-name fallback stands */ }
                visible = !!scopeMod.canAccess({ scope: convScope }, principal);
            }
        } catch (e) { visible = false; } // scope layer broken: fail-closed
        let snapshot = null;
        if (visible) {
            try { snapshot = require('./msg').exportSnapshot(p.convId); } catch (e) { snapshot = null; }
        }
        bus.send(from, 'msg', { snapshot, scope: snapshot ? snapshot.scope : null, from: busName(bus), reqId: p.reqId || null })
            .catch(() => { /* reply is best-effort; asker has a timeout */ });
    });

    // The reply leg resolves the asker's pending msgPull. Sender-bound:
    // the reply must come from the node that was ASKED (a third peer
    // replaying an observed reqId must not feed our merge path).
    bus.onDispatch('msg', (env) => {
        const payload = env && env.payload;
        if (!payload || typeof payload !== 'object' || !('snapshot' in payload)) return;
        const pending = _msgPending.get(pendingKey(bus, payload.reqId));
        if (!pending) return; // unsolicited / late reply — drop
        if (pending.from && env.from !== pending.from) return;
        clearTimeout(pending.timer);
        _msgPending.delete(pendingKey(bus, payload.reqId));
        pending.resolve({ pulled: payload.snapshot ? true : false, reason: payload.snapshot ? null : 'not_found', envelope: { from: env.from }, snapshot: payload.snapshot || null });
    });

    // The push leg: a peer PUSHES a conversation snapshot (e.g. after a
    // standup post on a JV channel). RECEIVER-SIDE gate: a scoped payload
    // must be accessible to THIS node's agentId (scope resolves here, where
    // our registry lives — the pass-50 rule on the push leg); crew-bus's
    // envelope gate already filtered once, this one re-checks with the
    // conversation's own scope record. Unscoped snapshots flow. Merge is
    // the same validating mergeSnapshot — adopt-only-unknown.
    bus.onDispatch('msg.push', (env) => {
        const from = env && env.from;
        const snapshot = env && env.payload && env.payload.snapshot;
        ...
        try {
            if (snapshot.scope) {
                const scopeMod = require('./scope');
                const me = (bus.status && bus.status().agentId) || busName(bus);
                allowed = !!scopeMod.canAccess({ scope: snapshot.scope }, me);
            }
        } catch (e) { allowed = false; } // fail-closed
        if (!allowed) return;
        try { require('./msg').mergeSnapshot(snapshot); } catch (e) { /* malformed push: drop */ }
    });
...
// ---------- msg snapshot legs (pass 59 / Wave D) ----------

// Pull round-trips for the msg legs. Same lifecycle contract as _pending:
// every entry resolves or times out.
const _msgPending = new Map();

const MSG_PULL_DEFAULT_TIMEOUT_MS = 8000;

/**
 * Pull a conversation snapshot from a peer. Mirrors agora-sync.pull:
 * the OWNER filters by the asker's membership (owner-side scope), the
 * reply is sender-bound, and the merge adopts only unknown messages via
 * msg.mergeSnapshot. Resolves:
 *   { pulled: true, merged: { merged, adopted, ... } }  — got it + merged
 *   { pulled: false, reason }                           — not found / timeout / refused
 * Never throws for wire-level failures.
 *
 * @param {object} bus - configured bus with the peer registered
 * @param {string} nodeName - peer to ask
 ...
 * @param {object} [opts] - { timeoutMs: 8000 }
 */
async function msgPull(bus, nodeName, convId, opts = {}) {
    if (!bus || typeof bus.send !== 'function') {
        throw new Error('agora-sync.msgPull: bus required');
    }
    if (typeof convId !== 'string' || !convId || convId.length > 100) {
        return { pulled: false, reason: 'invalid_conv_id' };
    }
    const timeoutMs = Math.min(parseInt(opts.timeoutMs, 10) || MSG_PULL_DEFAULT_TIMEOUT_MS, 60000);
    const reqId = crypto.randomBytes(8).toString('hex');
    ...
    const result = await new Promise((resolve) => {
        const timer = setTimeout(() => {
            _msgPending.delete(pendingKey(bus, reqId));
            resolve({ pulled: false, reason: 'timeout' });
        }, timeoutMs);
        _msgPending.set(pendingKey(bus, reqId), { resolve, timer, from: nodeName });
        bus.send(nodeName, 'msg.request', { convId, reqId }).catch((e) => {
            clearTimeout(timer);
            _msgPending.delete(pendingKey(bus, reqId));
            resolve({ pulled: false, reason: 'send_failed: ' + e.message });
        });
    ...
    let merged;
    try {
        merged = require('./msg').mergeSnapshot(result.snapshot);
    } catch (e) {
        return { pulled: true, merged: { merged: false, adopted: 0, reason: 'merge_error: ' + e.message } };
    }
    return { pulled: true, merged };
}

/**
 * Push a conversation's LOCAL snapshot to a peer (e.g. the JV standup
 * owner distributing its channel history). The receiving node gates
 * receiver-side on its own agentId + the conversation's scope record,
 * then merges merge-only. Resolves { pushed: true, ok, handlers } or
 * { pushed: false, reason }.
 *
 * @param {object} bus - configured bus with the peer registered
 ...
 * @param {string} convId - conversation id
 */
async function msgPush(bus, nodeName, convId) {
    if (!bus || typeof bus.send !== 'function') {
        throw new Error('agora-sync.msgPush: bus required');
    }
    if (typeof convId !== 'string' || !convId || convId.length > 100) {
        return { pushed: false, reason: 'invalid_conv_id' };
    }
    if (!_installed.has(bus)) install(bus);
    let snapshot;
    try { snapshot = require('./msg').exportSnapshot(convId); } catch (e) {
        return { pushed: false, reason: 'export_failed' };
    }
    if (!snapshot) return { pushed: false, reason: 'not_found' };
    try {
        const r = await bus.send(nodeName, 'msg.push', { snapshot, scope: snapshot.scope, from: busName(bus) });
        return { pushed: true, ok: r.ok, handlers: r.handlers };
    } catch (e) {
        return { pushed: false, reason: 'send_failed: ' + e.message };
    }
...
module.exports = { install, pull, push, vote, status, reap, gossipAsk, gossipRound, startGossip, stopGossip, msgPull, msgPush };
