...
class Memory {
    ...
    async query(key) {
        // Check cache first
        const cached = this._cache.get('learn:' + key);
        ...
        try {
            const data = await brain.load(category + '/' + fileKey);
    ...
    async state(key, value, opts = {}) {
        ...
        // Event + Audit
        this._emit('memory:stated', { key });
        this._auditLog('memory_state', { key });
    ...
    async recall(key) {
        ...
        // Try brain
        try {
            const data = await brain.load(STATE_CATEGORY + '/' + key + '.json');
            if (data) {
                const parsed = typeof data === 'string' ? JSON.parse(data) : data;
                if (parsed && !this._isExpired(parsed)) {
                    this._cache.set('state:' + key, parsed);
                    return parsed.value;
                }
            }
        } catch (e) {}
...
// Legacy wrapper for state-like remember (key, content) or (key)
// Used by Habitat/Nature persistence
async function rememberWrapper(key, content, opts) {
    if (content !== undefined) {
        return await memory.state(key, content, opts);
    } else {
...
module.exports = {
    ...
    // Legacy remember (key, content) or (key) - for Habitat/Nature
    // Also supports semantic: remember(type, data)
    learn: memory.learn.bind(memory),
    remember: rememberWrapper,
