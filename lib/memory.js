...
class Memory {
    ...
    async _save() {
        ...
        try {
            if (useSpecificBrain) {
                await brain.writeTo({ name: targetBrain, type: 'private' }, MEMORY_CATEGORY + '/experiences', JSON.stringify({
    ...
    async learn(key, content, opts = {}) {
        ...
        const targetBrain = brainName || this._getBrainName();
        if (brainName && brainName !== this._getBrainName()) {
            // Write to specific brain using writeTo
            await brain.writeTo({ name: brainName, type: 'private' }, category + '/' + fileKey, content);
        } else {
    ...
    async state(key, value, opts = {}) {
        ...
        try {
            if (brainName && brainName !== this._getBrainName()) {
                await brain.writeTo({ name: brainName, type: 'private' }, STATE_CATEGORY + '/' + key + '.json', JSON.stringify(entry));
            } else {
