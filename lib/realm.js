...
function gatherState() {
    const realmsData = [];
    for (const realm of _realms.values()) {
        realmsData.push({
            id: realm.id,
            name: realm.name,
            ideas: Array.from(realm._ideas.entries()),
            proposals: Array.from(realm._proposals.entries()),
            config: realm._config,
            stats: realm.stats()
        });
    }
    return {
...
async function restoreState(data) {
    _realms.clear();
    if (data && data.realms) {
        for (const realmData of data.realms) {
            const realm = create(realmData.name);
            realm.id = realmData.id;
            for (const [id, idea] of realmData.ideas) {
                realm._ideas.set(id, idea);
            }
            for (const [id, prop] of realmData.proposals) {
                realm._proposals.set(id, prop);
            }
            if (realmData.config) {
                realm._config = realmData.config;
            }
        }
    }
    return { restored: true, realms: _realms.size };
}
