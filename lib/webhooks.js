    ...
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (req.method === 'POST') {
                ...
                // (pass 65 live-fire fix) The ack previously reported the
                // EVENT-SYSTEM subscriber count as `handlers` — a receiver
                // that DROPPED the envelope in its own gate (crew-bus's
                // version/scope gates) still acked received:true handlers:N,
                // so a sender could not tell delivery from silent refusal.
                // The emit now carries a `dispatched` collector: handlers
                // that actually dispatch mark it, and the ack reports the
                // truth. received stays true (HTTP + HMAC verified);
                // dispatched is the honest per-route answer.
                const dispatchState = { dispatched: 0 };
                let handlerCount = 0;
                try {
