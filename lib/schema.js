...
function _checkRead() {
    ...
    if (sandbox && sandbox.canRead) {
        try {
            if (!sandbox.canRead()) {
                throw new errors.VantError('Read permission required for schema operations', { code: errors.CODES.CAPABILITY_NOT_ALLOWED });
            }
        } catch (e) {}
    }
}
...
// (F-1b) brain.json/_core.json read through the models FileStorage (module-load
// path frozen — deleted early protos (do.js, vibe.js); per-brain validation via arg below)
const _schemaStore = (() => {
    try {
        const Storage = require('./storage');
        return new Storage.FileStorage({ basePath: path.resolve(MODELS_PATH) });
    } catch (e) { return null; }
})();
const STATE_FILE = 'brain.json';
const LTC_FILE = '_core.json';
...
function validateFile(fileName) {
    _checkRead();
    if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(String(fileName))) {
        return { valid: false, error: 'Invalid file name: ' + fileName };
    }

    const store = _schemaStore || null;
    const raw = store ? store.read(fileName) : null;
    if (raw === null) {
...
function isValid() {
    ...
    // Check brain.json
    if (_schemaStore && _schemaStore.has(STATE_FILE)) {
        results.push(validateFile('brain.json'));
    }

    // Check _core.json
    if (_schemaStore && _schemaStore.has(LTC_FILE)) {
...
/**
 * Get schemas from all brains in the stack
 * @returns {Object} Combined schema info
 */
function getStackSchemas() {
    const brain = require('./brain');
    ...
    for (const brainName of stack) {
        try {
            ...
            results.byBrain[brainName] = { hasSchema: !!schema };
        } catch (e) {
