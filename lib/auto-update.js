...
function addMessage(role, content) {
    vaf.check(role, {type: 'string', name: 'role', maxLength: 20, pattern: /^(user|assistant|system)$/});
...
function pushToGitHub(token, repo, message = 'Auto-update brain') {
    vaf.check(token, {type: 'string', name: 'token', maxLength: 100});
    vaf.check(repo, {type: 'string', name: 'repo', maxLength: 100, pattern: /^[\w\-\.]+\/[\w\-\.]+$/});
    vaf.check(message, {type: 'string', name: 'message', maxLength: 1000});
    ...
    try {
        ...
        // SECURITY: Use git config instead of token in URL
if (token) execSync(`git config --local user.token ${token.slice(0,4)}`);
execSync(`git push https://github.com/${repo}.git`, { stdio: 'pipe' });
