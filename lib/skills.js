...
module.exports = {
    ...
    loadProto(name) {
        const fs = require('fs');
        const path = require('path');
        const protoPath = path.join(__dirname, '..', 'models', 'skills', name + '.md');
        try {
            if (fs.existsSync(protoPath)) {
                return { name, path: protoPath, content: fs.readFileSync(protoPath, 'utf8') };
            }
        } catch (e) {}
        return null;
    },
    listProtos() {
        const fs = require('fs');
        const dir = path.join(__dirname, '..', 'models', 'skills');
        try {
    ...
    loadDef(name) {
        ...
        if (!sb.canRead()) throw new Error('Read permission required');
        const fs = require('fs');
        const folderPath = path.join(__dirname, '..', 'models', 'skills', 'vant-skill-' + name, 'SKILL.md');
        try {
    ...
    async loadChain(name, loaded = []) {
        const skill = this.loadDef(name);
        if (!skill) return loaded;
        if (loaded.includes(name)) return loaded;
