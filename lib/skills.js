/**
 * Skils (v0.8.6)
 * WITH EVENT EMISSIONS - skill loads emit globally
...
// (storage migration) Manifest + skill-file reads route through the shared
// models FileStorage (containment/symlink/VAF/atomic-write). Directory
// enumeration stays on fs (pattern-glob limitation) at models-anchored paths.
const _skillsModelRoot = _getModelsPath();
let _skillsStore = null;
function _getStore() {
    if (!_skillsStore) {
        const Storage = require('./storage');
        _skillsStore = new Storage.FileStorage({ basePath: _skillsModelRoot });
    }
    return _skillsStore;
}
function _rel(...segs) {
    return path.relative(_skillsModelRoot, path.join(...segs));
}
// Skill names become brain-file path segments - restrict to safe charset
// (folder format prefixes vant-skill-, flat format is {name}.md)
...
async function _loadManifestFile() {
    const rel = _rel(_getModelsPath(), MANIFEST_FILE);
    try {
...
async function saveManifest(m) {
    await _getStore().write(_rel(_getModelsPath(), MANIFEST_FILE), JSON.stringify(m, null, 2));
}
...
async function get(name) {
    return pipeline.run(
        { name: 'skills:get', name, operation: 'read' },
        async () => {
...
async function load(name) {
    return pipeline.run(
        { name: 'skills:load', name, operation: 'load' },
        async () => {
            vaf.check(name, { type: 'string', maxLength: 50 });
            ...
            if (def.type === 'native' && def.module) {
                try {
                    ...
                    // EVENT: skill loaded
                    _emit('skills:loaded', { name, type: def.type, timestamp: Date.now() });
...
module.exports = {
    ...
    loadProto(name) {
        ...
        // (storage migration) name is interpolated into the store path -
        // validate before use
        if (!_safeSkillName(name)) return null;
        const fmt = require('./format');

        // Brain order: private > public
        const roots = [
            path.join(process.cwd(), 'models', 'private', 'skills'),
            path.join(process.cwd(), 'models', 'public', 'skills')
        ];
        ...
        for (const root of roots) {
            ...
            if (_getStore().has(_rel(fp))) { found = { p: fp }; break; }
            const fp2 = path.join(root, name + '.md');
            if (_getStore().has(_rel(fp2))) { found = { p: fp2 }; break; }
        }
        if (!found) return null;

        const content = _getStore().read(_rel(found.p));
    ...
    // (enumeration stays on fs - pattern-glob limitation - paths anchored)
    listProtos() {
        const s = new Set();

        ['private/skills', 'public/skills'].forEach(sub => {
            const dir = path.join(process.cwd(), 'models', sub);
            try {
                fs.readdirSync(dir)
    ...
    loadFolder(name) {
        // (storage migration) name validated before path use
        if (!_safeSkillName(name)) return null;
        const folderPath = path.join(process.cwd(), 'models', 'public', 'skills', 'vant-skill-' + name, 'SKILL.md');
        try {
            const content = _getStore().read(_rel(folderPath));
            if (content !== null) {
    ...
    listFolders() {
        const dir = path.join(process.cwd(), 'models', 'public', 'skills');
        try {
...
/**
 * Get skills list from all brains in the stack
 * @returns {Object} Combined skills from all brains
 */
function getStackSkills() {
    const brain = require('./brain');
    ...
    for (const brainName of stack) {
        try {
            ...
            const skills = list();
            results.byBrain[brainName] = { skills: skills || [] };
        } catch (e) {
...
module.exports.getStackSkills = getStackSkills;
