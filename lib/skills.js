...
module.exports = {
    ...
    // NEW: Proto loading from models/skills/
    loadProto(name) {
        const fs = require('fs');
        const path = require('path');
        const FORMAT = require('./format');
        const baseDir = path.join(__dirname, '..', 'models', 'skills');
        
        // Try folder format first: vant-skill-{name}/SKILL.md
        let filePath = path.join(baseDir, 'vant-skill-' + name, 'SKILL.md');
        let content = null;
        
        if (fs.existsSync(filePath)) {
            content = fs.readFileSync(filePath, 'utf8');
        } else {
            // Fallback flat: {name}.md
        ...
        // Parse YAML frontmatter (both formats use same structure)
        ...
        return {
            ...
            format: filePath.includes('vant-skill-') ? 'folder' : 'flat'
        };
    },

    // List all protos - combines folder + flat
    listProtos() {
        const fs = require('fs');
        const path = require('path');
        const baseDir = path.join(__dirname, '..', 'models', 'skills');
        ...
        try {
            // Folder format: vant-skill-*
            fs.readdirSync(baseDir)
                .filter(d => d.startsWith('vant-skill-'))
                .forEach(d => list.add(d.replace('vant-skill-', '')));
            
            // Flat format: *.md (exclude subfolder paths)
            fs.readdirSync(baseDir)
                .filter(f => f.endsWith('.md') && !f.startsWith('vant-'))
                .forEach(f => list.add(f.replace('.md', '')));
        } catch (e) { /* ignore empty dir */ }
