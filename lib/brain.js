...
// v0.9.0 fs->storage migration (slice 1): state persistence, brain-file
// read/write helpers, and tmp-space APIs route through FileStorage
...
function _getTmpSpaceStore() {
    if (!_tmpSpaceStore) {
        ...
        _tmpSpaceStore = new Storage.FileStorage({
            basePath: path.resolve(__dirname, '..', 'models', 'tmp-space')
        });
    }
...
async function _loadState() {
    try {
        ...
        const content = _bfs.has('state.json') ? _bfs.read('state.json') : null;
...
async function _saveState(state) {
    ...
    const doSave = async () => {
        ...
        let current;
        try {
            const content = _getBrainFileStore().read('state.json');
...
async function _runBrainSecurityChain(operation, options = {}) {
    ...
    // (B-2) security rejections rethrow by code, not by message sniffing;
    // infrastructure errors no longer swallow silently
    try {
        ...
        if (e.code === errors.CODES.INPUT_VALIDATION_FAILED) throw e;
        console.warn('[brain] vaf check unavailable:', e.message);
    }

    // 2. QoS: Rate limiting
    // (B-2) rejection rethrow by code; infra errors warn instead of vanishing
    let qosCleanup = null;
    try {
        ...
        console.warn('[brain] qos check unavailable:', e.message);
    }

    // 3. RLS: Row-Level Security check (both reads and writes)
    let rlsCleanup = null;
    try {
        ...
        if (rls && rls.checkRead) {
            ...
            const checkFn = isWrite ? rls.checkWrite : rls.checkRead;
            const permitted = await checkFn.call(rls, userCtx, resource, operation);
            if (!permitted) {
                if (qosCleanup) qosCleanup();
                _emit('brain:rls:denied', { operation, category: options.category, key: options.key, timestamp: Date.now() });
                throw new errors.VantError('RLS: Access denied', { code: errors.CODES.ESCROW_DENIED });
            }
            rlsCleanup = () => {};  // Placeholder for future RLS cleanup
        }
    } catch (e) {
        if (qosCleanup) qosCleanup();
        // (B-2) RLS denial rethrows by code; infra errors warn instead of vanishing
        if (e.code === errors.CODES.ESCROW_DENIED) throw e;
        console.warn('[brain] rls check unavailable:', e.message);
    }

    // 4. Escrow: Additional write approval
    if (isWrite) {
        try {
            ...
            if (qosCleanup) qosCleanup();
            // (B-2) escrow denial rethrows by code; infra errors warn
            if (e.code === errors.CODES.ESCROW_DENIED) throw e;
            console.warn('[brain] escrow check unavailable:', e.message);
        }
    }
...
function _writeToBrain(brainName, brainType, category, key, content) {
    ...
    const rel = path.relative(_getBrainFileStore().basePath, path.join(root, brainName, category, key + '.md'));
    _getBrainFileStore().write(rel, content, { format: 'md' });
...
function _readFromBrain(brainName, brainType, category, key) {
    ...
    const rel = path.relative(_bfs.basePath, filePath);
    if (_bfs.has(rel)) {
        return _bfs.read(rel);
    }
    // Try without .md extension in key
    const altRel = path.relative(_bfs.basePath, path.join(brainPath, category, key));
    if (_bfs.has(altRel)) {
        return _bfs.read(altRel);
    }
    return null;
}
...
// (slice 3: name validated before path interpolation)
...
async function _loadBrain(name, options) {
    ...
    await executePipeline(_mode, ctx, async () => {
        ...
        if (_mode === 'public') {
            ...
            try {
                const content = await fs.promises.readFile(publicFile, 'utf8');
            ...
            try {
                const content = await fs.promises.readFile(privateFile, 'utf8');
            ...
            if (!found) {
                ...
                for (const brainName of stack) {
                    ...
                    try {
                        const content = await fs.promises.readFile(brainFile, 'utf8');
                        found = {
                            name: resolved,
                            content,
...
async function hasBrain(name) {
    ...
    try {
        await fs.promises.readFile(path.join(brainPath, resolved + '.md'), 'utf8');
        return 'private';
    } catch (e) {
        if (e.code !== 'ENOENT') throw e;
    }
    try {
        await fs.promises.readFile(path.join(publicPath, resolved + '.md'), 'utf8');
        return 'public';
    } catch (e) {
        if (e.code !== 'ENOENT') throw e;
    }
...
module.exports = {
    ...
    myListFiles() {
        const myPath = path.resolve(__dirname, '..', 'models', 'tmp-space', 'myStuff');
    ...
    dropFile(name, content) {
        ...
        return { saved: name, path: path.join(__dirname, '..', 'models', 'tmp-space', 'dropbox', name) };
    },
    ...
    listFiles() {
        ...
        const dropPath = path.resolve(__dirname, '..', 'models', 'tmp-space', 'dropbox');
    ...
    clearDropbox() {
        // Clear all shared files (enumeration + dir removal stays on fs; anchored)
        const dropPath = path.resolve(__dirname, '..', 'models', 'tmp-space', 'dropbox');
