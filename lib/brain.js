...
async function loadCorpus() {
    ...
    // Helper: read directory async (supports multiple extensions)
    async function readDirAsync(dirPath, source) {
        ...
        for (const file of files) {
            ...
            // Get brain name (strips any extension)
            ...
            if (source === 'private' || !brain[name]) {
                brain[name] = { content, source, format: formatName };
            }
        }
    }
    ...
    // Cache the result (include format)
...
function loadCorpusSync() {
    ...
    // Helper: read directory sync (supports multiple extensions)
 ...
 * @returns {Promise<Array>} Array of brain objects with brain name tracking
 */
async function loadStackCorpus() {
    const stack = getStack();
    const results = [];
    const format = _getFormat();
    const extensions = format?.DEFAULT_EXTENSIONS || ['.md'];
    ...
    // Helper: read directory async (supports multiple extensions)
    async function readDirAsync(dirPath, brainName) {
        try {
            if (!await fs.promises.access(dirPath).then(() => true).catch(() => false)) return;
            const files = await fs.promises.readdir(dirPath);
            for (const file of files) {
                ...
                const content = await fs.promises.readFile(path.join(dirPath, file), 'utf8');
                // Track which brain this came from
                results.push({
                    id: name + '|' + brainName,  // Unique ID with brain
                    title: name,
                    content,
                    format: formatName,  // NEW: include format
                    source: brainName,  // The brain name, not 'public'/'private'
                    brain: brainName,
                    type: 'brain'
                });
            }
    ...
    // Load from each brain in stack (top to bottom)
    for (const brainName of stack) {
        const brainInfo = resolveBrainPath(brainName);
        if (brainInfo && brainInfo.path) {
            await readDirAsync(brainInfo.path, brainName);
        }
    }
...
/**
 * MULTIBRAIN: Load corpus from ALL brains in stack (sync version)
 * @returns {Array} Array of brain objects with brain name tracking
 */
function loadStackCorpusSync() {
    const stack = getStack();
    const results = [];
    
    // Helper: read directory sync
    function readDirSync(dirPath, brainName) {
        try {
            if (!fs.existsSync(dirPath)) return;
            const files = fs.readdirSync(dirPath);
            for (const file of files) {
    ...
    for (const brainName of stack) {
        const brainInfo = resolveBrainPath(brainName);
        if (brainInfo && brainInfo.path) {
            readDirSync(brainInfo.path, brainName);
        }
    }
...
module.exports = {
    ...
    loadCorpus,
    loadCorpusSync,
    loadStackCorpus,  // MULTIBRAIN: Load from all brains in stack
    loadStackCorpusSync,
    hasBrain,
    brainFiles,
