...
function loadCorpus(opts = {}) {
    ...
    // Process files - builds brain object
    ...
    if (isSync) {
        // SYNC MODE
        if (_mode === 'public') {
            const files = readDir(publicPath, 'public', { sync: true });
            processFiles(files, 'public');
        } else if (_mode === 'private') {
            const files = readDir(brainPath, 'private', { sync: true });
            processFiles(files, 'private');
        } else {
            // Dual mode: public first, then private overrides
            const publicFiles = readDir(publicPath, 'public', { sync: true });
            processFiles(publicFiles, 'public');
            const privateFiles = readDir(brainPath, 'private', { sync: true });
            processFiles(privateFiles, 'private');
        }
        ...
        return (async () => {
            if (_mode === 'public') {
                const files = await readDir(publicPath, 'public', { sync: false });
                processFiles(files, 'public');
            } else if (_mode === 'private') {
                const files = await readDir(brainPath, 'private', { sync: false });
                processFiles(files, 'private');
            } else {
                // Dual mode
                const [publicFiles, privateFiles] = await Promise.all([
                    readDir(publicPath, 'public', { sync: false }),
                    readDir(brainPath, 'private', { sync: false })
                ]);
                processFiles(publicFiles, 'public');
                processFiles(privateFiles, 'private');
            }
...
const loadCorpusSync = (opts) => loadCorpus({ ...opts, sync: true });
...
const loadStackCorpusSync = (opts) => loadStackCorpus({ ...opts, sync: true });
