...
// Cache for loaded handlers (prevents reload per request)
...
function getHandler(name) {
    // Check cache first
    if (_handlerCache.has(name)) {
        return _handlerCache.get(name);
    }
    // Check registered
    ...
    if (registered) {
        ...
        if (registered._island && !registered._loaded) {
            loadIslandMiddleware(registered._island).then(handler => {
                ...
                _handlerCache.set(name, handler);
            });
    ...
    // Fall back to defaults (load and cache)
    if (_defaults[name]) {
