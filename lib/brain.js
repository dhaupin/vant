...
async function executePipeline(mode, ctx, final) {
    ...
    for (const name of chain) {
        ...
        if (!handler) {
            if (isCritical) {
                ...
                console.error(`[BRAIN PIPELINE] FATAL: ${name} not available - cannot proceed`);
                throw err;
            }
            console.warn(`[BRAIN PIPELINE] WARN: ${name} not available - skipping (optional)`);
            continue;
        }
        ...
        try {
            ...
            if (typeof handler === 'function') {
                ...
                try {
                    ...
                    console.warn(`[BRAIN PIPELINE] WARN: ${name}.can() failed - ${e.message}`);
                    continue;
                }
            } else {
                // Unknown handler type - warn but continue
                console.warn(`[BRAIN PIPELINE] WARN: ${name} has no valid execute method - skipping`);
                continue;
            }
            
            console.log(`[BRAIN PIPELINE] OK: ${name}`);
            ...
            if (isCritical) {
                ...
                console.error(`[BRAIN PIPELINE] FATAL: ${name} threw error: ${e.message}`);
                console.error(`[BRAIN PIPELINE] Stack: ${e.stack}`);
            ...
            console.warn(`[BRAIN PIPELINE] WARN: ${name} error - ${e.message} (optional - continuing)`);
        }
    }
    ...
    if (errors.length > 0) {
        _pipelineState.degraded = true;
        console.warn(`[BRAIN PIPELINE] WARN: Pipeline finished with ${errors.length} error(s):`, 
...
async function _heal(brain) {
    ...
    if (fallback) {
        console.log(`[brain] ${brain} missing, falling back to ${fallback}`);
