...
function _getBrainRegistry() {
    if (!_brainRegistry) {
        try {
            _brainRegistry = require('./brain-registry');
        } catch (e) {
            console.warn('[brain] brain-registry not available:', e.message);
            return null;
        }
    }
    return _brainRegistry;
}

// ==================== PATH CONSISTENCY ====================
function getBrainPath() {
    // Check for multi-brain registry first
    const registry = _getBrainRegistry();
    if (registry) {
        try {
            return registry.getBrainPath();
        } catch (e) {
            // Fall through to legacy path
        }
    }
    
    // Use config system (storage.path), fallback to env for backwards compat
    let brainPath;
    try {
        const config = require('./config');
        brainPath = config.get('storage.path') || config.storagePath();
    } catch (e) {
        brainPath = process.env.MODEL_PATH || process.env.VANT_BRAIN_PATH || process.env.VANT_STORAGE_PATH || 'models/private';
    }
...
module.exports = {
    ...
    brainUse: (name) => {
        const registry = _getBrainRegistry();
        if (!registry) return { error: 'registry_not_available' };
        return registry.use(name);
    },
    brainCreate: async (name, options) => {
        const registry = _getBrainRegistry();
        if (!registry) return { error: 'registry_not_available' };
        return await registry.create(name, options);
    },
    brainRemove: async (name) => {
        const registry = _getBrainRegistry();
        if (!registry) return { error: 'registry_not_available' };
    ...
    brainShare: async (toBrain, fileName) => {
        const registry = _getBrainRegistry();
        if (!registry) return { error: 'registry_not_available' };
        return await registry.share(toBrain, fileName);
    },
    brainListShared: () => {
        const registry = _getBrainRegistry();
        if (!registry) return [];
        return registry.listShared();
    },
    brainNeurons: () => {
        const registry = _getBrainRegistry();
        if (!registry) return { orgs: [], teams: [], relationships: [] };
        return registry.getNeurons();
    },
    brainSaveNeurons: (neurons) => {
        const registry = _getBrainRegistry();
        if (!registry) return { error: 'registry_not_available' };
        return registry.saveNeurons(neurons);
    },
    ...
    // NEW: Memory integration (unified memory)
    // Proxy learn/remember to memory module (lazy load to avoid circular dep)
    async learn(key, content, options) {
        const { memory } = require('./memory');
        return await memory.learn(key, content, options);
    },

    async remember(key, content, options) {
        const { memory } = require('./memory');
        if (content !== undefined) {
            return await memory.state(key, content, options);
        } else {
            return await memory.recall(key);
        }
    },

    // Also expose memory methods directly (lazy)
    get memory() {
