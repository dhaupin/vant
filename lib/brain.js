...
async function executePipeline(mode, ctx, final) {
    ...
    for (const name of chain) {
        ...
        if (!handler) {
            if (isCritical) {
                ...
                audit.error(`[BRAIN PIPELINE] FATAL: ${name} not available - cannot proceed`);
                throw err;
            }
            audit.warn(`[BRAIN PIPELINE] WARN: ${name} not available - skipping (optional)`);
            continue;
        }
        ...
        try {
            ...
            if (typeof handler === 'function') {
                ...
                try {
                    ...
                    audit.warn(`[BRAIN PIPELINE] WARN: ${name}.can() failed - ${e.message}`);
                    continue;
                }
            } else {
                // Unknown handler type - warn but continue
                audit.warn(`[BRAIN PIPELINE] WARN: ${name} has no valid execute method - skipping`);
                continue;
            }
            
            audit.info(`[BRAIN PIPELINE] OK: ${name}`);
            ...
            if (isCritical) {
                ...
                audit.error(`[BRAIN PIPELINE] FATAL: ${name} threw error: ${e.message}`);
                audit.error(`[BRAIN PIPELINE] Stack: ${e.stack}`);
            ...
            audit.warn(`[BRAIN PIPELINE] WARN: ${name} error - ${e.message} (optional - continuing)`);
        }
    }
    ...
    if (errors.length > 0) {
        _pipelineState.degraded = true;
        audit.warn(`[BRAIN PIPELINE] WARN: Pipeline finished with ${errors.length} error(s):`, 
...
async function _heal(brain) {
    ...
    if (fallback) {
        audit.info(`[brain] ${brain} missing, falling back to ${fallback}`);
