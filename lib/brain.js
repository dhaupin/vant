...
function register(name, handler) {
    ...
    if (name === '__proto__' || name === 'constructor' || name === 'prototype') {
        throw new errors.VantError('Invalid handler name: ' + name, { code: errors.CODES.BRAIN_HANDLER_INVALID, retryable: false });
    }
        ...
        try {
            ...
            if (typeof handler === 'function') {
                ...
                try {
                    ...
                    if (!result) {
                        throw new errors.VantError('Pipeline blocked: ' + name + ' denied ' + capability + ' for ' + ctx.name, { code: errors.CODES.BRAIN_PIPELINE_BLOCKED, retryable: false });
                    }
                } catch (e) {
...
async function writeTo(brain, key, data, options = {}) {
    // (pass 41 live-fire fix) This was BORN BROKEN in the multibrain commit:
    // it called storage.set(), which exists on StateStorage only -
    // BrainStorage has get/write but no set - so every call threw
    // 'storage.set is not a function'. Zero callers existed (why nothing
    // noticed), but the export advertised a working API. Route through the
    // same write() security chain as every other brain write
    // (VAF->QoS->Escrow), pinning the target brain via options.brain -
    // which write() has supported all along.
    if (!brain || typeof brain.name !== 'string' || !brain.name) {
        throw new errors.VantError('writeTo: brain { name, type } required', { code: errors.CODES.VAF_INPUT_INVALID });
    }
    if (brain.type !== undefined && brain.type !== 'private' && brain.type !== 'public') {
        throw new errors.VantError('writeTo: brain.type must be private or public', { code: errors.CODES.VAF_INPUT_INVALID });
    }
    if (typeof key !== 'string' || !key) {
        throw new errors.VantError('writeTo: key required', { code: errors.CODES.VAF_INPUT_INVALID });
    }
    // Delegate to format.saveFile (same mechanism as the MCP brain_write
    // tool) resolved against the target brain's root, so the file lands
    // FLAT in models/<type>/<brain>/<key>.<ext> - the layout brain.read()
    // can read back. The earlier draft of this fix delegated to write(),
    // but write(category, key, content, ...) would have treated the data
    // arg as the CATEGORY and the options object as content - arg-order rot.
    const format = require('./format');
    const chosenFormat = options.format || (typeof data === 'object' && data !== null ? 'json' : 'md');
    // read() probes name+extension, so append the format extension when the
    // key carries none of the known ones.
    const hasKnownExt = /\.(md|json|yaml|yml|txt|ini)$/i.test(key);
    const keyWithExt = hasKnownExt ? key : key + '.' + chosenFormat;
    const root = brain.type === 'public' ? _brainRoots.public : _brainRoots.private;
    return format.saveFile(path.join(root, brain.name, keyWithExt), data, {
        ...options,
        format: chosenFormat
    });
}
