...
async function _processDream(payload) {
    ...
    try {
        ...
        // Skip RLS check for now - dream is an internal operation
        // Can be enabled when habitat RLS policies are configured for _dream:*
        // const sandbox = getSandbox();
        // if (sandbox && sandbox.rls && sandbox.rls.getHabitat && sandbox.rls.getHabitat()) {
        //     try {
        //         await sandbox.rls.checkWrite(
        //             { brain: _currentBrain, role: 'dream' },
        //             `_dream:${_currentBrain}`,
        //             'write'
        //         );
        //     } catch (e) {
        //         _emit('dream:blocked', { reason: 'RLS denied', brain: _currentBrain });
