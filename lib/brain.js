...
async function restoreNeuronState(state) {
    if (!state || typeof state !== 'object') {
        throw new Error('Invalid neuron state to restore');
    }
...
async function load(brains, options = {}) {
    ...
    if (!Array.isArray(brains)) {
        throw new Error('brain.load() requires string or array, got: ' + typeof brains);
    }
    ...
    if (type === 'private') {
        ...
        if (fs.existsSync(privatePath)) {
            ...
            if (fs.existsSync(publicPath)) {
                ...
                throw new Error('Brain not found: ' + name);
            }
        }
    }
...
/**
 * Core logic for loading corpus - used by both sync and async
 * @private
 */
function _loadCorpusCore(brainPath, publicPath, mode, isSync) {
    const brain = {};
    
    // Load based on mode
    if (mode === 'public') {
        const files = readDir(publicPath, 'public', { sync: isSync });
        for (const { name, content, format } of files) {
            if (!brain[name]) brain[name] = { content, source: 'public', format };
        }
    } else if (mode === 'private') {
        const files = readDir(brainPath, 'private', { sync: isSync });
        for (const { name, content, format } of files) {
            if (!brain[name]) brain[name] = { content, source: 'private', format };
        }
    } else {
        // Dual mode: public first, then private overrides
        const publicFiles = readDir(publicPath, 'public', { sync: isSync });
        for (const { name, content, format } of publicFiles) {
            brain[name] = { content, source: 'public', format };
        }
        const privateFiles = readDir(brainPath, 'private', { sync: isSync });
        for (const { name, content, format } of privateFiles) {
            brain[name] = { content, source: 'private', format };
        }
    }
...
function loadCorpus(opts = {}) {
    ...
    if (isSync) {
        // SYNC MODE
        const brain = _loadCorpusCore(brainPath, publicPath, _mode, true);
        ...
        // ASYNC MODE - need to resolve promises from readDir
        return (async () => {
            // Manually handle async readDir results
            const brain = {};
            
            const processFiles = async (dirPath, source) => {
                const result = readDir(dirPath, source, { sync: false });
            ...
            if (_mode === 'public') {
                await processFiles(publicPath, 'public');
            } else if (_mode === 'private') {
                await processFiles(brainPath, 'private');
            } else {
                await Promise.all([
                    processFiles(publicPath, 'public'),
                    processFiles(brainPath, 'private')
                ]);
            }
