...
/**
 * Restore lessons.md from brain state on boot
 * If lessons.md doesn't exist but state has recentInsights, recreate it
 */
async function _restoreLessonsFromState(state) {
    try {
        ...
        // If lessons.md exists, nothing to restore
        if (lessons && lessons.content && lessons.content.length > 10) {
            return { restored: false, reason: 'exists' };
        }
        
        // If no recentInsights in state, create empty lessons.md
        const insights = state?.recentInsights || [];
        if (insights.length === 0) {
            // Create empty lessons.md template
            const template = `# Lessons
...
## Session Insights
- (Insights will appear here after each session)

`;
            try {
                await write('lessons', 'restored', template);
                return { restored: true, insights: 0 };
            } catch (e) {
                return { restored: false, error: e.message };
            }
        }
        
        // Convert recentInsights to lessons.md format
        const insightText = insights.map(i => 
            `- ${new Date(i.timestamp).toLocaleDateString()}: ${i.insight}`
        ).join('\n');
...
## Session Insights (Restored from State)
${insightText}

`;
        await write('lessons', 'restored', lessonsContent);
...
// Shutdown: End evolution session + enable dream consolidation
onShutdown(async () => {
    // End evolution session (saves recentInsights to state)
    await endEvolutionSession();
    // Enable dream consolidation for next cron run
    dream(true, 3);
});
