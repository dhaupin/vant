 ...
 * 
 * Pattern: uses lib/do.js for unified async/sync
 */
function loadCorpus(opts = {}) {
    // Return cached synchronously if available
    if (_corpusCache && _corpusCacheMode === _mode) {
        return opts.sync ? _corpusCache : Promise.resolve(_corpusCache);
    }
    ...
    // Use lib/do.js for unified async/sync
    const { do: doLoad } = require('./do');
    
    return doLoad((isSync) => {
        const brain = {};
        
        // Get files from directory
        const getFiles = (dirPath, source) => readDir(dirPath, source, { sync: isSync });
        
        // Process files from a directory
        const processDir = (dirPath, source) => {
            const files = getFiles(dirPath, source);
