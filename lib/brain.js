...
// Constants
const PUBLIC_DIR = 'models/public';
const DEFAULT_PRIVATE = 'models/private';

/**
 * Get private brain path (with traversal protection)
 */
function getBrainPath() {
    let brainPath = process.env.MODEL_PATH || process.env.VANT_BRAIN_PATH || DEFAULT_PRIVATE;
    // Block path traversal
    if (brainPath.startsWith('/') || brainPath.includes('..')) {
        brainPath = DEFAULT_PRIVATE;
    }
    return brainPath;
}
