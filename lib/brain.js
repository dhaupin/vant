...
// ==================== DREAMING ====================
// Background consolidation at night (low usage periods)
let _dreamState = null;
let _dreamEnabled = false;
let _dreamHour = 3; // 3am default
...
/**
 * Dream - consolidate corpus in background
 */
async function _dream() {
    if (!_dreamEnabled) return;
    const corpus = loadCorpus();
    // Merge learnings + lessons = consolidation
    const learnings = corpus.find(c => c.id === 'learnings');
    const lessons = corpus.find(c => c.id === 'lessons');
    if (learnings && lessons) {
        // Semantically merge -> returns consolidated
