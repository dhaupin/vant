...
/**
 * Async write with security chain (VAF → QoS → Escrow)
 */
async function writeSecure(category, key, content, options = {}) {
    ...
    try {
        const result = write(category, key, content);
        if (qosCleanup) qosCleanup();
        return result;
    } catch (e) {
        if (qosCleanup) qosCleanup();
...
/**
 * Async load with security chain (VAF → QoS)
 */
async function loadSecure(name, options = {}) {
    ...
    try {
        const result = await loadBrain(name, options);
        if (qosCleanup) qosCleanup();
        return result;
    } catch (e) {
        if (qosCleanup) qosCleanup();
...
// Save brain file (writes directly to brain path, not a subdirectory)
function save(name, content, options = {}) {
    return write('', name + '.md', content, options);
}
...
module.exports = {
    ...
    // Secure versions (with security chain)
    writeSecure,
    loadSecure,
    ...
    save,
    write,
    writeBrain: save,  // Alias for MCP compatibility
    append,
    get,
