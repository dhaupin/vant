...
function _validateInput(operation) {
    ...
    if (operation.length > 200) {
        throw new Error('Operation name too long (max 200)');
    }
}
...
function check(operation, maxDepth) {
    ...
    if (current >= max) {
        _emit('recursion:depthExceeded', { operation, depth: current, max });
        return { allowed: false, depth: current, max, reason: 'max_depth_exceeded' };
    }
    
    _depthMap.set(operation, current + 1);
    _emit('recursion:check', { operation, depth: current + 1, max, allowed: true });
...
function release(operation) {
    ...
    _emit('recursion:released', { operation, depth: newDepth });
}
