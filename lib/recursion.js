...
/**
 * Input validation via VAF
 * @param {string} operation - Operation name
 * @param {any} data - Data to validate
 */
function _validateInput(operation, data) {
    const vaf = _getVaf();
    if (vaf?.check) {
        try {
            vaf.check(operation, { type: 'string', maxLength: 200 });
        } catch (e) {
            throw new Error('Invalid operation: ' + e.message);
        }
    }
}
...
module.exports = {
    ...
    getLayerStatus: () => ({ name: 'RecursionGuard', type: 'security', version: '0.8.7', enabled: true }),
