...
async function fetch(url, options = {}) {
    ...
    // `system: true` opts SYSTEM-INITIATED requests out of the per-call gate:
    // sync/remote providers authorize at their entry point (explicit config,
    // escrow budget, recursion guard, per-provider circuits + timeout caps),
    // and defaultSandbox.canNetwork() is false-by-default — an unconditional
    // gate here silently bricks every configured sync install. Agents and
    // user-space fetches MUST NOT pass system; they stay gated as usual.
    const sb = _getSandbox();
    if (!system && sb && typeof sb.can === 'function' && !sb.can('canNetwork')) {
