...
module.exports = {
    ...
    timeFn,
    timeFnAsync,
    getLayerStatus: () => ({ name: 'Metrics', type: 'instrumentation', version: require('./version'), enabled: true }),
    isOperationAllowed: () => ({ allowed: true }),
    getStatus: () => ({ enabled: true, metrics: _metrics.size, counters: _counters.size }),

    // Multibrain
    getBrainMetricsConfig,
    setBrainMetricsConfig,
    getStackMetricsConfigs,
...
const _brainMetricsConfigs = {};

function getBrainMetricsConfig() {
    const brain = require('./brain');
    const brainName = brain.getCurrentBrain();
    return _brainMetricsConfigs[brainName] || { enabled: true };
}

function setBrainMetricsConfig(config) {
    const brain = require('./brain');
    const brainName = brain.getCurrentBrain();
    _brainMetricsConfigs[brainName] = config;
    return true;
}

function getStackMetricsConfigs() {
    const brain = require('./brain');
    ...
    for (const brainName of stack) {
        try {
            ...
            results.byBrain[brainName] = getBrainMetricsConfig();
        } catch (e) {
...
function gatherState() {
    return {
        metrics: Array.from(_metrics.entries()),
        counters: Array.from(_counters.entries()),
        gauges: Array.from(_gauges.entries()),
        timings: Array.from(_timings.entries()),
        count: _metrics.size,
        gatheredAt: Date.now()
    };
}
function restoreState(data) {
    ...
    _timings.clear();
    if (data) {
        if (data.metrics) data.metrics.forEach(([k,v]) => _metrics.set(k,v));
        if (data.counters) data.counters.forEach(([k,v]) => _counters.set(k,v));
        if (data.gauges) data.gauges.forEach(([k,v]) => _gauges.set(k,v));
        if (data.timings) data.timings.forEach(([k,v]) => _timings.set(k,v));
    }
    return { restored: true, metrics: _metrics.size };
}
