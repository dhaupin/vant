/**
 * Vant Stream Module (v0.8.6)
 * WITH EVENT EMISSIONS - async queue emits globally
 *
...
// v0.9.0-axolotl T9: bundle 5 lazy security loaders into one _getSecurity().
// Was 5 separate getters (sandbox/vaf/qos/escrow/encrypt) each with the same
// try/catch + null-check pattern. Now one fetch returns the full bundle.
//
// v0.9.0-axolotl T9b: removed the 5 individual getters (nuclear breaking —
// 0.8.6 → 1.0.0 is a fresh foundation, no backward-compat wrappers).
// Callers that need a single security module should destructure it from the
// bundle: `const { sandbox } = _getSecurity();`
let _sandbox = null;
let _vaf = null;
let _qos = null;
let _escrow = null;
...
function _getSecurity() {
    if (_securityLoaded) {
        return { sandbox: _sandbox, vaf: _vaf, qos: _qos, escrow: _escrow, encrypt: _encrypt };
    }
...
// v0.9.0-axolotl: registered with boot's timer lifecycle (was bare unref'd setInterval)
if (typeof setInterval !== 'undefined') {
    ...
    if (_bootMod && _bootMod.registerTimer) {
        _bootMod.registerTimer('stream.leaseSweep', _leaseSweep, 10000, { unref: true });
    } else {
        setInterval(_leaseSweep, 10000).unref();
    }
}

// Pipeline: sandbox → vaf → qos → escrow → method handler
async function _gate(operation, streamName, data = {}) {
    // v0.9.0-axolotl T9: single fetch instead of 4 separate lazy-loads
    const { sandbox: sb, vaf: v, qos: q, escrow: esc } = _getSecurity();
...
async function enqueue(streamName, task) {
    ...
    const { encrypt } = _getSecurity();
    const keyPart = encrypt && encrypt.key ? encrypt.key(16) : Math.random().toString(36).slice(2, 18);
    const id = 'w_' + Date.now().toString(36) + keyPart;
    ...
    // EVENT: enqueued
    _emit('stream:enqueued', { id, stream: streamName, timestamp: Date.now() });
...
async function poll(streamName, options = {}) {
    ...
    for (const item of queue) {
        if (item.status === 'pending') {
            ...
            // EVENT: polled
            _emit('stream:polled', { id: item.id, stream: streamName, timestamp: Date.now() });
...
async function complete(workId, result) {
    ...
    // EVENT: completed
    _emit('stream:completed', { id: workId, stream: item.stream, timestamp: Date.now() });
...
function list(streamName, options = {}) {
    // If no streamName provided, return all stream names
    if (!streamName) {
...
async function load(streamName) {
    try {
        ...
        const b = await brain.load(`stream_${streamName}`);
        if (b?.content) {
...
async function deleteStream(streamName) {
    ...
    // SECURITY: Use brain.write (includes VAF → QoS → Escrow by default)
    await brain.write('stream', streamName, null);
...
/**
 * List streams from all brains in the stack
 * @param {Object} options - Filter options
 * @returns {Array} Combined streams from all brains
 */
function listStack(options = {}) {
    ...
    for (const brainName of stack) {
        try {
            ...
            const streams = list(options);
            if (Array.isArray(streams)) {
                streams.forEach(s => {
                    results.push({ ...s, brain: brainName });
                });
            }
        } catch (e) {
...
/**
 * Get stream info from all brains in stack
 * @param {string} streamId - Stream ID
 * @param {Object} options - Options
 * @returns {Object} Stream info if found
 */
function getStackStreamInfo(streamId, options = {}) {
    const brain = require('./brain');
    const stack = brain.getStack();
    ...
    for (const brainName of stack) {
        try {
            brain.pushBrain(brainName);
            const streamInfo = info(streamId, options);
            if (streamInfo) {
                brain.removeBrain();
                return { ...streamInfo, brain: brainName };
            }
        } catch (e) {
            // Try next brain
        } finally {
...
/**
 * Get stats from all brains in stack
...
function getStackStats() {
    ...
    for (const brainName of stack) {
        try {
            ...
            results.totalStreams += s.streams ? s.streams.length : 0;
        } catch (e) {
