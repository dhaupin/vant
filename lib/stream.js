...
async function _persist(streamName) {
    ...
    const queue = _queues.get(streamName);
    if (queue) {
        // Use secure write with security chain
        if (brain.writeSecure) {
            await brain.writeSecure('stream', streamName, JSON.stringify(queue));
        } else {
...
async function load(streamName) {
    try {
        // Use secure load with security chain
        const b = brain.loadSecure 
            ? await brain.loadSecure(`stream_${streamName}`)
            : await brain.loadBrain(`stream_${streamName}`);
        if (b?.content) {
...
async function deleteStream(streamName) {
    ...
    // Use secure write to clear brain
    if (brain.writeSecure) {
        await brain.writeSecure('stream', streamName, null);
    } else {
