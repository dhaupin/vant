...
let _detectEngine = async () => {
    ...
    try {
        const result = await compute.evaluate('println(1 + 1)', { lang: 'julia' });
...
`;
        const result = await compute.evaluate(code, { lang: 'julia' });
...
`;
        return (await compute.evaluate(code, { lang: 'julia' })).stdout.trim();
    }
