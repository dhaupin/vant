...
function project(barcode) {
    ...
    const fingerprint = getFingerprint(theta, phi, depth);
    ...
    return {
        ...
        _key: `${Math.round(theta*100)}_${Math.round(phi*100)}_${Math.round(depth*1000)}`
    };
}
...
function reconstruct(key) {
    const [thetaStr, phiStr, depthStr] = key.split('_');
    
    return {
        theta: parseInt(thetaStr) / 100,
        phi: parseInt(phiStr) / 100,
        depth: parseInt(depthStr) / 1000
    };
}
