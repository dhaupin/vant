...
function _getSandboxMod() {
    // (F-2 fix) cache-resilient: during the gate→sandbox→vaf→storage→gate
    // require cycle, this can first run while sandbox.js is mid-init — caching
    // a PARTIAL export whose defaultSandbox lacks methods. Verify the member
    // we rely on before caching; retry until sandbox finishes initializing.
    if (_sandboxMod && _sandboxMod.defaultSandbox) return _sandboxMod;
    try {
        const candidate = require('./sandbox');
        if (candidate && candidate.defaultSandbox) _sandboxMod = candidate;
        return _sandboxMod;
    } catch (e) {
        return _sandboxMod || null;
    }
}
