...
function _getAudit() {
    if (!_audit) {
        try { _audit = require('./audit'); } catch (e) { _audit = { info: () => {}, warn: () => {}, error: () => {} }; }
    }
    return _audit;
}
...
function _checkRLS(userCtx, resource, operation = 'read') {
    ...
    if (sandbox && sandbox.rls) {
        // Use sandbox.rls for RLS (carrier pattern)
        const checkFn = operation === 'write' ? sandbox.rls.checkWrite : sandbox.rls.checkRead;
        if (checkFn) {
            return checkFn.call(sandbox.rls, userCtx || {}, resource, operation);
        }
    }
    return true; // Default allow if no RLS
}
...
function _getStorePath() {
    ...
    // (O-7/F-10) default: brain-scoped store INSIDE the models tree so orgchart
    // data is gathered by the brain-tree gather, committed, and horcrux-
    // transported via brainStorage. Config override preserved.
    let base = '.agent_tmp/teams.json';
    if (cfg && cfg.get) base = cfg.get('teams.store', base);
    if (base === '.agent_tmp/teams.json') {
        try {
            ...
            if (brainName && /^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(brainName)) {
                base = 'models/private/' + brainName + '/orgchart/teams.json';
            }
...
function assign(agentId, options = {}) {
    ...
    const team = _resolveTeamRef(options.team);
    const dept = _resolveDeptRef(options.dept) || (team ? _teams.get(team)?.dept : null);
    const org = _resolveOrgRef(options.org) || (dept ? _depts.get(dept)?.org : null);
    const role = _resolveRoleRef(options.role, team);
    const reportsTo = options.reportsTo; // Agent ID this agent reports to

    // MULTIBRAIN: Get brain - from options or current brain
    let brain = options.brain;
    if (!brain) {
        try {
            const brainMod = require('./brain');
...
function restoreState(data) {
    ...
    if (data) {
        // (O-8) Accept BOTH gatherState() Map-entries format ([k,v] pairs) and
        // the legacy store-file array format (plain entity objects) — old
        // horcruxes in the wild carry the array form.
        if (data.orgs) data.orgs.forEach((o) => { const [k, v] = Array.isArray(o) ? o : [o.id, o]; if (k && v) _orgs.set(k, v); });
        if (data.depts) data.depts.forEach((o) => { const [k, v] = Array.isArray(o) ? o : [o.id, o]; if (k && v) _depts.set(k, v); });
        if (data.teams) data.teams.forEach((o) => { const [k, v] = Array.isArray(o) ? o : [o.id, o]; if (k && v) _teams.set(k, v); });
        if (data.roles) data.roles.forEach((o) => { const [k, v] = Array.isArray(o) ? o : [o.id, o]; if (k && v) _roles.set(k, v); });
        if (data.assignments) data.assignments.forEach((o) => { const [k, v] = Array.isArray(o) ? o : [o.agentId, o]; if (k && v) _assignments.set(k, v); });
        // Persist the rehydrated state to the (brain-scoped) store
