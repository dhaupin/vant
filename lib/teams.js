...
// v0.9.0 fs->storage migration: the teams JSON store goes through
// FileStorage (containment/symlink/VAF/atomic-write). The store path stays
// config-driven ('teams.store'); dirname becomes the store's basePath.
let _teamsStore = null;
function _getStore() {
    if (!_teamsStore) {
        const storePath = _getStorePath();
        _teamsStore = new Storage.FileStorage({ basePath: path.resolve(path.dirname(storePath)) });
    }
    return _teamsStore;
}
