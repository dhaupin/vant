...
// Team persistence
const TEAMS_STORE = '.agent_tmp/teams.json';
...
function _ensureStoreDir() {
    const dir = path.dirname(TEAMS_STORE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
}

async function _loadTeams() {
    _ensureStoreDir();
    if (fs.existsSync(TEAMS_STORE)) {
        try {
            const data = JSON.parse(fs.readFileSync(TEAMS_STORE, 'utf8'));
...
async function _saveTeams() {
    _ensureStoreDir();
    fs.writeFileSync(TEAMS_STORE, JSON.stringify({
