...
function createDept(name, options = {}) {
    ...
    const orgId = _resolveOrgRef(options.org) || (Array.from(_orgs.values())[0]?.id);
...
function createTeam(name, options = {}) {
    ...
    // (D-1) options.dept accepts name or ID; org context inherited from the dept record
    const deptId = _resolveDeptRef(options.dept) || (Array.from(_depts.values())[0]?.id);
...
function createRole(name, options = {}) {
    ...
    const teamId = _resolveTeamRef(options.team) || (Array.from(_teams.values())[0]?.id);
...
function restoreState(data) {
    ...
    _assignments.clear();
    if (data) {
        if (data.orgs) data.orgs.forEach(([k,v]) => _orgs.set(k,v));
        if (data.depts) data.depts.forEach(([k,v]) => _depts.set(k,v));
        if (data.teams) data.teams.forEach(([k,v]) => _teams.set(k,v));
        if (data.roles) data.roles.forEach(([k,v]) => _roles.set(k,v));
        if (data.assignments) data.assignments.forEach(([k,v]) => _assignments.set(k,v));
    }
    return { restored: true, orgs: _orgs.size };
}
