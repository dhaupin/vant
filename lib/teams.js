...
function _hydrateTeams() {
    ...
    try {
        ...
        let merged = 0;
        if (data.orgs) data.orgs.forEach((a) => { if (!_orgs.has(a.id)) { _orgs.set(a.id, a); merged++; } });
        if (data.depts) data.depts.forEach((a) => { if (!_depts.has(a.id)) { _depts.set(a.id, a); merged++; } });
        if (data.teams) data.teams.forEach((a) => { if (!_teams.has(a.id)) { _teams.set(a.id, a); merged++; } });
        if (data.roles) data.roles.forEach((a) => { if (!_roles.has(a.id)) { _roles.set(a.id, a); merged++; } });
        if (data.assignments) data.assignments.forEach((a) => { if (!_assignments.has(a.agentId)) { _assignments.set(a.agentId, a); merged++; } });
