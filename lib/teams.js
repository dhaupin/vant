...
function _getEscrow() {
    if (!_escrow) {
        try { 
            ...
            _escrow = escrowModule.create ? escrowModule : null;
        } catch (e) {
            _escrow = null;
        }
    }
...
async function deleteDept(deptId) {
    ...
    // CLEANUP: Remove assignments referencing this dept
    for (const [agentId, assign] of _assignments) {
...
async function assign(agentId, options = {}) {
    ...
    // Note: We only check assignments since that's where we track hierarchy
    ...
    if (escrow && org) {
        const budgetCheck = escrow.canSpend(org, 1); // 1 agent = 1 cost unit
    ...
    if (escrow && org) {
        escrow.recordSpend(org, 1); // Record 1 cost unit
    }
