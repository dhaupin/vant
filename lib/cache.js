...
class Cache {
    ...
    // Framework interface
    getLayerStatus() {
        return { name: 'Cache', type: 'unified-cache', version: require('./version'), enabled: true };
    }
...
// ==================== EXPORTS ====================
// v0.9.0-axolotl T13b: the module-level `defaultCache` singleton and the
// 30+ method aliases that proxied to it are removed. Every consumer in
// lib/ now instantiates its own `new Cache()` (brain, network, tmp,
// vant). Canonical usage:
//   const { Cache } = require('./cache');
