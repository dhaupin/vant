...
// audit was CALLED throughout this module but never required — every
// CLI-path commit/checkout/merge completed its git work then crashed with
// 'audit is not defined' (same live-bug class as the vaf 1 finding).
...
function git(args, options = {}) {
    ...
    if (useProvider && _getProvider()) {
        ...
        throw new errors.VantError('Use provider methods directly', { code: errors.CODES.BRANCH_USE_PROVIDER, retryable: false });
    }

    // CLI fallback — argv-array via execFileSync, NO shell. The old
    // execSync(`git ${args.join(' ')}`) shell-interpolated every arg
    // (agent ids become branch segments; commit messages originate in
    // brain content) — the same injection class fixed in
    // bin/branch-manager.js and the connectors.
    ...
    try {
        ...
        throw new errors.VantError('Git error: ' + e.message, { code: errors.CODES.GIT_ERROR, retryable: true });
    }
}
...
async function checkout(agentId, create = true) {
    ...
    throw new errors.VantError('Branch ' + branchName + ' does not exist', { code: errors.CODES.BRANCH_NOT_FOUND, retryable: false });
}
...
async function commit(agentId, message = 'Agent update') {
    // VAF validation for commit message. allowContent:true: a commit
    // message is arbitrary TEXT stored in git history, not a command —
    // and since git() went argv-array (no shell), metacharacters in the
    // message are inert. The old strict check rejected legitimate brain
    // quotes containing ';', backticks, etc. Agent id stays strict
    // (it becomes a branch path segment).
    vaf.check(message, {
        type: 'string',
        name: 'commit message',
        maxLength: 100000,
...
async function createPR(options) {
    if (!options || !options.title) {
        throw new errors.VantError('Title required for PR', { code: errors.CODES.BRANCH_TITLE_REQUIRED, retryable: false });
    }
