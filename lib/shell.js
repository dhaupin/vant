...
function _validateCommand(cmd) {
    if (!cmd || typeof cmd !== 'string') throw new errors.VantError('Invalid command', { code: errors.CODES.SHELL_EXEC_DENIED, retryable: false });
    ...
    if (!_getAllowedCommands().has(command)) {
        ...
        throw new errors.VantError('EDENIED: command not in whitelist', { code: errors.CODES.SHELL_EXEC_DENIED, retryable: false });
    }
...
function _checkTimeout(timeout) {
    ...
    if (t > MAX_TIMEOUT) throw new errors.VantError('ETIMEOUT: max timeout exceeded', { code: errors.CODES.SHELL_EXEC_DENIED, retryable: false });
    return t;
}
...
async function exec(cmd, options = {}) {
    ...
    if (!sudo.can(_getTaskId() || 'default', 'exec')) {
        throw new errors.VantError('EPERM: exec not allowed by sudo', { code: errors.CODES.SUDO_DENIED, retryable: false });
    }

    // 2. Sandbox: canExec (additional check)
    if (sb && typeof sb.can === 'function' && !sb.can('canExec')) {
        throw new errors.VantError('EPERM: canExec not allowed by sandbox', { code: errors.CODES.SANDBOX_EXEC_DENIED, retryable: false });
    }

    // 2. QoS: rate limit
    const q = qos?.canProceed?.('exec');
    if (q === false) {
        throw new errors.VantError('E429: rate limited', { code: errors.CODES.RATE_LIMIT_EXCEEDED, retryable: true });
    }

    // 3. Escrow: budget
    const budget = esc?.reserve?.('exec', 1) || 1;
    if (budget <= 0) {
        throw new errors.VantError('EBUDGET: insufficient budget', { code: errors.CODES.SANDBOX_BUDGET_EXCEEDED, retryable: true });
    }

    // 4. Lock: serialize
    // Pass-19 lock API fix: acquire() resolves to a token string; call
    // release(id, token) to free. The old code treated the token as a
    // function and invoked it at the end of every exec.
    const lockToken = await lock?.acquire?.('shell:exec');
    if (!lockToken) {
        throw new errors.VantError('ELOCK: could not acquire', { code: errors.CODES.LOCK_FAILED, retryable: true });
    }
    const release = () => lock.release?.('shell:exec', lockToken);
...
async function spawn(cmd, options = {}) {
    ...
    if (!sudo.can(_getTaskId?.() || 'default', 'shell')) {
        throw new errors.VantError('EPERM: shell not allowed by sudo', { code: errors.CODES.SUDO_DENIED, retryable: false });
    }
    if (sb && typeof sb.can === 'function' && !sb.can('canExec')) {
        throw new errors.VantError('EPERM: canExec not allowed', { code: errors.CODES.SANDBOX_EXEC_DENIED, retryable: false });
    }
