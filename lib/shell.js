/**
 * Shell - Command execution with full security chain (v0.8.7)
...
function _validateCommand(cmd) {
    ...
    // Check whitelist - use function for dynamic
    if (!_getAllowedCommands().has(command)) {
        ...
        for (const d of dangerous) {
            if (safe.toLowerCase().includes(d)) {
                throw new errors.Error('EDENIED: dangerous command blocked', { code: errors.CODES.SHELL_EXEC_DENIED, retryable: false });
            }
        }
    }
