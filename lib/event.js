...
// ==================== RECURSION GUARD ====================
const _emitStack = new Map();  // type -> depth
let _config = null;
function _getConfig() {
    if (!_config) {
...
const MAX_EMIT_DEPTH = () => {
    const cfg = _getConfig();
    return cfg?.get ? cfg.get('event.maxEmitDepth', 10) : 10;
};
...
class Event {
     ...
     */
    emit(type, data) {
        // Recursion guard: prevent infinite emit loops
        const currentDepth = _emitStack.get(type) || 0;
        if (currentDepth >= MAX_EMIT_DEPTH()) {
            // Emit blocked event for monitoring
            const { audit } = require('./audit') || { error: () => {} };
            console.warn('[Event] Recursion blocked: ' + type + ' at depth ' + currentDepth);
            return -1;  // -1 indicates blocked
        }
        
        _emitStack.set(type, currentDepth + 1);
        ...
        try {
            ...
            _emitStack.set(type, currentDepth);
        }
    }
