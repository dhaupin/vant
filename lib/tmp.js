...
// Security: Load on demand
function _getQoS() {
    let qosMod = global._qos;
    if (!qosMod) try { qosMod = global._qos = require('./qos'); } catch (e) {}
    return qosMod;
}
...
async function dropboxPut(name, content) {
    // 1. Sudo: canTmp?
    if (!sudo.can('default', 'write')) {
        throw new Error('EPERM: tmp not allowed by sudo');
    }
    ...
    // 2. QoS: rate limit
    if (!qos?.canProceed?.('tmp:write')) throw new Error('E429: rate limited');
    
    // 3. Escrow: budget
    const budget = esc?.reserve?.('tmp', 1) || 1;
    if (budget <= 0) throw new Error('EBUDGET: insufficient');
    
    // 4. Lock: serialize
    const release = await lock?.acquire?.('tmp:dropbox:write');
...
async function myStuffPut(name, content) {
    ...
    const budget = esc?.reserve?.('tmp', 1) || 1;
    if (budget <= 0) throw new Error('EBUDGET: insufficient');
    const release = await lock?.acquire?.('tmp:mystuff:write');
    if (!release) throw new Error('ELOCK: could not acquire');
...
async function yourStuffPut(name, content) {
    ...
    const budget = esc?.reserve?.('tmp', 1) || 1;
    if (budget <= 0) throw new Error('EBUDGET: insufficient');
    const release = await lock?.acquire?.('tmp:yourstuff:write');
    if (!release) throw new Error('ELOCK: could not acquire');
