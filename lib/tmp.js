...
class TmpSpace {
    ...
    _checkPerm(op) {
        ...
        if (this.secured) {
            ...
            if (!sudo.can(taskId, scope)) {
                throw new errors.VantError('EPERM: ' + scope + ' not allowed by sudo', { code: errors.CODES.SUDO_RESTRICTED, retryable: false });
            }
        }
        ...
        if (sb && typeof sb.can === 'function') {
            if (op === 'write' && !sb.can('canWrite')) throw new errors.VantError('EPERM: canWrite not allowed', { code: errors.CODES.STORAGE_WRITE_DENIED, retryable: false });
            if (op === 'read' && !sb.can('canRead')) throw new errors.VantError('EPERM: canRead not allowed', { code: errors.CODES.STORAGE_READ_DENIED, retryable: false });
            if (op === 'delete' && !sb.can('canDelete')) throw new errors.VantError('EPERM: canDelete not allowed', { code: errors.CODES.STORAGE_DELETE_DENIED, retryable: false });
        }
        ...
        if (budget <= 0) throw new errors.VantError('EBUDGET: insufficient', { code: errors.CODES.BUDGET_INSUFFICIENT, retryable: false });
    ...
    _checkLimits() {
        ...
        if (fs.readdirSync(dir).length >= this.maxFiles) {
            throw new errors.VantError('ELIMIT: max files reached', { code: errors.CODES.STORAGE_LIMIT_EXCEEDED, retryable: false });
        }
        return true;
    }
    ...
    // Sanitize filename
    _sanitize(name) {
        if (!name || typeof name !== 'string') throw new errors.VantError('Invalid name', { code: errors.CODES.VAF_INPUT_INVALID, retryable: false });
        const safe = vaf.sanitize(name);
        if (safe.includes('..') || safe.includes('/') || safe.includes('\\')) {
            throw new errors.VantError('EPATH: path traversal blocked', { code: errors.CODES.VAF_PATH_TRAVERSAL, retryable: false });
        }
    ...
    async put(name, content) {
        ...
        if (!token) throw new errors.VantError('ELOCK: could not acquire', { code: errors.CODES.LOCK_ACQUIRE_FAILED, retryable: true });
        ...
        try {
            if (content.length > this.maxFileSize) throw new errors.VantError('ESIZE: too large', { code: errors.CODES.STORAGE_LIMIT_EXCEEDED, retryable: false });
