/**
 * Tmp - Unified temporary storage for AI-first OS
 * 
 * Provides: dropbox, myStuff, yourStuff, cache
...
class TmpSpace {
    ...
    _getPath() {
        ...
        return path.join(storagePath, this.name === 'dropbox' ? 'tmp/dropbox' : 
...
const Tmp = {
    ...
    getLayerStatus: () => ({ 
        ...
        enabled: true, 
        secured: true,
        spaces: ['dropbox', 'myStuff', 'yourStuff', 'cache'],
        capabilities: ['put', 'get', 'list', 'delete', 'clear'],
        chain: ['sudo', 'sandbox', 'qos', 'escrow', 'lock', 'vaf', 'audit']
    }),
    getStatus: () => ({ enabled: true, taskId: _getTaskId(), spaces: ['dropbox', 'myStuff', 'yourStuff', 'cache'] }),
