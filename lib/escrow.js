...
class Escrow {
    ...
    getBreaker(serviceName) {
        ...
        if (!breaker) {
            breaker = new CircuitBreaker({ threshold: this.options.circuitThreshold, timeout: this.options.circuitTimeout });
            this._breakers.set(serviceName, breaker);
        }
        return breaker;
    }
    
    isOpen(serviceName) { return this.getBreaker(serviceName).getState() === "OPEN"; }
    recordFailure(serviceName) { this.getBreaker(serviceName).recordFailure(); }
    recordSuccess(serviceName) { this.getBreaker(serviceName).recordSuccess(); }
    
    // ==================== INTEGRATION ====================
    beforeExecute(context = {}) {
        const { agentId, operation, service, cost } = context;
        const results = {};
        ...
        const allowed = (results.budget?.allowed !== false) && (results.quota?.allowed !== false) && (results.circuit?.open !== true);
