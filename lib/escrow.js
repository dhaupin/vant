...
const RUNAWAY_CONFIG = {
    maxSpendsPerWindow: 30,     // Max spends per window
    windowMs: 60000,            // 1 minute window
    maxSpendsPerHour: 1000,     // Max spends per hour
    hourlyWindowMs: 3600000,
    alertThreshold: 20           // Alert at 20 spends/minute
};
...
module.exports = { Escrow, create: (o) => new Escrow(o),
    ...
    execute: async (ctx) => {
        ...
        if (!result.allowed) {
            throw new Error(`Escrow: ${result.results?.budget?.reason || 'Budget exceeded'}`);
        }
        return result;
    },
