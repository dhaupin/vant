...
function _getStorePath() {
    ...
    let base = '.agent_tmp/escrow.json';
    if (cfg && cfg.get) base = cfg.get('escrow.store', base);
    // (O-7 follow-up) default: brain-scoped store inside the models tree so
    // escrow budgets are gathered/committed/horcrux-transported like the rest
    // of the orgchart state. Config override preserved.
    if (base === '.agent_tmp/escrow.json') {
        try {
            ...
            if (brainName && /^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(brainName)) {
                base = 'models/private/' + brainName + '/orgchart/escrow.json';
            }
...
function _loadEscrow() {
    ...
    try {
        // models-tree store → FileStorage (vaf path-check + gate + containment)
        const rel = path.relative(path.join(__dirname, '..'), path.resolve(storePath));
        const raw = new (require('./storage').FileStorage)({ basePath: path.join(__dirname, '..') }).read(rel);
...
function _saveEscrow(data) {
    ...
    try {
        ...
        const rel = path.relative(path.join(__dirname, '..'), path.resolve(storePath));
        new (require('./storage').FileStorage)({ basePath: path.join(__dirname, '..') }).write(rel, JSON.stringify(data, null, 2));
