...
function _voteInternal(topic, outcome, agentId, options) {
    ...
    // (pass 40 / agora) Scope gate: a scoped ledger only accepts votes from
    // its member set. Deny-first: unknown owner entity or malformed scope
    // on a ledger with a scope field = denied.
...
function tally(topic) {
    ...
    // SECURITY: Check minimum quorum (use weight if trust enabled)
    const quorumVotes = useTrustWeight ? totalWeight : totalVotes;
    if (quorumVotes < ledger.minQuorum) {
        results.status = 'quorum';  // Waiting for more votes
        results.quorumNeeded = ledger.minQuorum - quorumVotes;
