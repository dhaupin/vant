...
// Create a new vote (SECURED)
function create(topic, options = {}) {
    const now = Date.now();
    
    // OPTIONS for security
    const minQuorum = options.minQuorum || 2;           // Min voters (default 2)
    const deposit = options.deposit || 0;                   // Stake required
...
// Cast vote (PROTECTED + CRYPTO + SECURED)
...
module.exports = {
    ...
    _auditLog,
    _audit,
    getLayerStatus: () => ({ name: 'Consensus', type: 'voting', version: '0.8.9', enabled: true, protected: true, crypto: true, verified: true, events: true }),
    isOperationAllowed: () => ({ allowed: true }),
    getStatus: () => ({ enabled: true, ledgers: _ledgers.size, protected: true, crypto: true, verified: true, events: true, listeners: _triggers.size })
};