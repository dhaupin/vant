...
function _createInternal(topic, options) {
    ...
    const ledger = {
        ...
        // (pass 58 / Wave C) Birth origin: created LOCALLY via consensus.create.
        // The synced-ledger reaper refuses any ledger carrying this flag —
        // sync provenance (syncedFrom) is not enough on its own, because
        // mergeTopic used to stamp it onto local topics that merely adopted
        // a remote ballot. Local ledgers are never reaped, by construction.
        localOrigin: true
    };
...
// Get vote info
function get(topic, viewerId) {
    // Return COPY to prevent reference manipulation
    const ledger = _ledgers.get(topic);
    if (!ledger) return null;
    // (pass 65 / star live-fire) SCOPE GATE ON THE READ PATH — the same
    // "scoped means unseen" rule the vote path has enforced since pass
    // 50. The star exercise caught a NON-MEMBER hub reading a scoped JV
    // ledger through get(): the vote gate held, the read gate did not
    // exist. Backward-compatible: no viewerId = the caller declares
    // itself trusted (internal callers: forum's decision-return path,
    // agora-sync's merge/summary internals); observability surfaces MUST
    // pass the viewer's principal. A scoped topic with an unresolvable
    // or non-member viewer returns null — same shape as a missing topic,
    // so existence itself stays invisible.
    if (ledger.scope) {
        if (viewerId === undefined) {
            // Legacy trusted-internal call: honor it, but scoped-ledger
            // reads through the un-gated door are auditable.
            try { require('./audit').info('[consensus] scoped ledger read WITHOUT viewerId: ' + ledger.topic); } catch (e) {}
        } else if (viewerId === null || !require('./scope').canAccess({ scope: ledger.scope }, viewerId)) {
            return null; // non-member: indistinguishable from not-found
        }
    }
...
function list(viewerId) {
    _hydrate();
    // (pass 65 / star live-fire) Scope filter on the read path: a viewer
    // principal sees scoped ledgers only where canAccess admits them;
    // unscoped ledgers flow to everyone. viewerId === undefined keeps
    // the legacy full list for trusted-internal callers (gossip summary
    // pre-filters its own, tests); observability surfaces pass a
    // principal (or null for the anonymous counts-only view).
    return Array.from(_ledgers.values())
        .filter((l) => {
            if (!l.scope || viewerId === undefined) return true;
            if (viewerId === null) return false; // anonymous: scoped topics are unnameable
            try { return !!require('./scope').canAccess({ scope: l.scope }, viewerId); }
        ...
        deadline: l.deadline,
        // (pass 58 / Wave C) Sync provenance + visibility flags for the
        // gossip summary leg and mesh hygiene surfaces. Additive fields:
        // syncedFrom marks wire-born ledgers (the reaper's only candidates);
        // localOrigin marks locally created ones (never reapable); scoped
        // flags scope-gated visibility (never summarized to non-members —
        // the OWNER filters, the wire carries no members).
...
// (pass 58 / Wave C) The synced-ledger reaper's single entry point.
// Deletes ONLY wire-born ledgers (syncedFrom-stamped by mergeTopic) that a
// predicate accepts — LOCALLY CREATED LEDGERS ARE UNREACHABLE HERE. The
// predicate receives a COPY-safe view and must decide age/retain rules;
// the age default lives in agora-sync (24h), not in consensus. Reaped
// topics are re-pullable by design (the pull seam is the recovery path).
// Returns the reaped topic names (empty when nothing qualified).
function reapSynced(predicate, opts = {}) {
    const max = parseInt(opts.max, 10) || 500; // batch bound: one pass, bounded work
    const reaped = [];
    _hydrate();
    for (const [topic, ledger] of _ledgers) {
        if (reaped.length >= max) break;
        // HARD GUARD (pass 58): reapable = wire-BORN ledgers only. Both
        // conditions must hold: the syncedFrom stamp (mergeTopic creation
        // path) AND absence of the localOrigin birth flag (consensus.create).
        // A locally created ledger that adopted remote ballots keeps
        // localOrigin:true and is unreachable here — the PRD's #1 reaper
        // invariant ("NEVER reaps locally-created ledgers") is enforced in
        // consensus itself, not in the caller's predicate.
        if (!ledger.syncedFrom || ledger.localOrigin) continue;
        let verdict = false;
        try {
            verdict = !!predicate(topic, {
                status: ledger.status,
                votes: Object.keys(ledger.votes || {}).length,
                syncedFrom: ledger.syncedFrom,
                created: ledger.created,
                deadline: ledger.deadline,
                metadata: ledger.metadata || {}
            });
        } catch (e) { verdict = false; } // a broken predicate can never reap
        if (verdict) {
            _ledgers.delete(topic);
            reaped.push(topic);
        }
    }
    if (reaped.length > 0) _persist();
    return reaped;
}
...
function mergeTopic(ledger) {
    ...
    if (existing) {
        // (pass 58) Adoption provenance: record WHO we last adopted from on
        // every merge, but NEVER brand a locally created ledger as synced
        // (syncedFrom stays unset for localOrigin ledgers — the reaper's
        // guard reads it). Provenance for audit lives in lastSyncFrom.
        ...
        //
        // (pass 51 / live-fire) SCOPE FILTER on adoption: a scoped topic
        // only ever counts ballots from its member set — the same rule the
        // local vote gate enforces. A wire snapshot is untrusted input, not
        // a trust oracle: without this filter a registered peer could push
        // a scoped ledger with its own pre-stuffed ballot and the owner
        // would tally it (the envelope vote path gates scope; the sync
        // path must too). Unresolvable member set = adopt nothing
        // (fail-closed — we cannot prove the ballots are legitimate).
        let scopeMembers = null;
        if (existing.scope && existing.scope.owner) {
            try {
                scopeMembers = require('./scope').resolveMembers(existing.scope.owner);
            } catch (e) { scopeMembers = null; }
            if (!scopeMembers) return { merged: false, reason: 'scope_unresolved' };
        }
        let adopted = 0;
        for (const [agentId, voteObj] of Object.entries(ledger.votes || {})) {
            if (!existing.votes[agentId]) {
                if (scopeMembers && !scopeMembers.has(agentId)) continue; // non-member ballot: drop
                existing.votes[agentId] = voteObj;
                adopted++;
            }
        }
        if (adopted > 0) {
            // (pass 58) Adoption provenance: record WHO we last adopted from
            // (audit trail), but NEVER brand a locally created ledger as
            // synced — syncedFrom stays unset for localOrigin ledgers and
            // the reaper's guard reads it. Wire-shaped bounded string.
            existing.lastSyncFrom = typeof ledger.syncedFrom === 'string'
                ? ledger.syncedFrom.slice(0, 64)
                : (typeof ledger.from === 'string' ? ledger.from.slice(0, 64) : existing.lastSyncFrom || 'unknown');
    ...
    const deadline = Number.isFinite(ledger.deadline)
        ...
        : created + 3600000;
    // (pass 51 / live-fire) A wire-born SCOPED topic must prove its ballots
    // are from members too: resolve the scope locally and drop non-member
    // ballots; an unresolvable scope rejects the merge entirely (fail-closed
    // — the crew-bus envelope scope gate should have kept this snapshot away
    // from a non-member node anyway; this is the belt to that suspenders).
    let wireScopeMembers = null;
    if (ledger.scope && ledger.scope.owner) {
        try {
            wireScopeMembers = require('./scope').resolveMembers(ledger.scope.owner);
        } catch (e) { wireScopeMembers = null; }
        if (!wireScopeMembers) return { merged: false, reason: 'scope_unresolved' };
    }
    const votes = {};
    for (const [agentId, voteObj] of Object.entries(ledger.votes || {})) {
        ...
        if (wireScopeMembers && !wireScopeMembers.has(agentId)) continue; // non-member ballot: drop
    ...
    const rebuilt = {
        ...
        // (pass 58) This stamp marks WIRE-BORN ledgers only — the adopt path
        // above must not set it on locally created topics.
