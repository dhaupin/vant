...
// ==================== ATOMIC FILE WRITE (fs-only, cycle-safe) ====================
//
// Lives in error.js on purpose: this module has ZERO Vant-module requires at
// load time (event/audit are lazy), so ANY module — including the ones inside
// the brain↔storage↔gate require cycle — can use it without adding an edge to
// the cycle. (Audit P2 #27: "Add atomic writes (temp + rename) to all file
// operations".)
//
// Contract: write to a hidden same-dir temp file, fsync, then rename over the
// final path. A crash mid-write can leave the TEMP file behind (harmless,
// swept by `vant clean cache`) but never a truncated/partial FINAL file.
const _afs = require('fs');
const _apath = require('path');
const _acrypto = require('crypto');

function atomicWriteFile(filePath, content) {
    if (typeof filePath !== 'string' || !filePath) {
        throw new Error('atomicWriteFile: filePath required');
    }
    const dir = _apath.dirname(filePath);
    _afs.mkdirSync(dir, { recursive: true });
    const tempPath = _apath.join(dir, `.${_apath.basename(filePath)}.${_acrypto.randomUUID()}.tmp`);
    let fd = null;
    try {
        fd = _afs.openSync(tempPath, 'w');
        _afs.writeFileSync(fd, content);
        try { _afs.fsyncSync(fd); } catch (e) { /* some filesystems refuse fsync on files */ }
    } finally {
        if (fd !== null) { try { _afs.closeSync(fd); } catch (e) { /* already closed */ } }
    }
    _afs.renameSync(tempPath, filePath);
}
