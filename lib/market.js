...
// Allow under the untouched default sandbox stub (one-time warning);
// enforce strictly once a real sandbox configuration is in place.
let _defaultMarketCan = null;
let _warnedDefaultMarket = false;
function _checkCapabilitySafe(capability) {
    const sandbox = _getSandbox();
    if (!sandbox || !sandbox.can) {
        return { allowed: true };
    }
    if (_defaultMarketCan === null) {
        _defaultMarketCan = sandbox.can;
    }
    if (sandbox.can === _defaultMarketCan) {
        if (!_warnedDefaultMarket) {
            _warnedDefaultMarket = true;
            // eslint-disable-next-line no-console
            console.warn('[market] Sandbox not configured; market allows by default. ' +
