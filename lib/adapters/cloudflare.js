...
function _getConnector() {
  if (!_connector) {
    try { _connector = require('../connectors/cloudflare'); } catch (e) {}
  }
  return _connector;
}
...
function setTransport(type, connector) {
  ...
  if (!validTypes.includes(type)) {
    throw new _getErrors().Error('Invalid transport type: ' + type, {
      code: _getErrors().CODES.CONFIGURATION_INVALID,
      retryable: false,
    });
  }
...
async function _ensureConnected() {
  if (!_transport) {
    throw new _getErrors().Error('No transport set', {
      code: _getErrors().CODES.BRAIN_NOT_INITIALIZED,
      retryable: false,
    });
  }
  ...
  return {
    ...
    async handshake(chainId, identity = {}) {
      ...
      if (_transportType === 'cloudflare') {
        ...
        throw new _getErrors().Error('Unsupported transport for sync', {
          code: _getErrors().CODES.RUNTIME_EXECUTION_FAILED,
          retryable: false,
        });
      }
  ...
  return {
    ...
    async get(key) {
      ...
      if (_transportType !== 'cloudflare' || !_transport.kvGet) {
        throw new _getErrors().Error('Transport does not support KV', {
          code: _getErrors().CODES.STORAGE_NOT_FOUND,
          retryable: false,
        });
      }
  ...
  return {
    ...
    async get(key) {
      ...
      if (_transportType !== 'cloudflare' || !_transport.r2Get) {
        throw new _getErrors().Error('Transport does not support R2', {
          code: _getErrors().CODES.STORAGE_NOT_FOUND,
          retryable: false,
        });
      }
    ...
    /**
     * Delete object (connector r2Delete via S3 client, v0.9.0-axolotl)
     */
    async delete(key) {
      await _ensureConnected();

      if (_transportType !== 'cloudflare' || !_transport.r2Delete) {
        throw new _getErrors().Error('Transport does not support R2 delete', {
          code: _getErrors().CODES.STORAGE_NOT_FOUND,
          retryable: false,
        });
      }

      const result = await _transport.r2Delete(key);

      _emit('cf:r2:delete', { key, transport: _transportType });
  ...
  return {
    ...
    async call(name, body, options = {}) {
      ...
      if (_transportType !== 'cloudflare' || !_transport.workerCall) {
        throw new _getErrors().Error('Transport does not support Workers', {
          code: _getErrors().CODES.RUNTIME_EXECUTION_FAILED,
          retryable: false,
        });
      }
