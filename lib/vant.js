...
async function wake(options = {}) {
    const { mcpPort = 3457, apiPort = 3456, debug = false } = options;

    if (AGENT_STATE.status !== 'sleeping') {
...
/**
 * Start Trifecta mode - Both MCP (JSON-RPC) + REST API
 * The full Vant experience: MCP for agents, REST for everything else
 *
 * @param {object} options - Configuration
 * @param {number} options.mcpPort - MCP port (default: 3457)
 * @param {number} options.apiPort - REST API port (default: 3456)
 * @param {boolean} options.debug - Enable debug logging
 * @returns {Promise<object>} Startup result
 */
async function startTrifecta(options = {}) {
    const {
        mcpPort = 3457,
        apiPort = 3456,
        debug = false,
    ...
    if (debug) console.log('[vant] Starting trifecta...');

    // Step 1: Init agent (includes boot.init())
    await init({ taskId, debug });

    // Step 2: Start MCP server (JSON-RPC for agents)
    if (debug) console.log('[vant] Starting MCP on port', mcpPort);
    const mcp = require('./mcp');
    await mcp.start({ port: mcpPort });

    // Step 3: Start REST API server
    if (debug) console.log('[vant] Starting API on port', apiPort);
    const { Server } = require('./server');
    const apiServer = new Server({ port: apiPort });
    await apiServer.listen();

    if (debug) console.log('[vant] Trifecta ready!');
    
    return {
        started: true,
        mode: 'trifecta',
        status: AGENT_STATE.status,
        ports: { mcp: mcpPort, api: apiPort },
        endpoints: {
