...
// v0.9.0-axolotl T13b: vant owns its own Cache instance. The
...
// NEW: Runtime operator (now from pipeline, absorbed runop)
let _pipeline = null;
const getPipeline = () => {
...
// ==================== COMPUTE & EMBED (from framework.js) ====================

const qos = require('./qos');
const escrow = require('./escrow');

const _computeRateLimit = new qos.RateLimiter({ windowMs: 60000, maxPerMinute: 10 });
const _embedRateLimit = new qos.RateLimiter({ windowMs: 60000, maxPerMinute: 50 });

/**
 * Compute with security wrapping (VAF + QoS + Escrow)
 */
async function computeEval(code, options = {}) {
    const { lang = 'node' } = options;

    const opKey = 'compute:eval';
    
    if (!_computeRateLimit.check(opKey)) {
        throw new errors.VantError('Compute rate limit exceeded', { code: errors.CODES.RATE_LIMIT_EXCEEDED });
    }

    const quota = escrow.checkQuota(opKey, 1);
    if (!quota.allowed) {
        throw new errors.VantError('Escrow quota exceeded for compute', { code: errors.CODES.ESCROW_DENIED });
    }

    const compute = getCompute();
    if (!compute || !compute.evaluate) {
        throw new errors.VantError('Compute module not available', { code: errors.CODES.MODULE_NOT_FOUND });
    }
...
/**
 * Embed text with security wrapping (VAF + QoS + Escrow)
 */
async function embedText(text, options = {}) {
    const opKey = 'embed:text';
    
    if (!_embedRateLimit.check(opKey)) {
        throw new errors.VantError('Embed rate limit exceeded', { code: errors.CODES.RATE_LIMIT_EXCEEDED });
    }

    const quota = escrow.checkQuota(opKey, 1);
    if (!quota.allowed) {
        throw new errors.VantError('Escrow quota exceeded for embed', { code: errors.CODES.ESCROW_DENIED });
    }

    const embed = getEmbed();
    if (!embed || !embed.generate) {
        throw new errors.VantError('Embed module not available', { code: errors.CODES.MODULE_NOT_FOUND });
    }
...
async function init(options = {}) {
    ...
    try {
        ...
        // Priority: private brain boundaries > public brain defaults
        try {
            const brain = getBrain();
            if (brain) {
                // Load workspaces from public brain (defaults)
                ...
                // Load boundaries from PUBLIC brain (default policies)
                const publicBoundariesData = await brain.read('rls/boundaries', { type: 'public' });
                if (publicBoundariesData?.data) {
                    const publicBoundaries = typeof publicBoundariesData.data === 'string'
                        ? JSON.parse(publicBoundariesData.data)
                        : publicBoundariesData.data;
                    if (publicBoundaries && typeof publicBoundaries === 'object') {
                        Object.assign(habitat.boundaries, publicBoundaries);
                    }
                }

                // Load boundaries from CURRENT PRIVATE brain (brain-specific policies override)
                // Now uses brain.read() with brain option (fixed to support specific brain paths)
                const currentBrain = brain.currentBrain ? brain.currentBrain() : 'vant';
                if (debug) console.log('[vant] Loading private boundaries for brain:', currentBrain);

                const privateBoundariesData = await brain.read('rls/boundaries', {
                    type: 'private',
                    brain: currentBrain
                });
                if (debug) console.log('[vant] Private boundaries found:', !!privateBoundariesData?.data);

                if (privateBoundariesData?.data) {
                    const privateBoundaries = typeof privateBoundariesData.data === 'string'
                        ? JSON.parse(privateBoundariesData.data)
                        : privateBoundariesData.data;
                    if (debug) console.log('[vant] Private boundaries:', JSON.stringify(privateBoundaries));
                    if (privateBoundaries && typeof privateBoundaries === 'object') {
                        // Private brain boundaries override public defaults
                        Object.assign(habitat.boundaries, privateBoundaries);
                    }
                }

                if (debug) console.log('[vant] RLS configs loaded from brain (public + private)');
            }
        } catch (e) {
...
async function startHeadless(options = {}) {
    const {
        ...
        debug = false,
        taskId = null,
        contextHeartbeat = true  // NEW: Auto-start context heartbeat
    } = options;
    ...
    // Step 2: Pre-warm context cache (v0.8.6)
    if (contextHeartbeat) {
        try {
            const context = require('./context');
            await context.build({ includeTools: true, includeHistory: true, includeDynamic: false });
            if (debug) console.log('[vant] Context pre-warmed');
        } catch (e) {
            if (debug) console.log('[vant] Context pre-warm:', e.message);
        }
    }

    // Step 3: Start HTTP server (no MCP)
    try {
    ...
    // Step 4: Event system ready
    try {
        const event = require('./event');
        event.getStatus?.();
    } catch (e) {}

    // Step 5: Start context heartbeat (v0.8.6)
    if (contextHeartbeat) {
        try {
            const cron = require('./cron');
            cron.startContextHeartbeat({ interval: 4 * 60 * 1000 });
            if (debug) console.log('[vant] Context heartbeat started');
        } catch (e) {
            if (debug) console.log('[vant] Heartbeat error:', e.message);
        }
    }
...
module.exports = {
    ...
    // v0.9.0-axolotl T14b: removed `Runtime` legacy alias class
    // (a thin wrapper exposing init/think/getStatus). Use the
    // top-level init/think/getStatus functions directly.

    getLayerStatus: () => ({ name: 'Runtime', type: 'runtime', version: require('./version'), enabled: true }),
...
// Compute & Embed (from framework, v0.9.6)
module.exports.computeEval = computeEval;
