...
async function startFull(options = {}) {
    ...
    // Start MCP if mode includes 'mcp' or 'all'
    ...
    // Start API if mode includes 'api' or 'all'
 ...
 * 
 * Backwards-compatible alias for startFull({ mode: 'all' })
 *
 * @param {object} options - Configuration
 * @param {number} options.mcpPort - MCP port (default: 3457)
...
async function startTrifecta(options = {}) {
    // Delegate to startFull with mode: 'all'
    return startFull({ ...options, mode: 'all' });
}
