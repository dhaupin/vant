    ...
    try {
        ...
        try {
            ...
            if (brain) {
                ...
                // Use direct filesystem access since brain.read() doesn't support specific brain paths
                const currentBrain = brain.currentBrain ? brain.currentBrain() : 'vant';
                const fs = require('fs');
                const path = require('path');
                const privateBoundariesPath = path.join(
                    process.cwd(), 
                    'models', 
                    'private', 
                    currentBrain, 
                ...
                try {
                    if (fs.existsSync(privateBoundariesPath)) {
                        const privateBoundaries = JSON.parse(fs.readFileSync(privateBoundariesPath, 'utf8'));
                        if (privateBoundaries && typeof privateBoundaries === 'object') {
                            Object.assign(habitat.boundaries, privateBoundaries);
                            if (debug) console.log('[vant] Loaded private boundaries from:', privateBoundariesPath);
                        }
                    }
                } catch (e) {
                    if (debug) console.log('[vant] Private boundaries load skipped:', e.message);
                }
