...
async function learn(key, content) {
    ...
    getMemoize().set('learn:' + key, content, { ttl: 86400000 });
...
async function remember(key, content) {
    if (content) {
        // Store - use ttl: 0 for never expire (or very long TTL)
        getMemoize().set('memory:' + key, content, { ttl: 31536000000 }); // 1 year = never in practice
