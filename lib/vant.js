...
async function shutdown(options = {}) {
    ...
    try {
        // TRIGGER DREAM: Consolidate learnings before shutdown
        try {
            const brain = require('./brain');
            // End evolution session (saves recentInsights to state)
            if (brain.endEvolutionSession) {
                await brain.endEvolutionSession();
                if (debug) console.log('[vant] Evolution session ended');
            }
            // Trigger dream consolidation
            if (brain.dream) {
                brain.dream(true, 3);  // Enable dreaming at 3am
                if (debug) console.log('[vant] Dream consolidation enabled');
            }
        } catch (e) {
            if (debug) console.log('[vant] Dream trigger skip:', e.message);
        }
...
async function dream(options = {}) {
    ...
    // Trigger brain dream consolidation
    try {
        const brain = require('./brain');
        if (brain.dream) {
            brain.dream(true, hour);  // Enable dreaming at specified hour
            if (debug) console.log('[vant] Brain dream enabled:', { hour });
        }
    } catch (e) {
