...
async function learn(key, content, options = {}) {
    ...
    // Determine TTL: per-memory > default (24 hours for learnings)
    const learnTTL = ttl || (expiresAt ? expiresAt.getTime() - Date.now() : 86400000);
...
async function remember(key, content, options = {}) {
    ...
    // Determine TTL: per-memory > default
    const memoryTTL = ttl || (expiresAt ? expiresAt.getTime() - Date.now() : 31536000000); // 1 year default
