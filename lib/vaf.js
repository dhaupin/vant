...
const BLOCKED_FILE = '.vaf-blocked-circuit.json';
...
function _loadBlockedIPs() {
    if (fs.existsSync(BLOCKED_FILE)) {
        try {
            ...
            console.log(`[VAF] Loaded ${valid.size} blocked IPs from ${BLOCKED_FILE}`);
            return valid;
        } catch (e) {
            console.log(`[VAF] Could not load blocked IPs: ${e.message}`);
        }
    }
    return new Map();
}
...
function _saveBlockedIPs() {
    try {
        ...
        console.log(`[VAF] Could not save blocked IPs: ${e.message}`);
    }
}
...
function hashIP(ip) {
    return Encrypt.sha256(ip).substring(0, 8);
}
