...
const CONFIG = {
    ...
    MAX_BURST: 10,  // Max requests in burst window
    ...
    DANGEROUS_PATTERNS: [
        // Word stacking/recursive pattern: vant vant vant, foo foo foo - common confusion attack
        // Blocks any word repeated exactly 3 times (vant vant vant)
        // For broader recursive detection, use checkWordStacking() directly
        /\b(\w+)\s+\1\s+\1\b/i,
        
        // Command-specific stacking: docker docker, node node, vant vant
        // These commands stacked are confusing/conflicting attack vectors
        /\b(docker|node|vant|mcp|run|load|sync|start)\s+\1\b/i,
...
function checkPathTraversal(inputPath) {
    ...
    if (normalized.includes('..') || normalized.startsWith('/etc/') || 
        normalized.startsWith('/usr/') || normalized.startsWith('/bin/') ||
        normalized.match(/^[a-z]:\\/i)) {  // Windows absolute
 ...
 * @param {number} minRepeats - Minimum times word must repeat (default: 3)
 * @returns {blocked: boolean, matches: string[]}
 */
function checkWordStacking(content, minRepeats = 3) {
    if (!content || typeof content !== 'string') {
    ...
    // Pattern: word repeated N times
    // Build dynamic pattern based on minRepeats
    if (minRepeats === 3) {
        // Most common case - threepeat
        const pattern = /\b(\w+)(?:\s+\1){2,}\b/gi;
        let match;
        while ((match = pattern.exec(content)) !== null) {
            matches.push(match[1]);
        }
    } else {
        // Dynamic pattern for N repeats
