...
// Control characters regex pattern - build from char codes to avoid ESLint no-control-regex
const CONTROL_CHARS = new RegExp('[' + String.fromCharCode(0, 8, 11, 12, 14, 31, 127) + ']', 'g');
...
function _checkRead() {
    ...
    if (sandbox && sandbox.canRead) {
        try {
            if (!sandbox.canRead()) {
                throw new errors.VantError('Read permission required', { code: errors.CODES.READ_PERMISSION_REQUIRED || errors.CODES.VAF_VALIDATION_FAILED, retryable: false });
            }
        } catch (e) {}
    }
}
...
function _checkWrite() {
    ...
    if (sandbox && sandbox.canWrite) {
        try {
            if (!sandbox.canWrite()) {
                throw new errors.VantError('Write permission required', { code: errors.CODES.WRITE_PERMISSION_REQUIRED || errors.CODES.VAF_VALIDATION_FAILED, retryable: false });
            }
        } catch (e) {}
    }
}
...
// STATE (must precede _loadBlockedIPs call below)
let requestLog = [];  // { timestamp, ip, agent, endpoint }
let failedAttempts = new Map();  // ip -> count

// Lazy storage — blocked-IP persistence rides the storage/security chain
// (vaf path-check + capability gate + containment). Repo-root scoped.
// Note: vaf is loaded BY storage for path checks, so we must not hold a
// top-level circular require — lazy init only.
let _ipStore = null;
function _getStore() {
    if (!_ipStore) {
        // (pass 24) anchor: repo-root scoped store honors VANT_REPO_ROOT.
        _ipStore = new (require('./storage').FileStorage)({ basePath: require('./anchor').getRepoRoot() });
    }
    return _ipStore;
}
...
function _loadBlockedIPs() {
    try {
        const raw = _getStore().read(BLOCKED_FILE);
        if (raw !== null) {
            ...
            audit('IP_BLOCKLIST_LOADED', { file: BLOCKED_FILE, count: valid.size });
            return valid;
        }
    } catch (e) {
        audit('IP_BLOCKLIST_LOAD_FAILED', { file: BLOCKED_FILE, error: e.message });
    }
    return new Map();
}
...
function _saveBlockedIPs() {
    try {
        ...
        _getStore().write(BLOCKED_FILE, JSON.stringify(data, null, 2));
    } catch (e) {
        audit('IP_BLOCKLIST_SAVE_FAILED', { file: BLOCKED_FILE, error: e.message });
    }
}
...
function audit(action, details) {
    ...
    try {
        const relPath = CONFIG.AUDIT_FILE;
        const line = JSON.stringify(entry) + '\n';
        // Append rides the storage chain too (vaf path-check + gate + containment).
        // FileStorage has no append — read-modify-write keeps the atomic contract.
        const existing = _getStore().read(relPath) || '';
        _getStore().write(relPath, existing + line);
    } catch (e) {
...
function validateString(value, options = {}) {
    ...
    if (value === undefined || value === null) {
        ...
        throw new errors.VantError(name + ' is required', { code: errors.CODES.VAF_REQUIRED_FIELD, retryable: false });
    }

    if (typeof value !== 'string') {
        if (options.tryCatch) return { valid: false, reason: name + ' must be a string' };
        throw new errors.VantError(name + ' must be a string', { code: errors.CODES.VAF_TYPE_INVALID, retryable: false });
    }

    const len = value.length;
    if (len < minLength) {
        if (options.tryCatch) return { valid: false, reason: name + ' too short' };
        throw new errors.VantError(name + ' too short: min ' + minLength, { code: errors.CODES.VAF_TOO_SHORT, retryable: false });
    }
    if (len > maxLength) {
        if (options.tryCatch) return { valid: false, reason: name + ' too long' };
        throw new errors.VantError(name + ' too long: max ' + maxLength, { code: errors.CODES.VAF_TOO_LONG, retryable: false });
    }

    if (pattern && !pattern.test(value)) {
        if (options.tryCatch) return { valid: false, reason: name + ' format invalid' };
        throw new errors.VantError(name + ' format invalid', { code: errors.CODES.VAF_TYPE_INVALID, retryable: false });
    }
...
function validateObject(obj, depth = 0) {
    if (depth > CONFIG.MAX_DEPTH) {
        throw new errors.VantError('Object depth too large: max ' + CONFIG.MAX_DEPTH, { code: errors.CODES.VAF_OBJECT_TOO_DEEP, retryable: false });
    }
    ...
    if (Array.isArray(obj)) {
        if (obj.length > CONFIG.MAX_ARRAY_LENGTH) {
            throw new errors.VantError('Array too large: max ' + CONFIG.MAX_ARRAY_LENGTH, { code: errors.CODES.VAF_ARRAY_TOO_LARGE, retryable: false });
        }
...
function checkPathTraversal(inputPath) {
    ...
    // P1-15: iterative URL-decode + Unicode normalization. Encoded traversal
    // (e.g. '%2e%2e%2f', '..%2f', double-encoded '..%252f', overlong UTF-8)
    // must not slip past the literal checks below. Decode until stable, then
    // run every check against BOTH the raw and decoded variants.
    ...
    for (let i = 0; i < 5; i++) {
        ...
        try {
            ...
            // Malformed percent-encoding (e.g. overlong UTF-8 '%c0%ae').
            // We cannot know how a downstream consumer would decode it -
            // fail closed.
            return { blocked: true, reason: 'Malformed percent-encoding in path' };
        }
        try {
            next = next.normalize('NFKC');
        } catch (e) { /* not a string - keep */ }
        if (next === decoded) break;
        decoded = next;
    }

    const _checkVariant = (p) => {
        const normalized = path.normalize(p);

        // Sensitive system paths (Linux) - use startsWith only (not includes)
        // to avoid blocking paths like models/public/vant/boot/
        ...
        // Also block absolute /lib/ and /lib64/ system paths
        ...
        return null; // this variant is clean
    };

    for (const variant of [inputPath, decoded]) {
        const blocked = _checkVariant(variant);
...
function validateSafePath(inputPath, allowedDir) {
    ...
    if (check.blocked) {
        throw new errors.VantError('Validation failed', { code: errors.CODES.VAF_VALIDATION_FAILED });
    }
    ...
    try {
        ...
        if (!resolved.startsWith(allowed)) {
            throw new errors.VantError('Path outside allowed directory', { code: errors.CODES.PATH_OUTSIDE_ALLOWED_DIRECTORY || errors.CODES.VAF_VALIDATION_FAILED, retryable: false });
        }
    } catch (e) {
        throw new errors.VantError('Invalid path', { code: errors.CODES.INVALID_PATH || errors.CODES.VAF_VALIDATION_FAILED, retryable: false });
    }
...
function check(input, options = {}) {
    // FLEXIBLE CTX: Handle both direct input and ctx object
    // ctx object can have: input, name, operation, or be the input itself
    let actualInput = input;
    let actualOptions = options;

    if (input && typeof input === 'object') {
        // It's a ctx object - extract input
        const ctx = input;
        actualInput = ctx.input || ctx.text || ctx.query || ctx.name || '';
        actualOptions = {
            ...options,
            name: options.name || ctx.operation || 'ctx.input',
    ...
    switch (type) {
        case 'string':
            ...
            if (!allowContent) {
                const contentCheck = checkContent(actualInput);
                if (contentCheck.blocked) {
                    ...
                    audit('BLOCKED_CONTENT', {
                        ...
                        snippet: actualInput.substring(0, 50)
                    });
                    throw new errors.VantError('Content blocked: ' + contentCheck.pattern, { code: errors.CODES.VAF_CONTENT_BLOCKED, retryable: false });
                }
            }
            break;

        case 'object':
            if (required && !actualInput) {
                throw new errors.VantError(name + ' is required', { code: errors.CODES.VAF_REQUIRED_FIELD, retryable: false });
            }
            if (actualInput) {
                validateObject(actualInput);
            }
            break;
        ...
        case 'file':
            if (actualInput) {
                const extCheck = checkFileExtension(actualInput);
                if (extCheck.blocked) {
                    audit('BLOCKED_FILE', {file: actualInput, ext: extCheck.extension});
                    throw new errors.VantError('File type not allowed: ' + extCheck.extension, { code: errors.CODES.VAF_FILE_TYPE, retryable: false });
                }
        ...
        default:
            throw new errors.VantError('Unknown type: ' + type, { code: errors.CODES.VAF_TYPE_INVALID, retryable: false });
    }
...
class Sanitize {
    ...
    static get CONTROL_PATTERN() {
        return vaf.CONTROL_CHARS;
    }

    removeControlChars(str) {
        return str.replace(vaf.CONTROL_CHARS, '');
    }
