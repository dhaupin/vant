...
function _checkWrite() {
    ...
    if (sandbox && sandbox.canWrite) {
        try {
            if (!sandbox.canWrite()) {
                throw new Error('Write permission required for prune operations');
            }
        } catch (e) {}
    }
}
...
function _checkRead() {
    ...
    if (sandbox && sandbox.canRead) {
        try {
            if (!sandbox.canRead()) {
                throw new Error('Read permission required for prune operations');
            }
        } catch (e) {}
    }
}
...
async function prune(options = {}) {
    ...
    console.log(`[Prune] Version: ${version}`);
    console.log(`[Prune] Dry run: ${dryRun}`);
    console.log(`[Prune] Stale days: ${staleDays}`);
    console.log(`[Prune] Remove fluff: ${removeFluff}`);
    ...
    for (const cat of categories) {
        ...
        for (const file of files) {
            ...
            if (age > staleDays) {
                if (dryRun) {
                    console.log(`[Prune] Would remove stale: ${cat}/${file} (${age} days)`);
                } else {
                    fs.unlinkSync(filePath);
                    console.log(`[Prune] Removed stale: ${cat}/${file}`);
                }
                stats.staleRemoved++;
                continue;
            }
            ...
            if (removeFluff && isFluff(content)) {
                if (dryRun) {
                    console.log(`[Prune] Would remove fluff: ${cat}/${file}`);
                } else {
                    fs.unlinkSync(filePath);
                    console.log(`[Prune] Removed fluff: ${cat}/${file}`);
                }
                stats.fluffRemoved++;
                continue;
            }
    ...
    if (!dryRun) {
        ...
        console.log(`[Prune] Saved LTC to: ${ltcPath}`);
    }
