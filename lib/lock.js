...
async function acquire(agentId = null, timeout = DEFAULT_TIMEOUT_MS, options = {}) {
    ...
    for (let attempt = 0; attempt < LOCK_CONFIG.MAX_ATTEMPTS; attempt++) {
        ...
        if (existing && existing.token === token) {
            ...
            audit.info(`[Lock] Refreshed lock for ${currentAgent}`);
            // Return cached token
        ...
        if (existing && isLockValid(existing)) {
            audit.info(`[Lock] Lock held by ${existing.agentId}, attempt ${attempt + 1}/${LOCK_CONFIG.MAX_ATTEMPTS}`);
        ...
        // Write atomically: storage.writeRaw uses temp+rename, which both
        // replaces atomically and neutralizes any symlink planted at lockPath
        const content = JSON.stringify(lockData, null, 2) + '\n---\n' + token;

        try {
            lockStore.writeRaw(lockPath, content);
            ...
            if (vData.agentId === currentAgent && vParts[1] === token) {
                audit.info(`[Lock] Acquired by ${currentAgent}`);
    ...
    audit.info(`[Lock] Could not acquire after ${LOCK_CONFIG.MAX_ATTEMPTS} attempts`);
    return null;
}
...
async function release(agentId = null, token = null, options = {}) {
    ...
    if (!lockStore.has(lockPath)) {
        audit.info(`[Lock] No lock to release`);
    ...
    try {
        ...
        if (existing.agentId === currentAgent) {
            ...
            if (!providedToken || (providedToken !== fileToken && providedToken !== cachedToken)) {
                audit.info(`[Lock] Token mismatch - release denied for ${currentAgent}`);
                return { success: false, message: 'Invalid token - release denied' };
            }

            lockStore.delete(lockPath);
            _clearToken(currentAgent, resolvedBrain);
            audit.info(`[Lock] Released by ${currentAgent}`);
            ...
            audit.info(`[Lock] Cannot release - owned by ${existing.agentId}`);
            return { success: false, message: `Lock owned by ${existing.agentId}` };
        }
    } catch (e) {
        audit.info(`[Lock] Error releasing: ${e.message}`);
        _clearToken(currentAgent, resolvedBrain);
...
function forceRelease(options = {}) {
    ...
    if (lockStore.has(lockPath)) {
        lockStore.delete(lockPath);
        audit.info(`[Lock] Force released`);
    }
}
