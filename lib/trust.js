...
// Safe-by-default capability gate (mirrors storage.js _checkWriteSafe).
// The default sandbox stub denies everything; an unconfigured module must not
// brick core flows, so allow with a one-time warning when the sandbox is the
// untouched default stub, and enforce strictly once explicitly configured.
let _defaultTrustCan = null;
let _warnedDefaultTrust = false;
function _checkCapabilitySafe(capability) {
    const sandbox = _getSandbox();
    if (!sandbox || !sandbox.can) {
        return { allowed: true };
    }
    if (_defaultTrustCan === null) {
        _defaultTrustCan = sandbox.can;
    }
    if (sandbox.can === _defaultTrustCan) {
        // Unconfigured default stub - allow with one-time warning
        if (!_warnedDefaultTrust) {
            _warnedDefaultTrust = true;
            // eslint-disable-next-line no-console
            console.warn('[trust] Sandbox not configured; trust allows by default. ' +
                'Configure sandbox capabilities to enforce.');
        }
        return { allowed: true };
    }
    // Real sandbox config - enforce
