...
// (pass 40 / agora) Quarantine + verification — folded from spirit.js
// (removed). These are trust-ledger concerns and they PERSIST now: a
// quarantined bad actor must stay quarantined across restarts.
const _quarantined = new Map();      // agentId -> { reason, quarantinedAt }
const _verifications = new Map();    // agentId -> [{ verified, ts, ... }]
...
function _applyState(data) {
    ...
    if (data.quarantined) for (const [k, v] of Object.entries(data.quarantined)) _quarantined.set(k, v);
    if (data.verifications) for (const [k, v] of Object.entries(data.verifications)) {
        _verifications.set(k, Array.isArray(v) ? v.slice(-50) : []);
    }
}
...
function _persist() {
    stateStore.persist({
        ...
        data: {
            ...
            quarantined: Object.fromEntries(_quarantined),
            verifications: Object.fromEntries(_verifications)
        }
    });
}
...
class Trust extends EventEmitter {
     ...
     */
    can(entityId, action) {
        // (pass 40 / agora) Quarantine has teeth: a quarantined agent is
        // denied everything, regardless of score.
        if (_quarantined.has(entityId)) return false;
    ...
    // ==================== QUARANTINE + VERIFICATION (pass 40 / agora) ====================
    // Folded from spirit.js (removed). Governance still gates the decision;
    // trust is the durable record. A quarantined agent is denied by can()
    // below — quarantine has teeth, not just a list.

    async quarantine(agentId, reason = 'unspecified') {
        // Governance check (same posture spirit had)
        let gov = null;
        try { gov = require('./governance'); } catch (e) { gov = null; }
        if (gov && gov.isAllowed) {
            const approved = await gov.isAllowed('quarantine', {
        ...
        _quarantined.set(agentId, { reason, quarantinedAt: Date.now() });
        // Quarantine is also a trust event: score it like a violation.
        this.record(agentId, 'violation', { positive: false, note: 'quarantined: ' + reason });
        _persist();

        _emit('trust:quarantined', { agent: agentId, reason });
        return { quarantined: true, agent: agentId, reason };
    }

    release(agentId) {
        const removed = _quarantined.delete(agentId);
        if (removed) _persist();
        _emit('trust:released', { agent: agentId });
        return { released: removed, agent: agentId };
    }

    isQuarantined(agentId) {
        _hydrate();
        return _quarantined.has(agentId);
    }

    getQuarantined() {
        _hydrate();
        return Array.from(_quarantined.entries()).map(([agentId, q]) => ({ agentId, ...q }));
    }

    async verifyAgent(agentId) {
        const score = this.getScore(agentId);
        const quarantined = this.isQuarantined(agentId);
        const verified = !quarantined && score >= (_roleTrust.get('default') || 0.3);
        const record = { verified, score, quarantined, ts: Date.now() };
        const history = _verifications.get(agentId) || [];
        history.push(record);
        _verifications.set(agentId, history.slice(-50));
        _persist();
        _emit('trust:verified', { agent: agentId, verified });
        return record;
    }

    getVerifications(agentId) {
        _hydrate();
        return _verifications.get(agentId) || [];
    }
...
module.exports = {
    ...
    clearState: () => { stateStore.clear(TRUST_STATE_FILE); _trustScores.clear(); _histories.clear(); _karma.clear(); _roleTrust.clear(); _quarantined.clear(); _verifications.clear(); _trustHydrated = true; return true; },
    ...
    // (pass 40 / agora) Quarantine + verification — folded from spirit.js
    // (removed): these are trust-ledger concerns, and now they PERSIST via
    // the trust state file instead of dying with the process.
    quarantine: (...args) => getTrust().quarantine(...args),
    release: (...args) => getTrust().release(...args),
    isQuarantined: (...args) => getTrust().isQuarantined(...args),
    getQuarantined: (...args) => getTrust().getQuarantined(...args),
    verifyAgent: (...args) => getTrust().verifyAgent(...args),
    getVerifications: (...args) => getTrust().getVerifications(...args),
