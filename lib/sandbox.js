 ...
 * Vant Sandbox Layer
 * 
 * Execution isolation layer for Vant - runs at same global scope as VAF
 ...
 *   - allowedDomains: Network domain whitelist
...
/**
 * Sandbox Class
 * Dedicated execution isolation - Layer 2 of 4 (VAF → Sandbox → QoS → Security)
 */
class Sandbox {
    /**
    ...
    constructor(options = {}) {
        ...
        // Quotas
        this.readQuota = options.readQuota || { perMinute: 100 };
        this.writeQuota = options.writeQuota || { perMinute: 20 };
        
        // Network
        this.allowedDomains = options.allowedDomains || [];
        this.blockExternal = options.blockExternal !== false;
    ...
    /**
     * Get current operation type limits
    ...
    _getLimits(type) {
        if (type === 'read') {
            return {
                ...
                quota: this.readQuota.perMinute,
                timeout: this.timeout
            };
        }
        return {
            maxConcurrent: 1,  // Serialized writes
            quota: this.writeQuota.perMinute,
            timeout: this.timeout
        };
    }
    ...
    async _canExecute(type) {
        const limits = this._getLimits(type);
        
        // Check concurrency
        if (this._activeOps >= limits.maxConcurrent) {
            return {allowed: false, reason: 'concurrency_limit', limit: limits.maxConcurrent};
        }
        
        // Check quota
        const now = Date.now();
        const lastMinute = this._ops.filter(op => 
            op.type === type && now - op.timestamp < 60000
        ).length;
        
        if (lastMinute >= limits.quota) {
            return {allowed: false, reason: 'quota_limit', limit: limits.quota};
        }
    ...
    async execute(operation, context = {}) {
        ...
        try {
            ...
            const timeout = this._getLimits(type).timeout;
