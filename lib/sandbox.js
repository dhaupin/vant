...
/**
 * Initialize sandbox with RLS
 * Call at boot with habitat instance
 * Uses sandbox as carrier for RLS (called everywhere)
 */
function initRLS(habitat) {
    _habitat = habitat;
    // Lazy load rls to avoid circular deps
    _rls = require('./rls');
    _rls.init(habitat);

    // Add RLS reference to default sandbox
    if (defaultSandbox) {
        defaultSandbox.rls = _rls;
        defaultSandbox._habitat = _habitat;
    }
...
/**
 * Generate capabilities from RLS context
 * Key: sandbox carries RLS, generates caps from workspace/role
 */
function generateCaps(userCtx, baseCaps = {}) {
    if (!_rls || !_habitat) {
        return baseCaps;
    }

    const workspace = userCtx.workspace || _habitat.getCurrentWorkspace();
    const roles = userCtx.roles || _habitat.getUserRoles(workspace, userCtx.userId);
...
// Legal Integration - module-level reference
let _legalRef = null;

/**
 * Initialize legal module (compliance layer)
 * Call to activate legal gate (default: dormant)
 * @param {string} level - 'warn' | 'block'
 */
function initLegal(level = 'warn') {
    _legalRef = require('./legal');
    _legalRef.activate(level);

    // Add to default sandbox
    if (defaultSandbox) {
        defaultSandbox._legal = _legalRef;
    }
...
// SECURITY: DENY BY DEFAULT - all capability functions return false
// These are the FIRST exports; final exports at bottom override with defaultSandbox
// If sandbox module loads but defaultSandbox isn't initialized, these deny-by-default stubs are used

module.exports = {
    initRLS,
    initLegal,    // Legal compliance gate
    generateCaps,  // RLS cap generation via sandbox (carrier pattern)
    canRead: () => false,   // DENY by default
    canWrite: () => false,  // DENY by default
    canNetwork: () => false,
    canExec: () => false,   // shell exec - DENY
    canSpawn: () => false,  // spawn agents - DENY
    canRecurse: () => true,  // ALLOW by default - recursion is safe with guard
    ...
    create: (caps = {}) => ({
        ...
        canRecurse: () => caps.canRecurse ?? true  // Default allow
    })
};
...
// Capability (sandbox) → scope (sudo) mapping (prd-sudo.md capability table)
const CAP_TO_SCOPE = {
    canRead: 'read',
    canWrite: 'write',
    canNetwork: 'network',
...
// Lazy sudo accessor - sudo.js requires nothing from sandbox, safe to lazy-load
let _sudo = null;
function _getSudoSafe() {
    if (!_sudo) {
        try { _sudo = require('./sudo'); } catch (e) { return null; }
    }
    return _sudo;
}
...
function _getLegal() {
    // Uses _legalRef from initLegal() closure
    return _legalRef;
}
...
// SECURITY: DENY BY DEFAULT - all dangerous capabilities explicitly set to false
// These are used by defaultSandbox (created at line 309) and sandbox.create() fallbacks
// Agents must explicitly opt-in by passing capabilities when creating a sandbox
// Early exports (lines 124-129) also return false for consistency

const DEFAULT_CAPABILITIES = {
    // Brain operations
    read: true,         // brain.get(), search()
    write: false,       // brain.write(), commit() - DENY by default
    load: true,         // brain.load()
    list: true,        // brain.list()
    exists: true,       // brain.exists()
    search: true,       // search operations
    // File operations
    canRead: true,      // sandbox read files
    canWrite: false,    // write files - DENY by default
    canNetwork: false,  // fetch() to external APIs - DENY by default
    canExec: false,     // shell exec - DENY by default
    canSpawn: false,    // create sub-agents - DENY by default
    canCommit: false,   // commit to git - DENY by default
    canCreateBranch: false, // create git branches
    canDelete: false,   // delete files/branches
    canAdmin: false,    // admin operations
    // (pass 38) Market trade capability: market.trade() asks the gate for
    // 'canTrade' but the Sandbox model never declared it, so an explicitly
    // configured sandbox always denied (undefined !== true) and no caller
    // could grant it. Declared DENY-by-default like its siblings; host
    // environments opt in via setCapabilities({ canTrade: true }).
    canTrade: false,    // knowledge market trades - DENY by default
    // Brain files
...
class Sandbox {
    ...
    constructor(options = {}) {
        ...
        this._readQuota = options.readQuota || 100;
        this._writeQuota = options.writeQuota || 20;
        this._failureThreshold = options.failureThreshold || 5;
        this._successThreshold = options.successThreshold || 3;
        this._circuitTimeout = options.circuitTimeout || 30000;

        // Lazy-loaded limiters (avoid circular dep at module load time)
        this._readLimiter = null;
        this._writeLimiter = null;
        ...
        // Legal - compliance layer (optional, dormant by default)
        this._legal = _getLegal();

        // Capabilities - what can this agent do?
        // DENY BY DEFAULT: DEFAULT_CAPABILITIES has dangerous ops = false
        // Agents must explicitly opt-in via options.capabilities
        this.capabilities = {...DEFAULT_CAPABILITIES, ...options.capabilities};

        // Scopes - which operation types are allowed?
        this.scopes = options.scopes || [...DEFAULT_SCOPES];

        // Tracks whether this sandbox was explicitly configured (constructor
        // options or later setScopes/setCapabilities). Lets middleware treat
        // "fresh default sandbox" (allow with warning) differently from
        // "explicitly locked down" (enforce denial).
        this._explicitlyConfigured = !!(options.capabilities || options.scopes);
    ...
    async _canExecute(type) {
        ...
        // Lazy init limiters on first use (avoid circular dep at module load)
        if (!this._readLimiter) {
            const QoS = _getQoS();
            this._readLimiter = new QoS.RateLimiter({ windowMs: 60000, maxPerMinute: this._readQuota });
            this._writeLimiter = new QoS.RateLimiter({ windowMs: 60000, maxPerMinute: this._writeQuota });
        }
    ...
    async execute(operation, context = {}) {
        ...
        // 1. VAF - Input validation/sanitization (FIRST)
        if (context.input) {
            vaf.sanitize(context.input);
        }

        // 2. RLS - Workspace/role capability check (via sandbox caps)
        const canExec = await this._canExecute(type);
        if (!canExec.allowed) {
            this._recordError(type, canExec.reason, 'blocked');
            const err = require('./error');
            throw new err.VantError(`Sandbox: ${canExec.reason} for ${type}`, { code: err.CODES.SANDBOX_EXEC_DENIED, retryable: false });
        }
        ...
        if (!budgetCheck.allowed) {
            ...
            const err = require('./error');
            throw new err.VantError(`Sandbox: budget exceeded for ${this.agentId}`, { code: err.CODES.SANDBOX_BUDGET_EXCEEDED, retryable: true });
        }

        // 4. Legal - Compliance check (if activated)
        if (this._legal && this._legal.checkGate) {
            const legalCheck = this._legal.checkGate('execute', { type, context });
            if (!legalCheck.allowed) {
                this._recordError(type, legalCheck.reason, 'legal_blocked');
                const err = require('./error');
                throw new err.VantError(`Sandbox: ${legalCheck.reason}`, { code: err.CODES.SANDBOX_EXEC_DENIED, retryable: false });
            }
        }
        ...
        // Lazy init CircuitBreaker on first use (avoid circular dep at module load)
        if (!this._circuitBreaker) {
            const QoS = _getQoS();
            this._circuitBreaker = new QoS.CircuitBreaker({
                failureThreshold: this._failureThreshold,
                successThreshold: this._successThreshold,
                timeout: this._circuitTimeout
            });
        }
        ...
        try {
            ...
            throw new errors.VantError(`Sandbox: circuit open - too many failures, try again later`, { code: errors.CODES.SANDBOX_CIRCUIT_OPEN, retryable: true });
        } finally {
            this._activeOps--;
    ...
    async write(operation) {
        ...
        if (this.requireLock) {
            ...
            if (!token) {
                throw new errors.VantError('Sandbox: Could not acquire lock for write', { code: errors.CODES.LOCK_FAILED, retryable: true });
            }
    ...
    setCapabilities(caps) {
        ...
        this._explicitlyConfigured = true;
    }
     ...
     *
     * Delegates to sudo when the task is known (prd-sudo.md: can(cap) →
     * sudo.can(taskId, scope)). Falls back to static capabilities when the
     * task has no sudo state, so standalone sandboxes keep working.
     */
    can(cap) {
        const scope = CAP_TO_SCOPE[cap] || null;
        if (scope && this.agentId) {
            const sudo = _getSudoSafe();
            if (sudo && sudo.getTask(this.agentId)) {
                // Scope-level sudo verdict wins when the task exists in sudo,
                // BUT never below the static capability floor: an explicit
                // capability grant on this sandbox must stay honored even if
                // the linked task lacks the scope (test harnesses grant caps
                // directly; vant.init later links a narrower boot task). The
                // effective answer is the UNION of task scope and capability,
                // preserving "widening only" semantics in both directions.
                if (sudo.can(this.agentId, scope)) return true;
            }
        }
     ...
     * @param {Object} options - { merge: true } widens the existing set
     *   instead of replacing it. Boot uses merge so initializing the runtime
     *   never narrows authority a host (test harness, embedded runtime) had
     *   already granted. Replace remains available for intentional lockdown.
     */
    setScopes(scopes, options = {}) {
        if (options.merge) {
...
// Add convenience methods to defaultSandbox (delegate to capabilities + scopes)
defaultSandbox.canNetwork = () => {
    return defaultSandbox.scopes.includes('network') &&
           defaultSandbox.capabilities.canNetwork !== false;
};
defaultSandbox.canRead = () => {
    return defaultSandbox.scopes.includes('read') &&
           defaultSandbox.capabilities.canRead !== false;
};
defaultSandbox.canWrite = () => {
    return defaultSandbox.scopes.includes('write') &&
           defaultSandbox.capabilities.canWrite !== false;
};
...
module.exports = {
    ...
    // Module-level capability checks — ROUTED THROUGH can(cap) so sudo
    // escalation (prd-sudo.md: time-bounded, audited grants) is honored by
    // every caller, not just call sites that use can() directly. Static
    // capability/scopes verdict remains the fallback when no sudo task
    // exists. (prd-security.md migration checklist item: unify canX()/can().)
    canRead: () => defaultSandbox.can('canRead'),
    canWrite: () => defaultSandbox.can('canWrite'),
    canNetwork: () => defaultSandbox.can('canNetwork'),
    canExec: () => defaultSandbox.can('canExec'),
    canSpawn: () => defaultSandbox.can('canSpawn'),
    ...
    // RLS accessor (convenience - same as defaultSandbox.rls)
    get rls() {
        return defaultSandbox.rls;
    },
    ...
    // Brain Handler Registry - wire brain.js to sandbox
    registerBrainHandler(operation, handler) {
        _brainHandlers[operation] = handler;
    },

    getBrainHandler(operation) {
        return _brainHandlers[operation];
    },
    ...
    // Multibrain
    getBrainSandboxConfig,
    setBrainSandboxConfig,

    // Multibrain Stack
    getStackSandboxConfigs
};
...
function getBrainSandboxConfig() {
    const brain = require('./brain');
    const brainName = brain.getCurrentBrain();
    return _brainConfigs[brainName] || defaultSandbox;
}

function setBrainSandboxConfig(config) {
    const brain = require('./brain');
    const brainName = brain.getCurrentBrain();
    _brainConfigs[brainName] = config;
    return true;
}

// ==================== MULTIBRAIN STACK SUPPORT ====================

function getStackSandboxConfigs() {
    const brain = require('./brain');
    ...
    for (const brainName of stack) {
        try {
            ...
            const config = getBrainSandboxConfig();
            results.byBrain[brainName] = { scopes: config.scopes, capabilities: config.capabilities };
        } catch (e) {
