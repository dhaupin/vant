...
function canBrain(operation) {
    ...
    return DEFAULT_CAPABILITIES[cap] !== false;
}
...
const DEFAULT_CAPABILITIES = {
    canRead: true,        // brain.get(), search()
    canWrite: true,     // brain.write(), commit()
...
class Sandbox {
    ...
    async execute(operation, context = {}) {
        ...
        if (!budgetCheck.allowed) {
            ...
            throw new Error(`Sandbox: budget exceeded for ${this.agentId}`);
        }
        ...
        try {
            ...
            throw new Error(`Sandbox: circuit open - too many failures, try again later`);
        } finally {
            this._activeOps--;
    ...
    getBudgetStatus() {
        ...
        const status = this._escrow.getStatus(this.agentId);
        return {
            agentId: this.agentId,
            limit: budget.limit,
            spent: budget.spent,
            available: budget.available,
            utilization: (budget.spent / budget.limit) * 100
        };
    }
...
// Merge all exports
module.exports = {
    ...module.exports,  // Keep early exports
