...
// v0.9.0 fs->storage migration: ledger + brain-file edits go through
// FileStorage (containment/symlink/VAF/atomic-write) instead of raw fs.
// Capability gating via shared gate (lib/gate.js).
function _checkRead() {
    gate.requireCapability('canRead', { scope: 'resolution', operation: 'read' });
}

function _checkWrite() {
    gate.requireCapability('canWrite', { scope: 'resolution', operation: 'write' });
}

// Use brain router for paths
const brain = require('./brain');
const Storage = require('./storage');
const fileStore = new Storage.FileStorage({ basePath: brain.getPublicPath() });
...
function getLedger() {
    _checkRead();
    const raw = fileStore.read('.resolution.json');
    if (raw === null) {
...
function updateEntryFrontmatter(filename, entryKey, resolution) {
    ...
    if (!fileStore.has(baseFilename)) return null;

    const content = fileStore.read(baseFilename);
    ...
    _checkWrite();
    fileStore.write(baseFilename, lines.join('\n'));
...
function addResolution(entry, status, reason, options = {}) {
    ...
    // Save ledger
    _checkWrite();
    fileStore.write('.resolution.json', JSON.stringify(ledger, null, 2));
