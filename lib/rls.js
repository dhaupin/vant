 ...
 * Integrates with existing sandbox module
 */
function createSandboxCaps(userCtx, baseCaps = {}) {
    // Start with base caps
    const caps = { ...baseCaps };
    
    // Override based on roles
    if (userCtx.roles.includes('admin')) {
        caps.canRead = true;
        caps.canWrite = true;
        caps.canNetwork = true;
        caps.canExec = true;
        caps.canSpawn = true;
    } else if (userCtx.roles.includes('editor')) {
        caps.canRead = true;
        caps.canWrite = true;
        caps.canNetwork = true;
    } else if (userCtx.roles.includes('viewer')) {
        caps.canRead = true;
        caps.canWrite = false;
        caps.canNetwork = false;
    }
    
    // Check workspace isolation
    if (_habitat && userCtx.workspace) {
        // Add workspace context to capabilities
        caps.workspace = userCtx.workspace;
    }
