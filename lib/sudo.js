...
// ==================== SERVICE HEALTH-CHECK REGISTRY (prd-sudo.md §12) ====================
// "Automated revalidation with service health checks": services may register
// an async health probe. The revalidation loop consults it when a
// revalidate:true grant expires — healthy extends, unhealthy revokes.
// No check registered for a service ⇒ treated as healthy (backward
// compatible with the plain-TTL revalidation behavior).
const HEALTH_CHECK_TIMEOUT_MS = _envInt(process.env.VANT_SUDO_HEALTH_TIMEOUT, 1000);

// service name -> { check, description }
let _healthChecks = new Map();

/**
 * Register a health check for a service (prd-sudo.md §12).
 * @param {string} service - Service name (e.g. 'network', 'storage')
 * @param {Function} check - Async () => boolean (or { ok }) — truthy means healthy
 * @param {Object} [options] - { description }
 */
function registerHealthCheck(service, check, options = {}) {
    try {
        if (typeof service !== 'string' || !service.trim()) return { registered: false, reason: 'invalid_service' };
        if (typeof check !== 'function') return { registered: false, reason: 'invalid_check' };
        _healthChecks.set(service, { check, description: String(options.description || '') });
        return { registered: true, service };
    } catch (e) {
...
function unregisterHealthCheck(service) {
    return { removed: _healthChecks.delete(service) };
}

/** List registered checks (inspection/testing). */
function getHealthChecks() {
    const out = {};
    for (const [service, hc] of _healthChecks) {
        out[service] = { description: hc.description, hasCheck: typeof hc.check === 'function' };
    }
    return out;
}

/**
 * Run one registered check with a hard timeout. Never throws.
 * @returns {Promise<{healthy: boolean, detail: string}>}
 */
async function _runCheck(hc) {
    let timer = null;
    try {
        const r = await Promise.race([
            Promise.resolve().then(hc.check).then(
                (v) => ({ healthy: !!v && !(typeof v === 'object' && v.ok === false), detail: '' }),
                (e) => ({ healthy: false, detail: e.message || 'check threw' })
            ),
            new Promise((resolve) => {
                timer = setTimeout(() => resolve({ healthy: false, detail: 'timeout' }), HEALTH_CHECK_TIMEOUT_MS);
            })
        ]);
        return r;
    } catch (e) {
        return { healthy: false, detail: e.message || 'check failed' };
    } finally {
...
/**
 * Run all registered health checks (CLI/inspection helper).
 * @returns {Promise<{service: string, healthy: boolean, detail: string}[]>}
 */
async function runHealthChecks() {
    const results = [];
    for (const [service, hc] of _healthChecks) {
        const r = await _runCheck(hc);
        results.push({ service, healthy: r.healthy, detail: r.detail });
    }
    return results;
}
...
// Env-tunable ints: garbage values fall back to the default instead of NaN
// (NaN would disable the audit cap, break rate math, and make timers fire
// immediately).
function _envInt(raw, dflt) {
    const n = parseInt(raw, 10);
    return Number.isFinite(n) && n > 0 ? n : dflt;
}
const _AUDIT_REL = 'models/private/sudo/escalations.jsonl';
const AUDIT_MAX_ENTRIES = _envInt(process.env.VANT_SUDO_AUDIT_MAX, 2000);

function _appendAuditLog(entry) {
    try {
        const Storage = require('./storage');
        if (typeof Storage.FileStorage !== 'function') return;
        // (pass 24) anchor: install root via VANT_REPO_ROOT (falls back to
        // this install tree) instead of __dirname guessing. _sudoStore is
...
function getEscalationAuditLog(limit = 100) {
    try {
        // (pass 24) shared store helper — was a duplicate store construction.
        const store = _sudoStore();
...
// default caps.
const RATE_WINDOW_MS = _envInt(process.env.VANT_SUDO_RATE_WINDOW_MS, 60000);
const RATE_MAX_PER_WINDOW = _envInt(process.env.VANT_SUDO_RATE_MAX, 20);
...
function _sudoStore() {
    const Storage = require('./storage');
    // (pass 24) anchor: the sudo store lives in the INSTALL tree's models/,
    // not the caller's cwd — resolve through lib/anchor so a host embedding
    // vant elsewhere (VANT_REPO_ROOT) still finds its own store.
    return new Storage.FileStorage({ basePath: require('./anchor').getRepoRoot() });
}
...
function defineTemplate(name, spec) {
    if (typeof name !== 'string' || !TEMPLATE_NAME_RE.test(name)) {
        throw new errors.VantError('Invalid template name', {
    ...
    if (!spec || typeof spec !== 'object' || Array.isArray(spec)) {
        throw new errors.VantError('Template spec must be an object', {
            code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
        });
    }
    const { service, scope, ttl, reason, description } = spec;
    if (typeof service !== 'string' || !service.trim()) {
        throw new errors.VantError('Template requires a service', {
            code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
        });
    }
    if (typeof scope !== 'string' || !ALL_SCOPES.has(scope)) {
        throw new errors.VantError(`Template scope must be one of: ${[...ALL_SCOPES].join(', ')}`, {
            code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
        });
    }
    if (ttl !== undefined && (typeof ttl !== 'number' || !Number.isFinite(ttl) || ttl <= 0)) {
        throw new errors.VantError('Template ttl must be a positive number (ms)', {
            code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
        });
    }
...
async function applyTemplate(name, taskId, options = {}) {
    ...
    if (!template) {
        throw new errors.VantError(`Template not found: ${name}`, {
...
function _resolvePoliciesPath() {
    ...
    if (rel.startsWith('..') || path.isAbsolute(rel)) {
        throw new errors.VantError('Policies path escapes repo root', {
...
function _readPoliciesFile() {
    ...
    if (!data || typeof data !== 'object' || Array.isArray(data)) {
        throw new errors.VantError('Policies file must be a JSON object of service entries', {
            code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
        });
    }
    return data;
...
function _planPolicies(file) {
    ...
    for (const [service, patch] of Object.entries(file)) {
        ...
        if (!current) {
            throw new errors.VantError(`Policies: unknown service '${service}' (adding services is not allowed)`, {
                code: errors.CODES.PERMISSION_DENIED, statusCode: 403
            });
        }
        if (!patch || typeof patch !== 'object' || Array.isArray(patch)) {
            throw new errors.VantError(`Policies: '${service}' entry must be an object`, {
                code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
            });
        }
        for (const key of Object.keys(patch)) {
            if (!POLICY_FIELDS.has(key)) {
                throw new errors.VantError(`Policies: unknown field '${key}' on '${service}' (typo?)`, {
                    code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                });
            }
        }
        ...
        if (patch.allowedScopes !== undefined) {
            if (!Array.isArray(patch.allowedScopes)) {
                throw new errors.VantError(`Policies: allowedScopes on '${service}' must be an array`, {
                    code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                });
            }
            for (const s of patch.allowedScopes) {
                if (!current.allowedScopes.includes(s)) {
                    throw new errors.VantError(`Policies: cannot add scope '${s}' to '${service}' (not currently allowed)`, {
                        code: errors.CODES.PERMISSION_DENIED, statusCode: 403
                    });
                }
            }
        ...
        if (patch.autoApprove !== undefined) {
            if (!Array.isArray(patch.autoApprove)) {
                throw new errors.VantError(`Policies: autoApprove on '${service}' must be an array`, {
                    code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                });
            }
            for (const s of patch.autoApprove) {
                if (!current.autoApprove.includes(s)) {
                    throw new errors.VantError(`Policies: cannot auto-approve '${s}' on '${service}' (not currently auto-approved)`, {
                        code: errors.CODES.PERMISSION_DENIED, statusCode: 403
                    });
                }
                if (!effAllowed.includes(s)) {
                    throw new errors.VantError(`Policies: autoApprove '${s}' on '${service}' is outside effective allowedScopes`, {
                        code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                    });
                }
            }
        ...
        if (patch.requiresCallback !== undefined) {
            if (!Array.isArray(patch.requiresCallback)) {
                throw new errors.VantError(`Policies: requiresCallback on '${service}' must be an array`, {
                    code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                });
            }
            for (const s of patch.requiresCallback) {
                if (!effAllowed.includes(s)) {
                    throw new errors.VantError(`Policies: requiresCallback '${s}' on '${service}' is outside effective allowedScopes`, {
                        code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                    });
                }
            }
            ...
            for (const s of mustKeep) {
                if (!patch.requiresCallback.includes(s)) {
                    throw new errors.VantError(`Policies: cannot remove callback requirement for '${s}' on '${service}'`, {
                        code: errors.CODES.PERMISSION_DENIED, statusCode: 403
                    });
                }
            }
        ...
        if (patch.maxTTL !== undefined) {
            if (typeof patch.maxTTL !== 'number' || !Number.isFinite(patch.maxTTL) || patch.maxTTL <= 0) {
                throw new errors.VantError(`Policies: maxTTL on '${service}' must be a positive number`, {
                    code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                });
            }
            if (patch.maxTTL > current.maxTTL) {
                throw new errors.VantError(`Policies: cannot raise maxTTL on '${service}' (${patch.maxTTL} > ${current.maxTTL})`, {
                    code: errors.CODES.PERMISSION_DENIED, statusCode: 403
                });
            }
        ...
        if (patch.revalidate !== undefined) {
            if (typeof patch.revalidate !== 'boolean') {
                throw new errors.VantError(`Policies: revalidate on '${service}' must be a boolean`, {
                    code: errors.CODES.INPUT_VALIDATION_FAILED, statusCode: 400
                });
            }
            if (current.revalidate === true && patch.revalidate === false) {
                throw new errors.VantError(`Policies: cannot disable revalidation on '${service}'`, {
                    code: errors.CODES.PERMISSION_DENIED, statusCode: 403
                });
            }
...
const REVALIDATE_INTERVAL_MS = _envInt(process.env.VANT_SUDO_REVALIDATE_INTERVAL_MS, 30000);
...
function createTask(taskId, scopes = DEFAULT_SCOPES) {
    if (_locked) throw new errors.VantError('ELOCKED', { code: errors.CODES.ELOCKED, retryable: true });
...
/**
 * Revalidate one expired grant (prd-sudo.md §5 + §12 health checks).
 * - revalidate: true + registered health probe → extend ONLY if probe passes,
 *   otherwise revoke with reason 'health_check_failed'
 * - revalidate: true without probe → extend (plain TTL revalidation)
 * - revalidate: false → revoke
 * Async because health probes are; never throws into the timer loop.
 */
async function _revalidateGrantOnce(taskId, scope, grants, g) {
    if (g._checking) return;
    g._checking = true;
    try {
        const policy = ESCALATION_WHITELIST[g.service] || ESCALATION_WHITELIST.default;
        const hc = _healthChecks.get(g.service);
        if (g.auto && policy.revalidate) {
            if (hc) {
                const r = await _runCheck(hc);
                if (grants.get(scope) !== g) return; // revoked/reset while probing
                if (!r.healthy) {
                    grants.delete(scope);
                    _emit('sudo:escalation_expired', {
                        taskId, scope, service: g.service,
                        reason: 'health_check_failed', detail: r.detail
                    });
                    const _mh = _m(); if (_mh) {
                        _mh.inc('vant_sudo_revalidations_total', { outcome: 'health_check_failed', service: g.service });
                        _gaugeActive();
                    }
                    return;
                }
 ...
 * - revalidate: true services → extend expiresAt (gated by service health
 *   checks when registered, prd-sudo.md §12)
    ...
    const _revalidateTick = () => {
        ...
        for (const [taskId, grants] of _grants) {
            for (const [scope, g] of grants) {
                ...
                if (g._checking) continue; // probe in flight from a previous tick
                // Fire-and-forget: health probes are async, the tick stays sync
                // (boot's registerTimer expects a plain function).
                Promise.resolve(_revalidateGrantOnce(taskId, scope, grants, g)).catch(() => {});
            }
        }
    };
...
module.exports = {
    ...
    // Service health checks for revalidation (prd-sudo.md §12)
    registerHealthCheck,
