    ...
    },
    {
        ...
        detect() {
            ...
            for (const brain of _listBrains()) {
                ...
                // (pass 37) ALSO live: arch-A protocol state files written by
                // lib/state-store.js — they carry { kind: 'vant-protocol-state'
                // }. That marker makes them skippable without a JSON.parse
                // per file, and migrate() eating trust.json/registry state is
                // exactly the failure this avoids.
                const legacy = _listDir(from).filter(f => {
                    if (f.endsWith('.json.md')) return false;
                    try {
                        const head = _relStore().read(path.join(from, f));
                        if (head) {
                            const parsed = JSON.parse(head);
                            if (parsed && parsed.kind === 'vant-protocol-state') return false;
                        }
                    } catch (e) { /* not JSON → legacy candidate */ }
                    return true;
                });
