...
const STEPS = [
    {
        ...
        detect() {
            const evidence = [];
            // A "brain dir" here means a dir holding brain CONTENT (.md), not
            // just any subdir: brain boot can create models/private/<name>/
            // orgchart/ stores on a legacy tree before migration runs, which
            // would otherwise mask the legacy signature forever.
            const hasBrainDirs = _listBrains().some(brain =>
                _walkModels(path.join('private', brain)).some(f => f.endsWith('.md'))
            );
