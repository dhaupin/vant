...
function _validateBrainName(brainName) {
    ...
    if (!brainName || typeof brainName !== 'string') {
        throw new errors.VantError('Invalid brain name: must be a non-empty string', { code: errors.CODES.VAF_VALIDATION_FAILED, retryable: false });
    }

    if (!/^[a-zA-Z0-9_-]+$/.test(brainName)) {
        throw new errors.VantError('Invalid brain name: alphanumeric, dash, underscore only', { code: errors.CODES.VAF_VALIDATION_FAILED, retryable: false });
    }
...
function _checkCanRead() {
    ...
    if (sandbox && !sandbox.canRead()) {
        throw new errors.VantError('Sandbox: read capability required', { code: errors.CODES.CAPABILITY_NOT_ALLOWED, retryable: false });
    }
}
...
function _checkCanWrite() {
    ...
    if (sandbox && !sandbox.canWrite()) {
        throw new errors.VantError('Sandbox: write capability required', { code: errors.CODES.CAPABILITY_NOT_ALLOWED, retryable: false });
    }
}
...
async function build(opts = {}) {
    // Run through unified pipeline for security
    return pipeline.run(
        { name: 'context:build', brain: opts.brain, operation: 'build' },
        async () => {
...
async function _gatherStatic(brainName) {
    ...
    // (F-1c) brain-tree reads through the models FileStorage (store-relative;
    // read contract: null on missing). Enumeration of secondary brains still
    // uses fs withFileTypes (pattern/limitation per PRD #7) but paths are
    // charset-gated and reads go through the store.
    const Storage = require('./storage');
    const modelsRoot = path.resolve('models');
    const store = new Storage.FileStorage({ basePath: modelsRoot });
    const _rel = (abs) => path.relative(modelsRoot, path.resolve(abs));

    // First, try to get static files in order
    for (const staticFile of STATIC_FILES) {
        const filePath = path.join(brainPath, staticFile);
        const content = store.read(_rel(filePath));
        if (content !== null) {
            files.push({
                name: staticFile,
    ...
    try {
        ...
        for (const b of allBrains) {
            ...
            // (R-6 pattern) brain name from the stack becomes a path segment
            if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(String(b)) || String(b).includes('..')) continue;
            ...
            if (fs.existsSync(bPath)) {
                ...
                for (const entry of entries) {
                    if (entry.isFile() && !STATIC_FILES.includes(entry.name)) {
                        const content = store.read(_rel(path.join(bPath, entry.name)));
...
function startHeartbeat() {
    ...
    const _tick = async () => { await ping(); };
    if (_bootMod && _bootMod.registerTimer) {
        _bootMod.registerTimer('context.heartbeat', _tick, HEARTBEAT_INTERVAL);
        _heartbeatTimer = true; // handled by boot registry
    } else {
        _heartbeatTimer = setInterval(_tick, HEARTBEAT_INTERVAL);
    }
...
function stopHeartbeat() {
    ...
    if (_heartbeatTimer) {
        ...
        const _bootMod = (() => { try { return require('./boot'); } catch (e) { return null; } })();
        if (_bootMod && _bootMod.unregisterTimer) _bootMod.unregisterTimer('context.heartbeat');
        else if (typeof _heartbeatTimer === 'object' && _heartbeatTimer) clearInterval(_heartbeatTimer);
