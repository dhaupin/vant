...
function info(msg, data) {
    if (_shouldLog('info')) _logOutput.write(`[INFO] ${msg}${data ? ' ' + JSON.stringify(data) : ''}\n`);
}
function warn(msg, data) {
    if (_shouldLog('warn')) _logOutput.write(`[WARN] ${msg}${data ? ' ' + JSON.stringify(data) : ''}\n`);
}
function error(msg, data) {
    if (_shouldLog('error')) _logOutput.write(`[ERROR] ${msg}${data ? ' ' + JSON.stringify(data) : ''}\n`);
}
