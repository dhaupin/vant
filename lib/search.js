...
// Helper: Validate brain path (block traversal)
function _getBrainPath() {
    // Read path: use brainPath() config or default to public for fresh installs
    let brainPath = process.env.MODEL_PATH || process.env.VANT_BRAIN_PATH || 'models/public';
    if (brainPath.startsWith('/') || brainPath.includes('..')) {
        brainPath = 'models/public';
    }
    return brainPath;
}
...
class HybridInner {
    ...
    loadCorpus() {
        const fs = require('fs');
        const path = require('path');
        // Read user's brain template path (MODEL_PATH), fallback to private
        const brainDir = path.join(__dirname, '../' + _getBrainPath());
...
function loadCorpus() {
    const fs = require('fs');
    const path = require('path');
    // Read user's brain template path
    const brainDir = path.join(__dirname, '../' + _getBrainPath());
...
module.exports = {
    ...
    queryBrain,
    query: queryBrain,  // alias for MCP compat
    getLTC,
    freshLTC,
    getSettings,
    loadCorpus,
    ...
    getCurrentCommit,
    getStats: () => {
        const corpus = new HybridInner().corpus;
        return {
