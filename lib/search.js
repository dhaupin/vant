 ...
 * 
 * Features:
 *   - Session caching (hybrid results)
 *   - Compact mode (summaries only)
 *   - Lazy-load search-hybrid (heavy module)
...
// Session cache for hybrid results
...
function getHybrid() {
    if (!hybridSearchLib) {
        hybridSearchLib = require('./search-hybrid');
    }
    return hybridSearchLib;
}
...
async function query(query, options = {}) {
    ...
    // Compact mode: just summary instead of full rehydrate
    if (compact) {
        return {
            results,
            context: results.map(r => `- ${r.summary || r.id}`).join('\n')
        };
    }
...
class HybridInner {
    ...
    add(doc) { this._docs.push(doc); return this; }
}
class HydeInner {
    constructor() {}
    async expand(prompt, count = 3) {
        const q = require('./search-hyde');
        return q.expandKeywords(prompt, count);
    }
}
class RerankInner {
    constructor() {}
    async rerank(memories, query, topK = 5) {
        const r = require('./rerank');
        return r.rerank(memories, query, topK);
    }
}
...
module.exports = {
    ...
    // Uses session cache + lazy-load
    _hybrid: null,
    hybrid(query, options = {}) {
        // Check cache first
        const cacheKey = Encrypt.md5.update(query).digest('hex');
        const cached = SEARCH_CACHE.get(cacheKey);
        if (cached) {
            return cached;
        }
        
        // Lazy load search-hybrid
        const hs = getHybrid();
        const result = hs.search(query, options);
