...
/**
 * Execute through VAF + QoS for search operations
 * Skips brain pipeline (vaf) since it expects different ctx structure
 * @private
 */
let _qos = null;
let _qosInstance = null;
async function _pipelineRun(query, options, operation) {
    // 1. VAF: Validate query input (directly, not through brain pipeline)
    if (vaf && vaf.check) {
        try {
            vaf.check(query, { 
                type: 'string', 
            ...
            _emit('search:vaf:blocked', { query, error: e.message, timestamp: Date.now() });
            throw new errors.VantError('SEARCH_VAF: ' + e.message, { 
    ...
    if (_qos && _qos.QoS) {
        if (!_qosInstance) {
            _qosInstance = new _qos.QoS({ maxPerMinute: 120, maxPerSecond: 20 });
        }
        
        try {
            await _qosInstance.check('_search_', 'query');
        } catch (e) {
            _emit('search:rate-limited', { query, timestamp: Date.now() });
            throw new errors.VantError('SEARCH_RATE: rate limit exceeded', { 
...
async function getLTC() {
    ...
    const item = await brain.load('start');
