/**
 * Vant QoS - Quality of Service (v0.8.6)
 * WITH EVENT EMISSIONS - rate limits emit globally
...
// Lazy audit — qos.js referenced audit.info/warn without ever importing it,
// which made every rate-limit block / circuit-open trip a swallowed ReferenceError.
let _audit = null;
function _getAudit() {
    if (!_audit) {
        try { _audit = require('./audit'); }
        catch (e) { _audit = { info: () => {}, warn: () => {}, error: () => {} }; }
    }
    return _audit;
}
const audit = { info: (...a) => _getAudit().info(...a), warn: (...a) => _getAudit().warn(...a), error: (...a) => _getAudit().error(...a) };
...
class Throttler {
    ...
    throttle(fn) {
        return (...args) => {
            const key = fn.name || Encrypt.key(16);
            const now = Date.now();
...
class RateLimiter {
    constructor(options) {
        // P3: tunable via env (VANT_* convention); explicit options still win
        this.options = {
            maxPerMinute: (options && options.maxPerMinute) || parseInt(process.env.VANT_QOS_MAX_PER_MINUTE || '60', 10),
            maxPerHour: (options && options.maxPerHour) || parseInt(process.env.VANT_QOS_MAX_PER_HOUR || '3600', 10),
            windowMs: (options && options.windowMs) || parseInt(process.env.VANT_QOS_WINDOW_MS || '60000', 10)
        };
    ...
    async check(clientId, operation) {
        ...
        if (globalRequests.length >= this.options.maxPerMinute) {
            ...
            // EVENT: rate limited
            _emit('qos:rate-limit', { clientId: '_global_', limit: this.options.maxPerMinute, timestamp: Date.now() });
...
class CircuitBreaker {
    ...
    _load() {
        ...
        try {
            // (F-2) when basePath is the models tree, read through FileStorage
            // (containment + atomic-contract); other basePaths keep raw fs
            const modelsRoot = path.resolve(process.cwd(), 'models');
            if (path.resolve(this._fullOptions.basePath) === modelsRoot) {
                const store = new (require('./storage').FileStorage)({ basePath: modelsRoot });
                const rel = path.relative(modelsRoot, path.resolve(filePath));
                const raw = store.read(rel);
                if (raw !== null) this._fullState = JSON.parse(raw);
                else this._fullState = { providers: {} };
            } else if (fs.existsSync(filePath)) {
                this._fullState = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            }
    ...
    _save() {
        ...
        const filePath = path.join(this._fullOptions.basePath, this._fullOptions.file);
        try {
            // (F-2) models-tree basePath → FileStorage atomic write
            const modelsRoot = path.resolve(process.cwd(), 'models');
            if (path.resolve(this._fullOptions.basePath) === modelsRoot) {
                const store = new (require('./storage').FileStorage)({ basePath: modelsRoot });
                const rel = path.relative(modelsRoot, path.resolve(filePath));
                store.write(rel, JSON.stringify(this._fullState, null, 2));
            } else {
                // (P2 #27) non-models fallback is atomic too — a crash mid-save
                // must never leave a truncated circuit snapshot that loads as
                // null and silently resets every breaker on next boot.
                require('./primitives').atomicWriteFile(filePath, JSON.stringify(this._fullState, null, 2));
            }
    ...
    async execute(fn) {
        ...
        if (this._state === 'OPEN') {
            ...
            else throw new errors.Error('Circuit OPEN', { code: errors.CODES.SANDBOX_CIRCUIT_OPEN, retryable: true });
        }
        try {
            var result = await fn();
            this._failures = 0;
...
class QoS {
    ...
    async check(clientId, operation) {
        ...
        if (this._bulkhead.isFull()) {
            throw new errors.Error('Too many concurrent requests', { code: errors.CODES.RATE_LIMIT_EXCEEDED, retryable: true });
        }
     ...
     */
    reset(clientId) {
        this._rateLimiter.reset(clientId);
    }

    /**
     * Alias of reset() - reset rate-limit state for a client
     * @param {string} clientId
     */
    resetRateLimiter(clientId) {
        this._rateLimiter.reset(clientId);
    }
...
// Lazy loader to avoid circular dependency at module load time
// Each export is a getter that returns the class only when accessed
const lazyExport = (name, value) => {
...
// Define exports lazily to prevent circular dep warnings
lazyExport('QoS', QoS);
lazyExport('RateLimiter', RateLimiter);
lazyExport('CircuitBreaker', CircuitBreaker);
lazyExport('Bulkhead', Bulkhead);
lazyExport('Throttler', Throttler);
lazyExport('Debouncer', Debouncer);

// Add aliases to QoS for backward compat (T12b — removed)
// All these were duplicates of methods already defined on the QoS class
// above. Removing the duplicates and the dead `defaultQoS` singleton.

// `RateLimit` alias (T12b — removed) was unused in the codebase; callers
// should `require('./qos').RateLimiter` directly.

// Default instances (T12b — removed; the canonical way to use QoS is
// `new QoS()` and pass it where needed).
...
function checkInputSize(content) {
    if (content && content.length > MAX_INPUT_SIZE) {
        throw new errors.Error('Input too large: max ' + MAX_INPUT_SIZE + ' bytes', { code: errors.CODES.VAF_INPUT_INVALID, retryable: false });
    }
}
...
// NOTE: Rate limit check is already done in _runBrainSecurityChain before pipeline runs
// This middleware is a pass-through that allows the operation if we got this far
module.exports.execute = async function(ctx) {
    // The rate limit was already checked in _runBrainSecurityChain before pipeline started
    // This is just a pass-through - if we got here, the operation was allowed
    return { allowed: true, layer: 'QoS', operation: ctx?.name || ctx?.operation || 'default' };
};

// ==================== MULTIBRAIN STACK SUPPORT ====================

/**
 * Get QoS stats from all brains in the stack
 * @returns {Object} Combined QoS info
 */
function getStackQoSStatus() {
...
module.exports.getStackQoSStatus = getStackQoSStatus;
