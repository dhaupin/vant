...
class CircuitBreaker {
    ...
    _isClosedFull(key) {
        ...
        if (state.open) {
            ...
            if (Date.now() - lastFailure > actualBackoff) {
                ...
                this._save();
                if (this._fullOptions.autoRetry) console.log(`[CircuitBreaker] Retrying ${key} after ${actualBackoff}ms`);
                return true;
            }
            return false;
        }
    ...
    _recordFailureFull(key) {
        ...
        if (state.failures >= this._options.threshold) {
            state.open = true;
            console.log(`[CircuitBreaker] OPEN for ${key} after ${state.failures} failures`);
        } else {
    ...
    _save() {
        ...
        } catch (e) { console.log(`[CircuitBreaker] Save error: ${e.message}`); }
    }
