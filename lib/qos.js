...
class CircuitBreaker {
    ...
    async execute(fn) {
        ...
        if (this._state === 'OPEN') {
            ...
            else throw new errors.VantError('Circuit OPEN', { code: errors.CODES.SANDBOX_CIRCUIT_OPEN, retryable: true });
        }
        try {
            var result = await fn();
            this._failures = 0;
...
class QoS {
    ...
    async check(clientId, operation) {
        ...
        if (!rateAllowed) {
            throw new errors.VantError('Rate limited', { code: errors.CODES.RATE_LIMIT_EXCEEDED, retryable: true });
        }

        if (this._bulkhead.isFull()) {
            throw new errors.VantError('Too many concurrent requests', { code: errors.CODES.RATE_LIMIT_EXCEEDED, retryable: true });
        }
...
function checkInputSize(content) {
    if (content && content.length > MAX_INPUT_SIZE) {
        throw new errors.VantError('Input too large: max ' + MAX_INPUT_SIZE + ' bytes', { code: errors.CODES.VAF_INPUT_INVALID, retryable: false });
    }
}
