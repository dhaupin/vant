...
module.exports.execute = async function(ctx) {
    // QoS middleware for brain pipeline - check rate limits
    const op = ctx?.name || ctx?.operation || 'default';
    if (!canProceed(op)) {
        throw new Error(`QoS: Rate limit exceeded for ${op}`);
    }
    return { allowed: true, layer: 'QoS', operation: op };
};