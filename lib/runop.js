 ...
 * Parallel to sandbox: legal provides emergency red-button switch
 */
...
async function run(operation, options = {}) {
    ...
    try {
        ...
        // ⚖️ LEGAL ISOLATED - FIRST check, then SANDBOX CONTAINS
        // If legal flipped (compromised), sandbox still contains it
        // Order: legal (check) → sandbox (contain) → execute
        if (_state.layers.legal?.checkGate) {
            ...
            // Even if legal passes, sandbox still restricts it
            if (_state.layers.sandbox?.can && !_state.layers.sandbox.can(operation)) {
                return { error: 'sandbox denied after legal', operation };
            }
        }
