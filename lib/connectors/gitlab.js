...
class GitLabProvider extends require("../remote").GitProvider {
    ...
    async checkout(branchName, create = true) {
        ...
        try {
            execSync(`git checkout ${branchName}`, this._gitOpts());
            return branchName;
        } catch (e) {
            if (create) {
                execSync(`git checkout -b ${branchName}`, this._gitOpts());
                return branchName;
            }
            throw e;
        }
    ...
    async commit(message, options = {}) {
        ...
        if (options.all !== false) {
            execSync(`git add -A`, this._gitOpts());
        }

        execSync(`git commit -m "${message}"`, this._gitOpts());
        return true;
    }
    ...
    async push(branch = null) {
        ...
        execSync(`git push -u origin ${branchName}`, this._gitOpts());
        return branchName;
    }
    ...
    async pull(branch = null) {
        const { execSync } = require('child_process');
        const branchName = branch || await this.currentBranch();
        execSync(`git pull origin ${branchName}`, this._gitOpts());
        return branchName;
    }
