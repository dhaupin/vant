...
class GitLabProvider extends require("../remote").GitProvider {
    constructor(config = {}) {
    ...
    async _request(endpoint, options = {}) {
        // P2 #25: through the base timeout layer (AbortController + coded
        // NETWORK_TIMEOUT) — never the bare global fetch.
    ...
    async push(branch = null) {
        ...
        this._gitExec(['push', '-u', 'origin', branchName]);
        return branchName;
    }

    async pull(branch = null) {
        const branchName = this._gitRef(branch || await this.currentBranch(), 'branch name');
        this._gitExec(['pull', 'origin', branchName]);
        return branchName;
    }
    ...
    async getPipelines(options = {}) {
        return await this._request(`/projects/${encodeURIComponent(this.repo)}/pipelines`, {
            ...options
        });
    }

    async getPipeline(pipelineId) {
        return await this._request(`/projects/${encodeURIComponent(this.repo)}/pipelines/${pipelineId}`);
    }

    async triggerPipeline(ref = 'main') {
        return await this._request(`/projects/${encodeURIComponent(this.repo)}/pipeline`, {
            method: 'POST',
    ...
    async getPipelineJobs(pipelineId) {
        return await this._request(`/projects/${encodeURIComponent(this.repo)}/pipelines/${pipelineId}/jobs`);
    }

    // ========== CHECKS (GitLab merges) ==========
    async getCheckRuns(sha) {
        // Get merge requests that contain this commit
        const mrs = await this.listMRs();
        return { check_runs: mrs.filter(mr => mr.sha === sha) };
    }

    // ========== ALIASES (for compatibility) ==========
    async getWorkflows() {
        // GitLab doesn't have "workflows" but has pipelines
        return await this.getPipelines();
    }

    async getWorkflowRuns(workflowId) {
        return await this.getPipeline(workflowId);
    }

    async triggerWorkflow(workflowId, ref = 'main') {
        return await this.triggerPipeline(ref);
    }
