class SelfHostedProvider extends require("../remote").GitProvider {
    constructor(config = {}) {
        super({ ...config, type: 'selfhosted' });
        // Explicit opt-in only. Selfhosted executes real git operations
        // (add/commit/push) against the git remote in the current working
        // directory, so "always configured" made every sync.pushAll()
        // broadcast side-effecting git commands in whatever CWD it ran in —
        // including repos that never asked for it. Configuration sources:
        //   - constructor config: { url } or { remoteUrl }
        //   - environment: VANT_SELFHOSTED_REMOTE (remote name or URL)
        //   - environment: VANT_SELFHOSTED=1 (use default remote 'origin')
        this.url = config.url || config.remoteUrl || process.env.VANT_SELFHOSTED_REMOTE || 'origin';
        this.token = config.token || process.env.GIT_TOKEN;
        this._explicit = !!(config.url || config.remoteUrl ||
                            process.env.VANT_SELFHOSTED_REMOTE ||
                            process.env.VANT_SELFHOSTED === '1');
    }

    isConfigured() {
        // Opt-in semantics: without explicit configuration the provider stays
        // dormant and sync broadcasts skip it entirely. (Previously hardcoded
        // `true` — see labs/TASKS.md session 2026-09-21 for the live-fire
        // demonstration of why that was a hazard.)
    ...
    async getRepoInfo() {
        ...
        const remoteUrl = execSync(`git remote get-url ${this.url}`, { encoding: 'utf8', ...this._gitOpts() }).trim();
        ...
        if (remoteUrl.includes('github.com')) {
            ...
            repo = execSync('basename $(pwd)', { encoding: 'utf8' }).trim(); // module-level util, not a remote op
        }
    ...
    async listBranches() {
        const { execSync } = require('child_process');
        const output = execSync('git branch -a', { encoding: 'utf8', ...this._gitOpts() });
        return output.split('\n').map(b => b.trim()).filter(b => b);
    }

    async currentBranch() {
        const { execSync } = require('child_process');
        return execSync('git branch --show-current', { encoding: 'utf8', ...this._gitOpts() }).trim();
    }
    ...
    async checkout(branchName, create = true) {
        ...
        try {
            execSync(`git checkout ${branchName}`, { encoding: 'utf8', ...this._gitOpts() });
            return branchName;
        } catch (e) {
            if (create) {
                execSync(`git checkout -b ${branchName}`, { encoding: 'utf8', ...this._gitOpts() });
                return branchName;
            }
            throw e;
        }
    ...
    async commit(message, options = {}) {
        ...
        if (options.all !== false) {
            execSync(`git add -A`, { encoding: 'utf8', ...this._gitOpts() });
        } else if (options.files?.length) {
            execSync(`git add ${options.files.join(' ')}`, { encoding: 'utf8', ...this._gitOpts() });
        }

        execSync(`git commit -m "${message}"`, { encoding: 'utf8', ...this._gitOpts() });
        return true;
    }
    ...
    async push(branch = null) {
        ...
        execSync(`git push -u ${this.url} ${branchName}`, this._gitOpts());
        return branchName;
    }
    ...
    async pull(branch = null) {
        const { execSync } = require('child_process');
        const branchName = branch || await this.currentBranch();
        execSync(`git pull ${this.url} ${branchName}`, this._gitOpts());
        return branchName;
    }
    ...
    async getTags() {
        const { execSync } = require('child_process');
        try {
            const output = execSync('git tag', { encoding: 'utf8', ...this._gitOpts() });
    ...
    async getContributors() {
        const { execSync } = require('child_process');
        try {
            const output = execSync("git shortlog -sne", { encoding: 'utf8', ...this._gitOpts() });
