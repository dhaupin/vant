/**
 * S3 Connector (v0.9.0-axolotl) — own S3-API client
 * (prd-storage.md "Remote connectors" — the last open prd-storage item)
 * NOTE: this is the S3-API OBJECT-STORAGE connector (S3/R2/MinIO/B2).
 * lib/remote.js is the GIT providers registry; connectors/cloudflare.js
 * talks the Cloudflare CONTROL API (Bearer, R2-only) — different things.
 *
 ...
 *
 *   const client = require('./connectors/s3').s3({ provider: 'r2', bucket: 'vant',
...
function createClient(options = {}) {
    ...
    return {
        ...
        /** PUT an object (string or Buffer). opts.contentType sets Content-Type. */
        async put(key, content, opts = {}) {
            const body = Buffer.isBuffer(content) ? content : Buffer.from(String(content), 'utf8');
            const headers = opts.contentType ? { 'content-type': String(opts.contentType) } : {};
            await _request('PUT', key, { body, headers });
            return true;
        },
...
/**
 * Parse a ListObjectsV2 XML body into [{key, size?, etag?}]. Shared by the
 * client's list() and the Cloudflare connector delegation (DI for tests).
 * p = the request prefix — the marker object equal to the prefix is skipped.
 */
function _parseListXml(xml, p) {
    const out = [];
    const re = /<(?:Key|Prefix)>([^<]*)<\/(?:Key|Prefix)>(?:[\s\S]*?<Size>(\d+)<\/Size>)?(?:[\s\S]*?<ETag>([^<]*)<\/ETag>)?/g;
    let m;
    while ((m = re.exec(xml)) !== null) {
        ...
        if (!k || k === p) continue;
        const item = { key: k };
        if (m[2] !== undefined) item.size = Number(m[2]);
        if (m[3] !== undefined) item.etag = m[3];
        out.push(item);
    }
    return out;
}
