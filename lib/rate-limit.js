...
function loadState() {
    ...
    try {
        ...
        // SECURITY: Validate loaded state has required fields and sane values
        if (!Number.isInteger(state.maxPerHour) || state.maxPerHour < 0 || state.maxPerHour > 10000) {
            state.maxPerHour = getConfig().maxPerHour;
        }
        if (!Number.isInteger(state.requestsThisHour) || state.requestsThisHour < 0) {
            state.requestsThisHour = 0;
        }
        if (!Number.isInteger(state.windowStart) || state.windowStart < 1000000000000) {
