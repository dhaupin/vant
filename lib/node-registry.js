...
// (pass 65 / star exercise) RE-HYDRATE for long-lived observers — the
// teams-refresh-seam rule applied to the PEER registry. A node that
// hydrated at boot (the coordination hub) never saw peers registered by
// other processes afterwards: its mesh-status view counted 1 peer while
// two orgs stood registered on the same store, and its liveness answers
// went stale. refresh() re-reads the store and MERGES unknown ids only
// (in-memory wins on conflict — the multi-process rule everywhere else).
// Throttled so a status poller cannot hammer the disk; force bypasses.
...
function refresh(options = {}) {
    ...
    let merged = 0;
    try {
        // Re-run the SAME hydration apply() the boot path uses (kind-marker
        // verified, merge-only) — hand-rolled re-reads would bypass the
        // marker check. _resetHydration + hydrate = merge-only re-read:
        // unknown ids adopted, in-memory wins on conflict.
        stateStore.hydrate({
            moduleName: 'node-registry',
            stateFile: STATE_FILE,
            apply: (snapshot) => {
        ...
        return { refreshed: true, merged, error: e.message };
    }
