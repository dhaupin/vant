...
function getStats() {
    return {
        total: _nodes.size,
        alive: _nodes.filter(n => n.status === 'alive').size,
        joining: _nodes.filter(n => n.status === 'joining').size,
        dead: _nodes.filter(n => Date.now() - n.lastSeen > 60000).size
    };
}
...
module.exports = {
    ...
    list,
    getStats,
    getLayerStatus: () => ({ name: 'Registry', type: 'discovery', version: '0.8.7', enabled: true }),
