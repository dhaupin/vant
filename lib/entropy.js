...
const MAX_BUFFER_SIZE = 10 * 1024 * 1024; // 10MB max input size limit
...
function generatePatches(input, options = {}) {
    ...
    if (isFile) {
        ...
        if (stats.size > MAX_BUFFER_SIZE) {
            throw new Error(`Input file too large: max ${MAX_BUFFER_SIZE / (1024*1024)}MB`);
        }
    ...
    // SECURITY: Validate buffer size to prevent CPU exhaustion
    if (buffer.length > MAX_BUFFER_SIZE) {
        throw new Error(`Input too large: max ${MAX_BUFFER_SIZE / (1024*1024)}MB`);
    }
    ...
    deassemble(buffer) {
        ...
        for (let i = 0; i <= buffer.length - winSize; i++) {
            ...
            if (this.isSpike(entropy)) {
                if (i > currentStart) {
                    const stableData = buffer.slice(currentStart, i);
                    patches.push({ type: "stable", data: stableData.toString("base64"), seed: this._generateSeed(stableData), start: currentStart, end: i });
                }
        ...
        if (currentStart < buffer.length) {
            const stableData = buffer.slice(currentStart);
            patches.push({ type: "stable", data: stableData.toString("base64"), seed: this._generateSeed(stableData), start: currentStart, end: buffer.length });
        }
        return patches;
    }
    // Generate semantic seed from stable data (first 50 chars as summary)
    _generateSeed(buffer) {
        if (!buffer || buffer.length === 0) return "";
        const text = buffer.toString("utf8").substring(0, 50).replace(/\n/g, " ").trim();
        return text.length < buffer.toString("utf8").length ? text + "..." : text;
    }
...
function createAdaptivePatch(input, options = {}) {
    ...
    return { 
        ...
        stats, 
        metadata: { 
            windowSize: adaptive.windowSize, 
            sensitivity: adaptive.sensitivity, 
            totalPatches: patches.length, 
            spikeCount: patches.filter(p => p.type === "spike").length,
            generation: options.generation || 1,
            parentEntropy: options.parentEntropy || null,
            entropyDelta: options.parentEntropy ? (stats.mean - options.parentEntropy).toFixed(4) : null
        } 
    };
}
// Calibration: auto-detect optimal sensitivity from sample
function calibrate(input, targetSensitivity = 1.5) {
    const sample = Buffer.isBuffer(input) ? input.slice(0, 1024) : Buffer.from(input, "utf8").slice(0, 1024);
    const ae = new AdaptiveEntropy({ sensitivity: targetSensitivity });
    ae.deassemble(sample);
    return ae.getStats();
}
