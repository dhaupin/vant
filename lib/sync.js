...
// NOTE (P2 #25 follow-up): _checkNetwork is deliberately FAIL-OPEN on the
// sandbox probe (`catch {}` below): the default sandbox denies canNetwork,
// and sync is a system-initiated broadcast whose per-op authorization lives
// at the entry point (recursion guard, escrow budget, circuit breakers,
// wall-clock caps via _capOp) plus network-layer SSRF walls on every HTTP
// call (GitProvider._requestJson → network.fetch). Flipping this to
// fail-closed would brick configured installs until sandbox scopes gain an
// explicit 'network' grant path for system tasks (prd-security canX()/can()
// migration item — tracked in labs/MEM.md).
    ...
    if (sandbox && sandbox.canNetwork) {
        try {
            if (!sandbox.canNetwork()) {
                throw new errors.VantError('Network permission required', { code: errors.CODES.NETWORK_DENIED, retryable: false });
            }
        } catch (e) {}
    }
}

// ==================== P2 #25: PROVIDER-OP WALL CLOCK ====================
// Belt-and-suspenders over the connectors' own _requestJson/_gitOpts
// timeouts: caps ANY provider op so a hung op (HTTP, git child, or an
// overridden method that never resolves) can no longer hang a sync
// broadcast/apply forever. Fails CLOSED with a coded, retryable error.

const SYNC_OP_TIMEOUT_MS = (() => {
    const raw = parseInt(process.env.VANT_SYNC_OP_TIMEOUT_MS || '', 10);
    return (Number.isFinite(raw) && raw > 0) ? raw : 120000;
})();

function _capOp(promise, ms = SYNC_OP_TIMEOUT_MS, label = 'provider op') {
    let timer = null;
    const timeout = new Promise((_, reject) => {
        timer = setTimeout(() => {
            reject(new errors.VantError(`${label} timed out after ${ms}ms`, {
...
function _providersRel() {
    return path.relative(path.resolve(process.cwd(), 'models'),
...
function getConfiguredProviders() {
    ...
    // Test DI seam (P2 #23/#24): fake providers registered via
    // _setTestProvider ride along for tests only. Production paths never
    // populate the map, so behavior is unchanged when it's empty.
    for (const provider of _testProviders.values()) {
        if (provider.isConfigured()) configured.push(provider);
    }

    return configured;
}

// ---------- Test provider DI (test-only; empty in production) ----------
const _testProviders = new Map();
function _setTestProvider(name, fake) { _testProviders.set(name, fake); }
function _clearTestProviders() { _testProviders.clear(); }
    ...
    try {
        ...
        for (const provider of providers) {
            ...
            try {
                ...
                const opMs = (Number.isFinite(options.opTimeoutMs) && options.opTimeoutMs > 0)
                    ? options.opTimeoutMs : SYNC_OP_TIMEOUT_MS;
                await _capOp(provider.commit(commitMessage, { all: true }), opMs, `${name}:commit`);
                await _capOp(provider.push(), opMs, `${name}:push`);
 ...
 */
// ==================== P2 #23/#24: real pull apply + conflict handling ====================

/**
 * Diff two corpus snapshots (pure). before/after entries: {id, content}.
 * Returns {added, updated, unchanged, total} with sorted name arrays.
 */
function diffCorpus(before, after) {
    const beforeMap = new Map((before || []).map(e => [e.id, e.content]));
    const afterMap = new Map((after || []).map(e => [e.id, e.content]));

    const added = [];
    const updated = [];
    let unchanged = 0;

    for (const [id, content] of afterMap) {
        if (!beforeMap.has(id)) {
            added.push(id);
        } else if (beforeMap.get(id) !== content) {
            updated.push(id);
        } else {
            unchanged++;
        }
    ...
    added.sort();
    updated.sort();
    return { added, updated, unchanged, total: afterMap.size };
}

// Git conflict-marker signatures (line-start anchored, like git's own
// checkout-conflict detection). At least one of these in a brain file means
// an unresolved merge landed in the working tree.
const _CONFLICT_MARKERS = /^<{7}( |$)/m;    // <<<<<<<
const _CONFLICT_MARKERS_2 = /^={7}$/m;     // =======
const _CONFLICT_MARKERS_3 = /^>{7}( |$)/m; // >>>>>>>

/**
 * Scan brain files for unresolved conflict markers.
 * @param {Array<{name: string, content: string}>} files
 * @returns {string[]} file names carrying markers
 */
function scanConflictMarkers(files) {
    const hits = [];
    for (const f of files || []) {
        if (!f || typeof f.content !== 'string') continue;
        if (_CONFLICT_MARKERS.test(f.content) ||
            _CONFLICT_MARKERS_2.test(f.content) ||
            _CONFLICT_MARKERS_3.test(f.content)) {
            hits.push(f.name);
        }
    }
    return hits;
...
/**
 * Snapshot the current brain corpus as {id, content} pairs (async, uncached
 * read so the snapshot is honest). Falls back to empty on failure — a
 * snapshot failure must not fake a diff.
 */
async function _corpusSnapshot() {
    try {
        const corpus = await loadCorpusFresh();
        return corpus.map(c => ({ id: c.id, content: c.content }));
    } catch (e) {
        audit.warn('[Sync] corpus snapshot failed: ' + e.message);
        return [];
    }
}

// loadCorpus bypassing the module cache (invalidate first, then read).
// brain.loadCorpus() returns the WARM cache verbatim, so a stale cache would
// fake an all-unchanged diff and hide pulled/conflicted content.
async function loadCorpusFresh() {
    try {
        const brain = require('./brain');
        brain.invalidateCorpusCache();
        return await brain.loadCorpus();
    } catch (e) { return []; }
}
    ...
    try {
        ...
        for (const provider of providers) {
            ...
            try {
                ...
                const opMs = (Number.isFinite(options.opTimeoutMs) && options.opTimeoutMs > 0)
                    ? options.opTimeoutMs : SYNC_OP_TIMEOUT_MS;
                const repoInfo = await _capOp(provider.getRepoInfo(), opMs, `${name}:repoInfo`);

                // P2 #23: honest corpus diff. Snapshot BEFORE the pull
                // (bypassing the corpus cache), pull, snapshot after.
                if (options.dryRun === true) {
                    const repo = `${repoInfo.owner}/${repoInfo.repo}`;
                    audit.info(`[Sync] ${name}: dry-run — pull skipped`);
                    results[name] = { success: true, dryRun: true, repo };
                    continue;
                }

                const before = await _corpusSnapshot();
                const pulledBranch = await _capOp(provider.pull(branch), opMs, `${name}:pull`);
                const after = await _corpusSnapshot();
                const applied = diffCorpus(before, after);

                // New files landed on disk — drop the corpus cache so the
                // next read reflects the pulled state.
                try { require('./brain').invalidateCorpusCache(); } catch (e) {}
                ...
                saveProviderState(name, 'healthy', Date.now(), { userCtx: SYSTEM_USER_CTX });
                recordSuccess(name);
                audit.info(`[Sync] ${name}: ✓ pulled ${pulledBranch} (+${applied.added.length} ~${applied.updated.length})`);
        ...
        if (success) {
            ...
            throw new errors.VantError('All providers failed', { code: errors.CODES.GITHUB_SYNC_FAIL, retryable: true });
        }
...
async function getStatus() {
    ...
    for (const provider of providers) {
        ...
        try {
            const branches = await _capOp(provider.listBranches(), SYNC_OP_TIMEOUT_MS, `${name}:listBranches`);
            const current = await _capOp(provider.currentBranch(), SYNC_OP_TIMEOUT_MS, `${name}:currentBranch`);
...
async function rebase(providerName) {
    // P2 #24: structured results (callers + CLI report these); DI first so
    // test providers resolve exactly like real ones.
    const providers = remote.getAllProviders();
    let provider = providers[providerName];
    if (!provider && _testProviders.has(providerName)) {
        provider = _testProviders.get(providerName);
    }
    ...
    try {
        // P2 #24: real rebase semantics. Snapshot → pull → classify → scan →
        // push ONLY when the working tree is clean of conflict markers.

        const before = await _corpusSnapshot();

        let pulledBranch;
        try {
            pulledBranch = await _capOp(provider.pull(), SYNC_OP_TIMEOUT_MS, `${providerName}:pull`);
        } catch (pullErr) {
            // Classify the pull failure: git conflict text → needsManual.
            const msg = pullErr.message || '';
            if (/CONFLICT|conflict|Merge conflict|needs merge|uncommitted|would be overwritten/i.test(msg)) {
                audit.error(`[Sync] ${providerName}: pull conflict — manual resolution required`);
                return {
                    success: false,
                    provider: providerName,
                    conflict: true,
        ...
        // Marker scan over the current brain corpus. A polluted brain file
        // must NEVER be pushed — that is how conflict soup propagates.
        const conflictedFiles = scanConflictMarkers(
            (after || []).map(c => ({ name: c.id, content: c.content }))
        );
        if (conflictedFiles.length > 0) {
            audit.error(`[Sync] ${providerName}: conflict markers in ${conflictedFiles.length} brain file(s) — push BLOCKED`);
            return {
                success: false,
                provider: providerName,
                conflict: true,
                needsManual: true,
                conflictedFiles,
                error: 'Unresolved conflict markers in brain files: ' + conflictedFiles.join(', ')
            };
        }

        // Clean tree → push the reconciled state (wall-clock capped, P2 #25)
        await _capOp(provider.push(), SYNC_OP_TIMEOUT_MS, `${providerName}:push`);

        saveProviderState(providerName, 'healthy', Date.now(), { userCtx: SYSTEM_USER_CTX });
        audit.info(`[Sync] ${providerName}: Rebased ✓ (+${applied.added.length} ~${applied.updated.length})`);

        return { success: true, provider: providerName, branch: pulledBranch, applied, conflict: false, pushed: true };
    } catch (e) {
...
module.exports = { getSummary,
    ...
    getProviderState,
    saveProviderState,
    // getAllCircuits exported for bin/validate.js. isCircuitClosed /
    // recordFailure / recordSuccess stay internal (module functions) — the
    // exports were pass-through aliases, not a supported surface.
    getAllCircuits,
...
function hybrid_setPrivacy(repo, privacy) {
    if (!['public', 'private'].includes(privacy)) throw new errors.VantError('Privacy must be public or private', { code: errors.CODES.GITHUB_SYNC_FAIL, retryable: false });
...
// (dead-export sweep round 2: hybrid_savePrivacyConfig / hybrid_setPrivacy /
// hybrid_getPublicRepos / hybrid_getPrivateRepos removed — zero external
// callers; hybrid_getPrivacyConfig + hybrid_getPrivacy retained, used
// internally by getStackPrivacy)
