...
function isCircuitClosed(provider) {
    ...
    const providerState = state.providers[provider] || { failures: 0, open: false };
    
    // If already open, don't try
    if (providerState.open) {
        // Check if should retry after timeout
        if (providerState.lastFailure && Date.now() - providerState.lastFailure > 60000) {
            // Auto-reset after 60s - try again
            state.providers[provider] = { failures: 0, open: false };
...
/**
 * Record failure - open circuit after 3 failures
 * @param {string} provider - Provider name
 */
function recordFailure(provider) {
    const state = getCircuitState();
    state.providers[provider] = state.providers[provider] || { failures: 0, open: false };
    state.providers[provider].failures++;
    state.providers[provider].lastFailure = Date.now();
    
    if (state.providers[provider].failures >= 3) {
        state.providers[provider].open = true;
        console.log('[CircuitBreaker] OPEN for ' + provider);
    }
