...
async function load(name, options = {}) {
    ...
    if (!depthCheck.allowed) {
        throw new Error('Island recursion depth exceeded for: ' + name);
    }
    ...
    try {
        ...
        if (def?.type === 'static' || def?.source === 'corpus') {
            ...
            if (options.brain) {
                ...
                if (brainInfo) {
                    const filePath = path.join(brainInfo.path, name + '.md');
                    if (fs.existsSync(filePath)) {
                        const content = await format.loadFile(filePath);
        ...
        if (def?.source === 'runtime') {
            if (name === 'trust') {
                ...
                return { name, type: 'runtime', module: trust, data: trust.export() };
            }
            if (name === 'market') {
                const market = require('./market');
                return { name, type: 'runtime', module: market, data: market.stats() };
            }
        }
