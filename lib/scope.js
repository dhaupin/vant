 ...
 */
function _resolveMembersFresh(ownerRef) {
...
/**
 * (pass 53) Member resolution with a STALE-VIEW RESCUE: on a miss, one
 * throttled synchronous refresh of the org model from the shared store,
 * then a single retry. The two-org JV exercise (pass 52) caught why: a
 * long-lived node hydrates teams ONCE at boot, so org-model writes made
 * by another process after that boot were invisible — receiving-side
 * scope gates denied real members forever (boot-race-dependent, correct
 * by luck of timing). The refresh is MERGE-ONLY (adopts unknown ids, never
 * overwrites local state) and the retry still fails closed: an entity
 * that does not exist in the shared store is DENY before and after.
 * Throttling (2s) bounds disk reads on adversarial miss floods.
 */
function resolveMembers(ownerRef) {
    const fresh = _resolveMembersFresh(ownerRef);
    if (fresh) return fresh;
    try {
        const t = _teams();
        if (t && typeof t._refreshSync === 'function') {
            const r = t._refreshSync();
            if (r && r.refreshed) return _resolveMembersFresh(ownerRef);
        }
    } catch (e) { /* refresh unavailable — the miss stands (fail-closed) */ }
    return null;
}
