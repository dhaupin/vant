...
function createBus(opts = {}) {
    ...
    function _onWebhookEvent(data) {
        ...
        if (handler) {
            ...
            // (pass 65 live-fire fix) Report the REAL dispatch to the
            // webhook ack: a dropped envelope (version/scope gates above)
            // never marks this, so the sender's ack carries dispatched:0
            // — delivery vs silent refusal is now observable over the wire.
            if (data._dispatchState) data._dispatchState.dispatched++;
        }
