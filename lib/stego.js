...
function encode(message, inputPath, outputPath, options = {}) {
    ...
    if (options.encrypt) {
        const encrypted = Encrypt.aesGcmEncrypt(message, options.encrypt);
...
function decode(imagePath, options = {}) {
    ...
    if (message.startsWith('ENC:') && options.decrypt) {
        try {
            ...
            message = Encrypt.aesGcmDecrypt(encrypted, options.decrypt).toString('utf8');
...
function encodeToBuffer(message, password) {
    ...
    // Encrypt if password provided
    if (password) {
        const encrypted = Encrypt.aesGcmEncrypt(message, password);
        dataToHide = 'ENC:' + encrypted.toString('base64');
    }
...
function decodeFromBuffer(buffer, password) {
    ...
    if (password && message.startsWith('ENC:')) {
        const encrypted = Buffer.from(message.slice(4), 'base64');
        message = Encrypt.aesGcmDecrypt(encrypted, password).toString('utf8');
    }
...
function createBootstrap(manifest, password) {
    ...
    const encrypted = Encrypt.aesGcmEncrypt(json, password);
...
function parseBootstrap(bootstrapStr, password) {
    ...
    const decrypted = Encrypt.aesGcmDecrypt(data, password);
    try {
...
module.exports = {
    ...
    decode,
    hasData,
    // Use Encrypt class for encryption (stego uses Encrypt internally)
    encodeToBuffer,
    decodeFromBuffer,
