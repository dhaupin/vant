...
function encode(message, inputPath, outputPath, options = {}) {
    ...
    if (inputPath) {
        ...
        if (inCheck.blocked) {
            throw new Error('VAF: Input path blocked: ' + inCheck.reason);
        }
    }
    if (outputPath) {
        const outCheck = vaf.checkPathTraversal(outputPath);
        if (outCheck.blocked) {
            throw new Error('VAF: Output path blocked: ' + outCheck.reason);
        }
    }
...
function decode(imagePath, options = {}) {
    ...
    if (imagePath) {
        ...
        if (pathCheck.blocked) {
            throw new Error('VAF: Path blocked: ' + pathCheck.reason);
        }
    }
