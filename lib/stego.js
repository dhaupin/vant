...
function encode(message, inputPath, outputPath, options = {}) {
    ...
    console.log(`[Stego] Encoded ${method}${message.length} bytes into ${outputPath}`);
}
...
function decode(imagePath, options = {}) {
    ...
    if (message.startsWith('ENC:') && options.decrypt) {
        try {
            ...
            console.log(`[Stego] Decrypted message`);
        } catch (e) {
            throw new Error('Decryption failed - wrong password?');
        }
    } else if (message.startsWith('ENC:') && !options.decrypt) {
        throw new Error('Message is encrypted, provide decrypt password');
    }
