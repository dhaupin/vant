...
function encode(message, inputPath, outputPath, options = {}) {
    ...
    if (options.password && options.encrypt !== false) {
        const encrypted = Encrypt.encrypt(message, options.password, { algorithm: "aes-256-gcm" });
        data = 'ENC:' + encrypted;
    }
    ...
    // Atomic write (same-dir temp + rename) so a crashed encode can't leave
    // a truncated horcrux image
    const atomicWrite = require('./storage').atomicWrite;
    atomicWrite(outputPath, Buffer.from(modified));
...
function decode(imagePath, options = {}) {
    ...
    if (options.password && data.startsWith('ENC:') && options.decrypt !== false) {
        data = data.slice(4);
        data = Encrypt.decrypt(data, options.password, { algorithm: "aes-256-gcm" });
    }
