...
async function init(options = {}) {
    ...
    try {
        ...
        // 9. Check for horcrux restore (fresh agent boot)
...
async function _tryHorcruxRestore(options = {}) {
    ...
    try {
        const password = options.horcruxPassword || '$brAin_sEed!!';
 ...
 */
async function restoreFromHorcrux(horcruxPath, password, options = {}) {
    const transform = require('./transform');
 ...
 */
async function inspectHorcrux(horcruxPath, password) {
    const transform = require('./transform');
    return transform.inspectHorcrux(horcruxPath, password);
}
