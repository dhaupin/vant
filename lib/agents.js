...
module.exports = {
    ...
    loadProto(name) {
        ...
        for (const root of roots) {
            ...
            for (const candidate of [rel, name + '.md']) {
                if (_getModelsStore().has(path.relative('models', path.join(root, candidate)))) {
        ...
        const content = _getModelsStore().read(found.rel);
    ...
    loadFolder(name) {
        ...
        for (const relPath of searchPaths) {
            try {
                if (_getModelsStore().has(relPath)) {
                    const content = _getModelsStore().read(relPath);
    ...
    listFolders() {
        ...
        // Search in brain-specific paths
        for (const brainName of brains) {
            if (!_validProtoName(brainName)) continue;  // (R-6) path-segment guard
...
async function restoreState(data) {
    ...
    for (const agent of data.agents) {
        if (!agent || typeof agent.id !== 'string' || !_validProtoName(agent.name || '')) continue;
