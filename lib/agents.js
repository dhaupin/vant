...
module.exports = {
    ...
    // NEW: Proto loading from models/agents/
    loadProto(name) {
        const fs = require('fs');
        const path = require('path');
        const FORMAT = require('./format');
        const baseDir = path.join(__dirname, '..', 'models', 'agents');
        
        // Try folder format first: vant-agent-{name}/AGENT.md
        let filePath = path.join(baseDir, 'vant-agent-' + name, 'AGENT.md');
        let content = null;
        ...
        return {
            ...
            format: filePath.includes('vant-agent-') ? 'folder' : 'flat'
        };
    },
    listProtos() {
        const fs = require('fs');
        ...
        try {
            // Folder format: vant-agent-*
            fs.readdirSync(baseDir)
                .filter(d => d.startsWith('vant-agent-'))
                .forEach(d => list.add(d.replace('vant-agent-', '')));
            
            // Flat format: *.md (exclude vant-* which are folders)
            fs.readdirSync(baseDir)
