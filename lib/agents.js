...
const _agentStore = new Storage.FileStorage({ basePath: path.resolve(path.dirname(AGENT_STORE)) });
const AGENT_STORE_FILE = path.basename(AGENT_STORE);
...
async function _saveAgents(agents) {
    _agentStore.write(AGENT_STORE_FILE, JSON.stringify(Array.from(agents)));
}

async function _loadAgents() {
    if (_agentStore.has(AGENT_STORE_FILE)) {
        try {
            return new Map(JSON.parse(_agentStore.read(AGENT_STORE_FILE)));
...
function spawn(options = {}) {
    ...
    // Get brain from options or auto-detect
...
module.exports = {
    ...
    getStatus: () => ({ enabled: true, agents: _agents.size }),    // Unified proto loader: brain priority + folder + flat + YAML parse
    loadProto(name) {
        if (!_validProtoName(name)) return null;
        ...
        for (const root of roots) {
            ...
            for (const candidate of [rel, name + '.md']) {
                if (_agentStore.has(candidate)) { found = { p: path.join(root, candidate), rel: candidate }; break; }
            }
            if (found) break;
        }
        if (!found) return null;

        const content = _agentStore.read(found.rel);
    ...
    loadFolder(name) {
        ...
        const searchPaths = [];
        for (const brainName of brains) {
            if (!_validProtoName(brainName)) continue;
            searchPaths.push(
        ...
        for (const relPath of searchPaths) {
            try {
                if (_agentStore.has(relPath)) {
                    const content = _agentStore.read(relPath);
...
async function restoreState(data) {
    if (!data || !data.agents) return { restored: 0 };
    return { restored: data.agents.length };
}
