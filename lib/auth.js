...
function _getStore() {
    if (!_store) {
        _store = new (require('./storage').FileStorage)({ basePath: path.join(__dirname, '..') });
    }
    return _store;
}
...
function _loadLockedAuth() {
    try {
        ...
        if (raw !== null) {
            ...
            _getAudit().info(`[Auth] Loaded ${valid.size} lockouts from ${LOCKOUT_FILE}`);
            return valid;
        }
    } catch (e) {
        _getAudit().info(`[Auth] Could not load lockouts: ${e.message}`);
    }
    return new Map();
}
...
function _saveLockedAuth(failedAttempts) {
    try {
        ...
        _getStore().write(LOCKOUT_FILE, JSON.stringify(data, null, 2));
    } catch (e) {
        _getAudit().info(`[Auth] Could not save lockouts: ${e.message}`);
    }
}
...
module.exports = {
    ...
    hashPassword: (pwd) => new Encrypt().hash(pwd),  // Uses Encrypt internally
