...
class SecurityGates {
    ...
    /**
     * Persist gate state
     * (P2 #27) Atomic write — a crash mid-save must never leave a truncated
     * trust/block DB (that would silently reset every breaker on next load).
     * The write failing (e.g. capability refusal) is logged, never thrown —
     * persistence must not break trust evaluation.
     */
    _save() {
        try {
            require('../primitives').atomicWriteFile(GATE_DB, JSON.stringify({
