...
function _getEscrow() {
    if (!_escrow) {
        try { _escrow = require('./lib/escrow'); } catch(e) { return null; }
    }
    return _escrow;
}
    ...
    try {
        ...
        if (memory.memory && memory.memory._cache) {
            for (const [key, value] of memory.memory._cache) {
                if (key.startsWith('state:')) stateData[key.replace('state:', '')] = value;
                else if (key.startsWith('learn:')) learnData[key.replace('learn:', '')] = value;
            }
        }
        return { stats, state: stateData, learned: learnData };
