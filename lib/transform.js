...
async function gather(options = {}) {
    ...
    if (opts.agents) result.agents = await gatherAgents();
    if (opts.islands) result.islands = await gatherIslands();
    if (opts.runtime) result.runtime = await gatherRuntime();
    if (opts.boot) result.boot = await gatherBoot();
    if (opts.config) result.config = await gatherConfig();
    if (opts.corpus) result.corpus = await gatherCorpus();
    if (opts.neurons) result.neurons = await gatherNeurons();
    if (opts.brainStorage) result.brainStorage = await gatherBrainStorage();
    if (opts.privateBrains) result.privateBrains = await gatherPrivateBrains();
    if (opts.metrics) result.metrics = await gatherMetrics();
    if (opts.handlers) result.handlers = await gatherHandlers();
    if (opts.configStorage) result.configStorage = await gatherConfigStorage();
    if (opts.islandState) result.islandState = await gatherIslandState();
    if (opts.mode) result.mode = await gatherMode();
    
    return result;
}

/**
 * Package for horcrux/SVG
 */
async function toHorcrux(options = {}) {
...
/**
 * Create horcrux file - format agnostic
 * @param {string} outputPath - Path to save horcrux (.svg for steganography, .json for plain)
 * @param {Object} options - { password, templateSvg, ...gatherOptions }
 * @returns {Promise<Object>} Result with path and size
 */
async function toHorcruxFile(outputPath, options = {}) {
    const fs = require('fs');
    const path = require('path');
    ...
    if (ext === '.svg') {
        ...
        if (!templateSvg) {
            ...
            templateSvg = path.join(__dirname, '..', 'models', 'public', 'boot', 'hypha-brain-horcrux.svg');
        ...
        // Read template
        const svgContent = fs.readFileSync(templateSvg, 'utf8');
...
/**
 * Embed horcrux data into SVG
 * Wires transform.toHorcrux() to stego.encodeSvg()
 */
async function embedToSvg(svgPath, password = 'brain') {
    const fs = require('fs');
    const path = require('path');
    const stego = require('./stego');
    
    // Security check
    _checkSecurity('embed-svg');
    
    // Gather horcrux data
    const json = await toHorcrux();
    ...
    // Save
    const outPath = svgPath.replace('.svg', '-horcrux.svg');
    fs.writeFileSync(outPath, embedded, 'utf8');
    
    return { embedded: true, path: outPath, size: embedded.length };
}

/**
 * Extract horcrux data from SVG
 */
async function fromSvg(svgPath, password = 'brain') {
    const fs = require('fs');
    const stego = require('./stego');
    
    _checkSecurity('from-svg');
 ...
 * Supports: .svg files (steganography)
 * @param {string} horcruxPath - Path to horcrux file
 * @param {string} password - Password for decryption (or null to use secret.js)
 * @returns {Promise<Object>} Parsed horcrux data
 */
async function fromHorcrux(horcruxPath, password = null) {
    const fs = require('fs');
    const path = require('path');
    ...
    // Get password from secret.js if not provided
    if (!password) {
        try {
            const secret = require('./secret');
            password = await secret.getPassword();
    ...
    if (ext === '.svg') {
        ...
        return fromSvg(horcruxPath, password);
    }
    
    // Fallback: treat as JSON file
 ...
 */
async function inspectHorcrux(horcruxPath, password = null) {
    const fs = require('fs');
    const path = require('path');
...
/**
 * Restore all systems from horcrux data
 */
async function restore(data) {
    ...
    // Restore agents (delegations, metrics) - track but don't overwrite active state
    if (data.agents) {
        // Agents state restoration is informational - don't overwrite running agents
        // Could restore delegations metrics in future
        results.restored.push('agents');
    }
    
    // Restore islands (manifests)
 ...
 */
async function embedToSvgFull(svgPath, password = 'brain', options = {}) {
    const fs = require('fs');
    const stego = require('./stego');
