...
function validateHorcrux(data, options = {}) {
    ...
    // Wrapped: { timestamp, version, type: 'vant-horcrux', data: {...} }
    // Unwrapped (from fromHorcrux): { timestamp, version, agents, islands, ... }
    ...
    } else if (timestamp < Date.now() - 31536000000) { // 1 year
    ...
    // Check type (only for wrapped format)
    if (isWrapped) {
        if (!data.type || data.type !== 'vant-horcrux') {
            errors.push('Invalid or missing horcrux type');
        }
    } else {
        // Unwrapped - check that it has expected inner structure
    ...
    // Check data structure (inner data)
    if (!innerData || typeof innerData !== 'object') {
        ...
        // Brain storage check
        if (d.brainStorage && typeof d.brainStorage !== 'object') {
        ...
        if (d.brainStorage && d.brainStorage.files) {
            const suspicious = d.brainStorage.files.filter(f => 
