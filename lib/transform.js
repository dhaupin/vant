...
function validateHorcruxData(data, options = {}) {
    ...
    // Handle both wrapped and unwrapped formats (data.payload is the new format, data.data is deprecated)
    ...
    const isWrapped = data.type === 'vant-horcrux' && (hasPayload || hasData);
    const innerData = isWrapped ? (data.payload || data.data) : data;
...
async function fromHorcrux(horcruxPath, options = {}) {
    ...
    if (ext === '.svg') {
        ...
        if (parsed.type === 'vant-horcrux' && (hasPayload || hasData)) {
            return parsed.payload || parsed.data;
        }
    ...
    try {
        ...
        // Unwrap vant-horcrux format (JSON case) - payload is new, data is deprecated
        const pHasPayload = parsed.payload !== undefined;
        const pHasData = parsed.data !== undefined;
        if (parsed.type === 'vant-horcrux' && (pHasPayload || pHasData)) {
            return parsed.payload || parsed.data;
        }
...
async function inspectHorcrux(horcruxPath, options = {}) {
    ...
    const hasPayload = data.payload !== undefined;
    const hasData = data.data !== undefined;
    if (data.type === 'vant-horcrux' && (hasPayload || hasData)) {
