...
async function restore(data) {
    ...
    if (false) { // LEGACY store-file write replaced by teams.restoreState (O-8)
        try {
            const teams = require('./teams');
            ...
            // (R-3) config-driven path — vaf-check before writing
            const _tvCheck = _getVaf().checkPathTraversal(storePath);
            if (_tvCheck.blocked) {
                throw new errors.VantError('Teams store path blocked', { code: errors.CODES.SECURITY_PATH_TRAVERSAL });
            }

            // Write teams data (atomic)
            const teamsData = {
            ...
            require('./storage').atomicWrite(storePath, JSON.stringify(teamsData, null, 2));
