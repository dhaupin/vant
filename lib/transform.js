...
async function toHorcruxFile(outputPath, options = {}) {
    ...
    if (ext === '.svg') {
        ...
        if (!templateSvg) {
            // Try default template in models/horcrux/
            templateSvg = path.join(__dirname, '..', 'models', 'horcrux', 'template.svg');
            if (fs.existsSync(templateSvg)) {
                ...
                try {
                    ...
                    throw new Error('SVG template not found. Provide templateSvg option or create models/horcrux/template.svg');
                }
            }
        } else {
