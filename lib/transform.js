...
// ==================== MODELS STORE HELPERS (R-3 follow-up fix) ====================
// The R-3 batch edit added CALL SITES for these in restore()'s legacy
// privateBrains path but the definitions never landed — every file restore
// through that path failed with "_safeBrainName is not defined" (silently
// converted to per-file errors). Defined here, once, matching the islands.js
// pattern.
...
async function gatherPrivateBrains() {
    await _checkSecurity('gather-private-brains');
...
async function gather(options = {}) {
    ...
    const {
        ...
        neurons = true,
        brainStorage = true,
        privateBrains = false,  // DISABLED: brainStorage covers this now (v0.8.6+)
        metrics = true,
        handlers = true,
        configStorage = true,
        islandState = true,
...
async function inspectHorcrux(horcruxPath, options = {}) {
    ...
    // Build preview - handle both old (privateBrains) and new (agents) formats
    const hasPrivateBrains = !!data.privateBrains;
    const privateBrainsData = data.privateBrains?.brains || [];
    ...
    // Check for duplicate brain storage formats
    const hasBothFormats = !!data.brainStorage && !!data.privateBrains;
    ...
    return {
        ...
        preview: {
            hasAgents: !!data.agents || hasPrivateBrains,
            hasIslands,
            hasRuntime,
            hasConfig,
            hasCorpus,
            hasPrivateBrains,
            hasTeams,
            agentCount: data.agents?.agents?.length || brainStorageData.length || privateBrainsData.length || 0,
            ...
            brainCount: brainStorageData.length || privateBrainsData.length || 0,
            ...
            // Warn if both brainStorage and privateBrains present
        },
        teamsError,
        hasBothFormats,
        raw: data
...
async function restore(data) {
    ...
    // 2b. Restore PRIVATE BRAINS (legacy format from old horcruxes)
    if (data.privateBrains && data.privateBrains.loaded && data.privateBrains.brains) {
        try {
            const privatePath = 'models/private';

            for (const brainData of data.privateBrains.brains) {
                const brainName = brainData.name;
                const brainPath = path.join(privatePath, brainName);
                ...
                for (const file of brainData.files || []) {
                    try {
                        // SECURITY (R-3): payload-controlled name — validate
                        if (!_safeBrainName(file.name)) {
                            throw new errors.VantError('Unsafe file name', { code: errors.CODES.SECURITY_PATH_TRAVERSAL });
                        }
                        const vaf = require('./vaf');
                        const fullPath = path.join(brainPath, file.name + '.md');
                        vaf.validateSafePath(fullPath, brainPath);

                        const rel = _modelsRel(fullPath);

                        // Only write if doesn't exist (don't overwrite)
                        if (!_getStore().has(rel)) {
                            _getStore().write(rel, file.content);
                            filesRestored++;
                        } else {
                            results.warnings = results.warnings || [];
                            results.warnings.push(`Skipped existing file: ${brainName}/${file.name}`);
                        }
                    } catch(e) {
                        results.errors.push({ file: `${brainName}/${file.name}`, error: e.message });
                    }
                }
                console.log(`[transform] Restored ${filesRestored} files to ${brainName} (privateBrains)`);
            }

            results.restored.push('privateBrains');
        } catch(e) {
            results.errors.push({ privateBrains: e.message });
        }
    }
