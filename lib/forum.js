...
class Forum {
    ...
    event.on('vote:consensus', (data) => {
      try {
        ...
        const decision = {
          ...
          proposal: vote ? vote.proposal : null,
          author: vote ? vote.author : null,
          decidedAt: Date.now()
        };
  ...
  /**
   * v0.9.0-axolotl T6: Delete / unpublish a publication.
  ...
  async unpublish(barcode) {
    // v0.9.0-axolotl T6: hard sandbox capability gate before any delete
  ...
  //   create(topic, { options: [..], minQuorum, scope, ... })
  ...
  async vote(proposal, options = {}) {
    ...
    const choices = Array.isArray(options.options) && options.options.length >= 2
        ? options.options
        : ['yes', 'no'];
