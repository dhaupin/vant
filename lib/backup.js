...
const _backupModelsRoot = path.resolve(__dirname, '..', 'models');
...
class BackupScheduler extends EventEmitter {
    ...
    async restore(backupPath, options = {}) {
        if (this.config.validateOnRestore) {
            ...
            const validation = _transform.validateHorcruxData(data);

            if (!validation.valid) {
                throw new errors.VantError('Backup validation failed', { code: errors.CODES.VALIDATION_FAILED });
            }
