...
const SUPPORTED_FORMATS = {
    ...
    txt: { extensions: ['.txt'], mime: ['text/plain'] }
};

const DEFAULT_EXTENSIONS = ['.yaml', '.yml', '.json', '.md', '.txt'];
...
function detectFromPath(filePath) {
    ...
    // Check extension
    for (const [fmt, info] of Object.entries(SUPPORTED_FORMATS)) {
        if (info.extensions.includes(ext)) {
            return { format: fmt, confidence: 0.95, source: 'path' };
        }
    }
    
    // Default: guess from path patterns
    return { format: 'txt', confidence: 0.3, source: 'default' };
}
        ...
        if (format === 'json') {
            ...
            if (raw.includes('!!')) {
                ...
                if (vaf?.check) {
                    ...
                    if (check.blocked) {
                        throw new Error('YAML unsafe tags blocked: ' + check.reason);
                    }
                }
            }
