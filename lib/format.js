 ...
 *   format.prepare(c)       → chained operations
 */
...
function serialize(data, format, opts = {}) {
    // (pass 41 live-fire fix) Primitives are serializable content: a string
    // IS valid md/txt/json-scalar content. The old guard returned '' for
    // every non-object, so saveFile wrote an EMPTY file and reported
    // { success: true } - silent data loss for any string-body write.
...
/**
 * Prepare data through format operations: detect → sanitize → parse → validate → serialize
 * @param {string|object} input - Content or parsed data
 * @param {object} opts - { chain, format, schema, ... }
 * @returns {object} Prepared data with format, content, data, steps, error
 */
...
async function loadFile(filePath, opts = {}) {
    ...
    return pipeline.run(
        { name: 'format:loadFile', filePath, operation: 'read' },
        async () => {
...
async function saveFile(filePath, data, opts = {}) {
    // Run through unified pipeline
    return pipeline.run(
        { name: 'format:saveFile', filePath, operation: 'write' },
        async () => {
            const detected = detectFromPath(filePath);
            ...
            try {
                if (storage?.write) {
                    ...
                    // (P2 #27) atomic fallback — the pipeline-routed path is
                    // already crash-safe through the store; the raw escape must be too.
                    await require('./primitives').atomicWriteFile(filePath, content);
                }
                return { success: true };
            } catch (e) {
