...
class Wal {
    ...
    _append(record) {
        ...
        try { fs.fsyncSync(fdForAppend(this.logPath)); } catch (e) { /* best effort */ }
    }
    ...
    replay(applyFn) {
        ...
        try {
            ...
            for (const rec of last.values()) {
                ...
                const target = path.resolve(this.basePath, rec.file);
                let targetMtime = 0;
                ...
                if (targetMtime > intentTs + 1) {
                    ...
                    this._append({ seq: rec.seq, file: rec.file, resolved: true, ts: Date.now() });
                    continue;
                }
                ...
                try {
                    ...
                    applyFn(rec.op, rec.file, payload);
                    stats.replayed++;
...
/** Small helper: open+append+fsync in one go (keeps _append tidy). */
function fdForAppend(file) {
