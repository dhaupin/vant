...
function fetch(url) {
    return new Promise((resolve, reject) => {
        https.get(url, { headers: { 'User-Agent': 'Vant-Updater' } }, (res) => {
            ...
            res.on('end', () => {
                try {
                    try { resolve(JSON.parse(data)); } catch { resolve({ tag_name: 'v0.0.0' }); }
                } catch (e) {
                    reject(e);
                }
            });
...
async function getLatestRelease() {
    vaf.check(REPO_NAME, {type: "string", name: "repo", maxLength: 50, pattern: /^[\w-]+$/});
    vaf.check(REPO_OWNER, {type: "string", name: "owner", maxLength: 50, pattern: /^[\w-]+$/});
    try {
