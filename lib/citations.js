...
// Shared safe-by-default capability gate (lib/gate.js)
function _checkRead() {
    gate.requireCapability('canRead', { scope: 'citations', operation: 'read' });
}

function _checkWrite() {
    gate.requireCapability('canWrite', { scope: 'citations', operation: 'write' });
}

// All file I/O goes through the storage layer (containment + symlink + VAF
// checks) instead of raw fs.
const fileStore = new Storage.FileStorage({ basePath: brain.getBrainPath() });
const CITATIONS_FILE = '.citations.json';
...
function getCitations() {
    ...
    const raw = fileStore.read(CITATIONS_FILE);
    if (raw !== null) {
        try {
...
function saveCitations(data) {
    ...
    fileStore.write(CITATIONS_FILE, JSON.stringify(data, null, 2));
}
...
function addSource(commit, context = '') {
    ...
    // Skip audit logging if module not available
    return source;
}
