...
class FileStorage {
    constructor(options = {}) {
        ...
        if (this._encrypt && !this._encryptKey) {
            throw new errors.Error('Encryption enabled but no key: pass encryptKey or set VANT_STORAGE_KEY', { code: 'STORAGE_ENCRYPT_KEY_REQUIRED', retryable: false });
        }
    ...
    _encryptContent(content) {
        ...
        if (typeof Encrypt.encrypt !== 'function') {
            throw new errors.Error('encrypt.js unavailable for storage encryption', { code: 'STORAGE_ENCRYPT_UNAVAILABLE', retryable: false });
        }
    ...
    _decryptContent(content) {
        ...
        if (!this._encryptKey) {
            throw new errors.Error('Encrypted file encountered but store has no key (VANT_STORAGE_KEY?)', { code: 'STORAGE_DECRYPT_KEY_REQUIRED', retryable: false });
        }
        const Encrypt = require('./encrypt');
        ...
        if (out && typeof out === 'object' && out.error) {
            throw new errors.Error('Storage decrypt failed: ' + out.error, { code: 'STORAGE_DECRYPT_FAILED', retryable: false });
        }
        return out;
    }
    ...
    read(filePath) {
        ...
        if (vaf?.checkPathTraversal) {
            ...
            if (check.blocked) {
                throw new errors.Error('Security: Path blocked', { code: 'VAF_PATH_BLOCKED', retryable: false });
            }
        }
    ...
    _validSnapshotLabel(label) {
        ...
        if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,63}$/.test(str)) {
            throw new errors.Error('Invalid snapshot label (use [a-zA-Z0-9._-], max 64): ' + str.slice(0, 40), { code: 'STORAGE_SNAPSHOT_LABEL_INVALID', retryable: false });
        }
        return str;
    }
    ...
    _readSnapshotManifest(id) {
        ...
        if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(String(id || ''))) {
            throw new errors.Error('Invalid snapshot id', { code: 'STORAGE_SNAPSHOT_LABEL_INVALID', retryable: false });
        }
        const raw = this.read(path.join(this._snapshotRoot(), id, 'manifest.json'));
        if (raw === null) {
            throw new errors.Error('Snapshot not found: ' + id, { code: 'STORAGE_SNAPSHOT_NOT_FOUND', retryable: false });
        }
        try {
            ...
            throw new errors.Error('Snapshot manifest corrupt: ' + e.message, { code: 'STORAGE_SNAPSHOT_CORRUPT', retryable: false });
        }
    }
    ...
    restoreSnapshot(id) {
        ...
        for (const f of manifest.files) {
            if (vaf?.checkPathTraversal) {
                ...
                if (check.blocked) {
                    throw new errors.Error('Security: Snapshot manifest contains blocked path: ' + f.path, { code: 'VAF_PATH_BLOCKED', retryable: false });
                }
            }
        }
...
class BrainStorage {
    ...
    _getFilePath(category, key) {
        ...
        if (!safeCategory || !safeKey) {
            throw new errors.Error('Invalid category/key', { code: errors.CODES.VAF_INPUT_INVALID, retryable: false });
        }
 ...
 * lib/remote-s3.js. ONE implementation covers AWS S3, Cloudflare R2, MinIO, and
...
class RemoteStorage {
    ...
    _getClient() {
        ...
        if (this._lazyError) {
            throw new errors.Error(this._lazyError, { code: 'STORAGE_REMOTE_NOT_CONFIGURED', retryable: false });
        }
        if (!this._client) {
            const remote = require('./remote-s3');
    ...
    _remoteKey(k) {
        ...
        if (s.startsWith('/') || s.includes('\\') || s.split('/').some(p => p === '.' || p === '..')) {
            throw new errors.Error('Security: absolute/backslash/traversal remote key refused', { code: errors.CODES.SECURITY_PATH_ESCAPE, retryable: false });
        }
