...
// Default write path for agents - keeps their brain separate from user's public brain
// User overrides with MODEL_PATH (or VANT_STORAGE_PATH for backwards compat)
let modelsPath = process.env.MODEL_PATH || process.env.VANT_STORAGE_PATH || 'models/private';
// SECURITY: Block path traversal
if (modelsPath.startsWith('/') || modelsPath.includes('..')) {
    modelsPath = 'models/private';
}
const MODELS_PATH = modelsPath;
...
// ==================== STATE STORAGE ====================
// Note: State now stored in models/private/.state.json (agent runtime brain)
class StateStorage {
    constructor(options = {}) {
