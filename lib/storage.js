...
function _checkReadSafe() {
    ...
    const sandbox = _getSandbox();
    if (!sandbox || typeof sandbox.canRead !== 'function') return;  // no sandbox in this build
    if (sandbox.canRead === _defaultSandboxRead) {
        // Default stub - sandbox hasn't been replaced. Allow with one-time warning.
    ...
    // Sandbox is a real config. Enforce.
    if (!sandbox.canRead()) {
...
module.exports = {
    ...
    // v0.9.0-axolotl: explicit unsafe bypass for callers that have already
    // validated inputs. Same shape as the safe variants but no inline
    // capability or VAF checks.
