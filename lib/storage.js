...
class FileStorage {
    ...
    _checkContainment(fullPath) {
        ...
        if (!resolved.startsWith(baseResolved)) {
            throw new Error('Security: Path escape detected');
        }
    }
    ...
    _checkSymlink(fullPath) {
        ...
        try {
            ...
            if (stats.isSymbolicLink()) {
                throw new Error('Security: Symlink attack detected');
            }
        } catch(e) {
            if (e.code === 'ENOENT') {
    ...
    write(filePath, content) {
        ...
        if (this._isDangerousFilename(filePath)) {
            throw new Error('Security: Dangerous filename blocked');
        }
        ...
        try {
            ...
            if (stats.isSymbolicLink()) {
                ...
                throw new Error('Security: Detected symlink attack');
            }
        } catch(e) {
            if (e.code === 'ENOENT') {
...
class StateStorage {
    ...
    setCurrent(key, value) {
        ...
        if (typeof key === 'object' && key !== null) {
            Object.assign(this._state.current, key);
        } else {
            this._state.current[key] = value;
        }
        this._save();
    ...
    setStatic(key, value) {
        ...
        if (typeof key === 'object' && key !== null) {
            Object.assign(this._state.static, key);
        } else {
