...
function loadBrainConfig(brain) {
    ...
    // (F-2) brain config read through the models FileStorage (containment +
    // read contract: null on missing) instead of raw fs
    try {
        const Storage = require('./storage');
        const modelsRoot = path.resolve(process.cwd(), 'models');
        const store = new Storage.FileStorage({ basePath: modelsRoot });
        const rel = path.relative(modelsRoot, path.resolve(configPath));
...
function saveBrainConfig(brain, config) {
    ...
    try {
        // (F-2) brain config write through the models FileStorage (atomic +
        // containment) instead of raw fs
        const Storage = require('./storage');
        const modelsRoot = path.resolve(process.cwd(), 'models');
        const store = new Storage.FileStorage({ basePath: modelsRoot });
        const rel = path.relative(modelsRoot, path.resolve(configPath));
        store.write(rel, JSON.stringify(config, null, 2));
...
function load() {
    ...
    _config = {
        ...
        // cloudflare config removed (v0.9.0-axolotl): R2 goes through the
        // S3 connector directly (provider 'r2' / VANT_REMOTE_* env) — no
        // CF-specific config or connector exists anymore.
    ...
    if (fs.existsSync(configPath)) {
        try {
            // SECURITY: evaluate config in a bare vm context - no require/
            // process/fs available. Configs are data, not code.
            const vm = require('vm');
            ...
            _config = { ..._config, ...(sandbox.module.exports || {}) };
        } catch (e) { /* ignore */ }
    }
    return _config;
}
...
function mcpRequireKey() {
    ...
    // (pass 42 live-fire) getFlag() returns NULL for an unset flag, not
    // undefined — the old `!== undefined` check was always true, so the
    // VANT_MCP_REQUIRE_KEY env var was dead code and the gate could never
    // be enabled from the environment. Null (unset) now falls through.
    const flagValue = getFlag('mcp.requireKey');
    if (flagValue !== null && flagValue !== undefined) return flagValue ? 'true' : 'false';
