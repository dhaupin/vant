...
function _checkRead() {
    // Permission check - always allow by default for now
    // TODO: integrate with actual sandbox permission system
    const sandbox = _getSandbox();
    if (sandbox && sandbox.canRead) {
        try {
            if (!sandbox.canRead()) {
                throw new Error('Read permission required for config loading');
            }
        } catch (e) {
            // If canRead fails, allow by default
        }
    }
}
...
function load() {
    ...
    _config = {
        ...
        mcp: { port: parseInt(process.env.VANT_MCP_PORT || '3100'), bind: process.env.VANT_MCP_BIND || '0.0.0.0', requireKey: process.env.VANT_MCP_REQUIRE_KEY || 'false', apiKey: process.env.VANT_MCP_API_KEY || null },
        server: { port: parseInt(process.env.VANT_SERVER_PORT || '3456'), bind: process.env.VANT_SERVER_BIND || '0.0.0.0', cert: process.env.VANT_SERVER_CERT || null, key: process.env.VANT_SERVER_KEY || null, insecure: process.env.VANT_SERVER_INSECURE === '1', authRequired: process.env.VANT_SERVER_AUTH_REQUIRED === '1' }
    };
...
function mcpBindAddress() { return process.env.VANT_MCP_BIND || '0.0.0.0'; }
...
function serverBind() { return process.env.VANT_SERVER_BIND || '0.0.0.0'; }
