...
function _checkRead() {
    ...
    if (sandbox && sandbox.canRead) {
        // DENY by default (sandbox returns false unless explicitly allowed)
        // If canRead() returns false, block config loading
        try {
            if (sandbox.canRead() === false) {
                throw new Error('Read permission denied by sandbox for config loading');
            }
        } catch (e) {
            // Log but don't block - sandbox is optional security layer
            if (_getLogger) {
                const logger = _getLogger();
                if (logger.warn) logger.warn('[Config] Sandbox check: ' + e.message);
            }
        }
    }
}
