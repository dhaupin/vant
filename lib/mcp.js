...
// Auto-wire ALL lib functions to MCP (lazy loader)
function autoWireAllLibs() {
    const fs = require('fs');
    const libDir = './lib';
    let wired = 0;
    
    const libs = fs.readdirSync(libDir).filter(f => f.endsWith('.js') && f !== 'mcp.js');
    
    libs.forEach(libFile => {
        const libName = libFile.replace('.js', '');
        if (_methods.has('vant_' + libName + '_hub')) return;
        ...
        try {
            ...
            const hubName = 'vant_' + libName + '_hub';
            _methods.set(hubName, {
                description: libName + ' hub (' + funcs.length + ' functions)',
                inputSchema: { type: 'object', properties: { 
                    fn: { type: 'string', enum: funcs },
                    args: { type: 'object' }
                }, required: ['fn'] },
                handler: async ({ fn, args = {} }) => {
                    const m = require('./' + libFile);
                    if (typeof m[fn] === 'function') {
                        return m[fn](args);
                    }
                    return { error: 'function not found', fn };
                }
            });
            wired++;
        } catch(e) {}
...
// NEW: Universal vant_call (call any function)
const _universalCache = {};
_methods.set('vant_call', {
    description: 'Call any Vant function (lib.fn or tool name)',
    inputSchema: { type: 'object', properties: { 
        name: { type: 'string', description: 'Function name like brain.load or vant_brain_load' },
        args: { type: 'object', description: 'Arguments to pass' }
    }, required: ['name'] },
    handler: async ({ name, args = {} }) => {
        // Check MCP tools first
        if (_methods.has(name)) {
            return _methods.get(name).handler(args);
        }
        // Check cached libs
        const [libName, fn] = name.includes('.') ? name.split('.') : [name, null];
        
        if (fn && _universalCache[libName]) {
            const lib = _universalCache[libName];
        ...
        try {
            ...
            _universalCache[libName] = lib;
            if (fn && typeof lib[fn] === 'function') {
                return lib[fn](args);
            }
        } catch(e) {
        ...
        return { available: Object.keys(_universalCache[libName] || {}).filter(k => typeof _universalCache[libName][k] === 'function') };
    }
});

// List all available functions
_methods.set('vant_list_functions', {
    description: 'List all available Vant functions',
    inputSchema: { type: 'object' },
    handler: async () => {
        const fs = require('fs');
        const result = [];
        const libs = fs.readdirSync('./lib').filter(f => f.endsWith('.js') && f !== 'mcp.js');
        libs.forEach(libFile => {
            const libName = libFile.replace('.js', '');
            try {
                const lib = require('./' + libFile);
                const funcs = Object.keys(lib).filter(k => typeof lib[k] === 'function' && !k.startsWith('_'));
                funcs.forEach(fn => result.push(libName + '.' + fn));
            } catch(e) {}
        });
        return { total: result.length, functions: result };
    }
});

// Lazy load all libs on startup
const _autoWired = autoWireAllLibs();
if (_autoWired > 0) console.log('[MCP] Auto-wired', _autoWired, 'lib hubs');
