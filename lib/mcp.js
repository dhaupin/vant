...
_methods.set('consensus_get', {
    description: 'Get a consensus ledger (votes, scope, metadata, status)',
    inputSchema: {
        type: 'object',
        properties: { topic: { type: 'string' } },
        required: ['topic']
    ...
    handler: async ({ topic }) => {
        return _consensus.get(topic);
    }
});

_methods.set('consensus_list', {
    description: 'List all consensus topics with status and vote counts',
    inputSchema: { type: 'object' },
    handler: async () => {
        return _consensus.list();
    }
});
