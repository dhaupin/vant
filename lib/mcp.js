...
// Lazy require for v0.9.0 modules
...
async function _runSecurityChain(method, params, context = {}) {
    const isWrite = method.startsWith('brain_save') || method.startsWith('brain_delete') || 
                    method.startsWith('storage_write') || method.startsWith('brain_link');
    
    // 1. VAF: Input Validation
    try {
        const vaf = require('./vaf');
        if (vaf && vaf.check) {
            const vafResult = vaf.check(JSON.stringify(params), { 
                mode: isWrite ? 'strict' : 'read' 
            });
            if (vafResult && vafResult.blocked) {
                _emit('mcp:vaf:blocked', { method, timestamp: Date.now() });
    ...
    try {
        ...
        if (qos && qos.canProceed) {
            if (!qos.canProceed()) {
                _emit('mcp:qos:throttled', { method, timestamp: Date.now() });
                throw new Error('Rate limit exceeded - circuit breaker open');
            }
            // Check input size for write operations
            if (isWrite) {
                const sizeCheck = qos.checkInputSize(JSON.stringify(params));
                if (!sizeCheck.valid) {
                    throw new Error('Input too large: ' + sizeCheck.reason);
                }
            }
            qos.incrementActive();
            context._qosCleanup = () => qos.decrementActive();
        }
    } catch (e) {
    ...
    if (isWrite) {
        try {
            ...
            if (escrow && escrow.create) {
                const escrowInstance = escrow.create({ habitat: context.habitat || 'default' });
                if (escrowInstance && escrowInstance.canWrite) {
                    const canWrite = await escrowInstance.canWrite(context.userCtx || {}, { method, params });
                    if (!canWrite) {
                        _emit('mcp:escrow:denied', { method, timestamp: Date.now() });
...
async function _executeWithSecurity(method, handler, params) {
    ...
    if (!depthCheck.allowed) {
        throw new Error('MCP execute recursion depth exceeded: ' + depthCheck.depth);
    }
    ...
    try {
        ...
        // QoS cleanup on success
        if (context._qosCleanup) context._qosCleanup();
        return result;
    } catch (e) {
        // QoS cleanup on error
        if (context._qosCleanup) context._qosCleanup();
        throw e;
    } finally {
...
_methods.set('brain_stack_async', {
    description: 'Get brain stack (waits for state load)',
    inputSchema: { type: 'object' },
    handler: async () => {
        return { stack: await _brain().getStackAsync() };
    }
});
...
_methods.set('forum_enter', {
    ...
    handler: async ({ forumId, userId }) => {
        return await _forum.enter(forumId, userId);
    }
});

_methods.set('forum_message', {
    description: 'Post message to forum',
    inputSchema: {
        type: 'object',
        properties: {
            forumId: { type: 'string' },
        ...
        required: ['forumId', 'userId', 'content']
    },
    handler: async ({ forumId, userId, content }) => {
        return await _forum.message(forumId, userId, content);
    }
});

_methods.set('forum_vote', {
    description: 'Vote on forum topic',
    inputSchema: {
        type: 'object',
        properties: {
            forumId: { type: 'string' },
        ...
        required: ['forumId', 'userId', 'topic', 'vote']
    },
    handler: async ({ forumId, userId, topic, vote }) => {
        return await _forum.vote(forumId, userId, topic, vote);
    }
});
...
_methods.set('forum_status', {
    ...
    inputSchema: {
        type: 'object',
        properties: { forumId: { type: 'string' } },
        required: ['forumId']
    },
    handler: async ({ forumId }) => {
        return await _forum.status(forumId);
    }
});

// ==================== ENCOUNTER / PEER DISCOVERY ====================

_methods.set('encounter_discover', {
    ...
    handler: async ({ capability, limit = 10 }) => {
        return await _encounter.discover(capability, limit);
    }
});

_methods.set('encounter_announce', {
    description: 'Announce peer capability',
    inputSchema: {
        type: 'object',
        properties: {
            peerId: { type: 'string' },
        ...
        required: ['peerId', 'capability']
    },
    handler: async ({ peerId, capability }) => {
        return await _encounter.announce(peerId, capability);
    }
});

_methods.set('encounter_list', {
    description: 'List peers',
    inputSchema: { type: 'object' },
    handler: async () => {
        return { peers: await _encounter.listPeers() };
    }
});

_methods.set('encounter_status', {
    description: 'Get encounter status',
    inputSchema: { type: 'object' },
    handler: async () => {
        return await _encounter.getStatus();
    }
});
...
_methods.set('escrow_create', {
    description: 'Create escrow budget for org',
    inputSchema: {
        type: 'object',
        properties: {
            org: { type: 'string' },
    ...
    handler: async ({ org, budget = 100, period = 86400000 }) => {
        return await _escrow.create(org, budget, period);
    }
});
...
_methods.set('escrow_canSpend', {
    ...
    handler: async ({ org, amount = 1 }) => {
        return await _escrow.canSpend(org, amount);
    }
});

_methods.set('escrow_hold', {
    description: 'Hold budget for operation',
    inputSchema: {
        type: 'object',
        properties: {
            org: { type: 'string' },
        ...
        required: ['org', 'amount', 'operation']
    },
    handler: async ({ org, amount, operation }) => {
        return await _escrow.hold(org, amount, operation);
    }
});

_methods.set('escrow_release', {
    description: 'Release held budget',
    inputSchema: {
        type: 'object',
        properties: {
            org: { type: 'string' },
    ...
    handler: async ({ org, amount, operation }) => {
        return await _escrow.release(org, amount, operation);
    }
});
...
_methods.set('escrow_status', {
    ...
    handler: async ({ org }) => {
        return await _escrow.getStatus(org);
    }
});
...
_methods.set('sudo_escalate', {
    ...
    handler: async ({ task, level = 1 }) => {
        return await _sudo.escalate(task, level);
    }
});
...
_methods.set('compute_eval', {
    ...
    handler: async ({ code, language = 'javascript' }) => {
        // For JS, wrap in parentheses for expression evaluation
        if (language === 'javascript') {
            return await _compute.eval('(' + code + ')', language);
        }
        return await _compute.eval(code, language);
    }
});
...
_methods.set('recursion_retry', {
    ...
    handler: async ({ operation, simulate = false, maxRetries = 3, backoff = 1000 }) => {
        ...
        const retryFn = async () => {
            attempts++;
            if (simulate && attempts < 3) throw new Error('Simulated failure #' + attempts);
...
_methods.set('brain_share', {
    ...
    handler: async ({ insight, source = 'unknown', tags = [] }) => {
        ...
        if (fs.existsSync(insightsPath)) {
            try { insights = JSON.parse(fs.readFileSync(insightsPath, 'utf8')); } catch (e) { console.warn("[mcp] Parse error:", e.message); }
        }
...
_methods.set('brain_link', {
    ...
    handler: async ({ from, to, type = 'relates' }) => {
        ...
        if (fs.existsSync(linksPath)) {
            try { links = JSON.parse(fs.readFileSync(linksPath, 'utf8')); } catch (e) { console.warn("[mcp] Parse error:", e.message); }
        }
...
_methods.set('brain_evolution_start', {
    name: 'brain_evolution_start',
    description: 'Start a new evolution session for tracking changes and insights',
    params: { sessionId: { type: 'string', description: 'Optional session ID' } },
...
_methods.set('brain_evolution_insight', {
    name: 'brain_evolution_insight',
    description: 'Record an insight during evolution session',
    params: { insight: { type: 'string', description: 'Insight text' } },
    handler: async (ctx, { insight }) => {
        const brain = _getBrain();
...
const _defaultPort = parseInt(process.env.VANT_MCP_PORT || config.mcpPort() || '3457');
...
async function start(options = {}) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        req.on('end', async () => {
            try {
                // AUTH: Check API key for POST requests
                ...
                if (config.mcpRequireKey() === 'true') {
                    ...
                    if (!result.valid) {
                        ...
                        res.end(JSON.stringify({
                            jsonrpc: '2.0',
                            error: { code: -32600, message: result.reason || 'Unauthorized' },
                            id: null
                        }));
                        return;
                    }
                ...
                _emit('mcp:request', { method, timestamp: Date.now() });
                ...
                const result = await _executeWithSecurity(method, handler.handler, params);
                
                // EVENT: MCP response
                _emit('mcp:response', { method, success: true, timestamp: Date.now() });
                ...
                // EVENT: MCP error
                _emit('mcp:error', { error: e.message, timestamp: Date.now() });
    ...
    return new Promise(resolve => {
        server.listen(port, () => {
            audit.info(`[MCP] Server running on port ${port}`);
            _server = server;
            resolve({ port });
        });
    });
...
// =========== ENVIRONMENT tools ===========
_methods.set('vant_environment_list', {
    description: 'List all registered environments',
    inputSchema: { type: 'object' },
    handler: async () => {
        const env = require('./environment');
        return { environments: env.list(), current: env.current().name };
    }
});

_methods.set('vant_environment_get', {
    description: 'Get environment details',
    inputSchema: { type: 'object', properties: { name: { type: 'string' } }, required: ['name'] },
    handler: async ({ name }) => {
        const env = require('./environment');
        const environment = env.get(name);
        if (!environment) {
            return { error: `Environment not found: ${name}` };
        }
        return environment;
    }
});

_methods.set('vant_environment_use', {
    ...
    handler: async ({ name }) => {
        ...
        try {
            ...
            return { error: e.message, available: env.list() };
        }
    }
});

_methods.set('vant_environment_register', {
    ...
    handler: async ({ name, config }) => {
        const env = require('./environment');
        const environment = env.register(name, config);
        return { registered: environment };
    }
});

_methods.set('vant_environment_status', {
...
_methods.set('vant_environment_languages', {
    description: 'Get available languages in current environment',
    inputSchema: { type: 'object' },
    handler: async () => {
        const env = require('./environment');
...
_methods.set('vant_environment_exec', {
    description: 'Execute code in current environment',
    inputSchema: { type: 'object', properties: { 
        code: { type: 'string' },
        lang: { type: 'string', default: 'node' },
        taskId: { type: 'string', default: 'default' }
    }, required: ['code'] },
    handler: async ({ code, lang, taskId }) => {
        const env = require('./environment');
        try {
            return await env.exec(code, { lang, taskId });
        } catch (e) {
...
module.exports = {
    start: async (options = {}) => {
            ...
            if (req.url === '/mcp/exec' && req.method === 'POST') {
                ...
                req.on('end', async () => {
                    try {
                        ...
                        if (!tool) {
                            throw new Error('Missing tool or method');
                        }
        ...
        const cfg = require('./config');
        const port = options.port || cfg.mcpPort() || 3457;
