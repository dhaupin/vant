...
_methods.set('brain_load', {
    ...
    handler: async ({ name }) => {
        const b = await _brain().loadBrain(name);
...
const _port = parseInt(process.env.VANT_MCP_PORT || config.mcpPort() || '3457');
...
module.exports = {
    start: async (options = {}) => {
        ...
        const server = http.createServer(async (req, res) => {
            ...
            // REST endpoints for habitat/streams
            ...
            if (url === '/streams' && req.method === 'POST') {
                let body = '';
                req.on('data', c => body += c);
                req.on('end', async () => {
                    try {
            ...
            // POST /streams/:id/enqueue - add work
            const enqueueMatch = url.match(/^\/streams\/([^/]+)\/enqueue$/);
            ...
            // POST /streams/:id/poll - poll for work
            const pollMatch = url.match(/^\/streams\/([^/]+)\/poll$/);
            ...
            const infoMatch = url.match(/^\/streams\/([^/]+)$/);
            if (infoMatch && req.method === 'GET') {
                const result = await mcp.execute('stream_info', { workId: infoMatch[1] });
                res.end(JSON.stringify(result));
                return;
            }
            
            // REST endpoints for brain
            // GET /brain - brain state
            if (url === '/brain' && req.method === 'GET') {
                const result = await mcp.execute('brain_state', {});
                res.end(JSON.stringify(result));
                return;
            }
            
            // GET /brain/ls - list brain files
            if (url === '/brain/ls' && req.method === 'GET') {
                const result = await mcp.execute('brain_list', {});
                res.end(JSON.stringify(result));
                return;
            }
            
            // GET /brain/:name - load brain file
            const brainLoadMatch = url.match(/^\/brain\/([^/]+)$/);
            if (brainLoadMatch && req.method === 'GET') {
                const result = await mcp.execute('brain_load', { name: brainLoadMatch[1] });
                res.end(JSON.stringify(result));
                return;
            }
            
            // POST /brain/:name - save brain file
            if (brainLoadMatch && req.method === 'POST') {
                let body = '';
                req.on('data', c => body += c);
                req.on('end', async () => {
                    try {
                        const args = JSON.parse(body);
                        args.name = brainLoadMatch[1];
            ...
            res.end(JSON.stringify({ error: 'not found', endpoints: [
                ...
                'GET /streams/:id', 'POST /streams/:id/enqueue', 'POST /streams/:id/poll',
                'GET /brain', 'GET /brain/ls', 'GET /brain/:name', 'POST /brain/:name'
            ]}));
        });
