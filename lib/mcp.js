...
_methods.set('vant_tmp_dropboxPut', {
    ...
    handler: async ({ name, content }) => {
        ...
        return tmp.dropboxPut(name, content);
    }
});
...
_methods.set('vant_tmp_dropboxList', {
    ...
    handler: async () => {
        const tmp = require('./tmp');
        return tmp.dropboxList();
    }
});
_methods.set('vant_tmp_dropboxDelete', {
    description: 'Delete from dropbox',
    inputSchema: { type: 'object', properties: { name: {type:'string'} } },
    handler: async ({ name }) => {
        const tmp = require('./tmp');
        return tmp.dropboxDelete(name);
    }
});
...
_methods.set('vant_tmp_myStuffPut', {
    description: 'Save private data (myStuff)',
    inputSchema: { type: 'object', properties: { name: {type:'string'}, content: {type:'string'} } },
    handler: async ({ name, content }) => {
        const tmp = require('./tmp');
        return tmp.myStuffPut(name, content);
    }
});
_methods.set('vant_tmp_myStuffGet', {
    ...
    handler: async ({ name }) => {
        const tmp = require('./tmp');
        return tmp.myStuffGet(name);
    }
});
_methods.set('vant_tmp_myStuffList', {
    description: 'List private data',
    inputSchema: { type: 'object' },
    handler: async () => {
        const tmp = require('./tmp');
        return tmp.myStuffList();
    }
});
_methods.set('vant_tmp_myStuffDelete', {
    description: 'Delete private data',
    inputSchema: { type: 'object', properties: { name: {type:'string'} } },
    handler: async ({ name }) => {
        const tmp = require('./tmp');
        return tmp.myStuffDelete(name);
    }
});

// Tmp: yourStuff (shared)
_methods.set('vant_tmp_yourStuffPut', {
    description: 'Save shared data (yourStuff)',
    inputSchema: { type: 'object', properties: { name: {type:'string'}, content: {type:'string'} } },
    handler: async ({ name, content }) => {
        const tmp = require('./tmp');
        return tmp.yourStuffPut(name, content);
    }
});
_methods.set('vant_tmp_yourStuffGet', {
    ...
    handler: async ({ name }) => {
        const tmp = require('./tmp');
        return tmp.yourStuffGet(name);
    }
});
_methods.set('vant_tmp_yourStuffList', {
    description: 'List shared data',
    inputSchema: { type: 'object' },
    handler: async () => {
        const tmp = require('./tmp');
        return tmp.yourStuffList();
    }
});
_methods.set('vant_tmp_yourStuffDelete', {
    ...
    handler: async ({ name }) => {
        const tmp = require('./tmp');
        return tmp.yourStuffDelete(name);
    }
});


// Shell via tmp
_methods.set('vant_tmp_shellExec', {
    description: 'Execute command via shell',
    inputSchema: { type: 'object', properties: { cmd: {type:'string'}, options: {type:'object'} } },
    handler: async ({ cmd, options }) => {
        const tmp = require('./tmp');
        return tmp.shellExec(cmd, options);
    }
});
_methods.set('vant_tmp_shellCapture', {
    description: 'Execute and capture output',
    inputSchema: { type: 'object', properties: { cmd: {type:'string'}, options: {type:'object'} } },
    handler: async ({ cmd, options }) => {
        const tmp = require('./tmp');
        return { output: await tmp.shellCapture(cmd, options) };
    }
});
_methods.set('vant_tmp_shellSpawn', {
    description: 'Execute background command',
    inputSchema: { type: 'object', properties: { cmd: {type:'string'}, options: {type:'object'} } },
    handler: async ({ cmd, options }) => {
        const tmp = require('./tmp');
        return tmp.shellSpawn(cmd, options);
    }
});
