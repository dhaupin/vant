...
// v0.9.0 fs->storage migration: MCP's own file-backed stores and the raw
// vant_storage_* file tools route through FileStorage (containment/symlink/
// VAF/atomic-write). Insights/skills/agents scans anchored to models roots.
const _mcpStore = new (require('./storage').FileStorage)({
    basePath: path.resolve(__dirname, '..', 'models')
});
const _MCP_ROOT = path.resolve(__dirname, '..', 'models');
...
_methods.set('brain_share', {
    ...
    handler: async ({ insight, source = 'unknown', tags = [] }) => {
        ...
        if (require('fs').existsSync(agentsDir)) {
            for (const file of require('fs').readdirSync(agentsDir)) {
                if (!file.endsWith('.md') || !_validSegment(file)) continue;
