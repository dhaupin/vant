...
_methods.set('recursion_batch', {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            processor: { type: 'string', description: 'Function name: processItem(item, depth)' },
            maxDepth: { type: 'number', description: 'Max recursion depth (default: 10)' }
        },
        required: ['operation', 'items', 'processor']
    },
    handler: async ({ operation, items, processor, maxDepth = 10 }) => {
        const guard = require('./recursion');
        // Get processor function from global or create simple mapper
        const fn = global[processor] || ((item) => item);
...
_methods.set('recursion_traverse', {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            getChildren: { type: 'string', description: 'Function name: getChildren(node)' },
            visitor: { type: 'string', description: 'Function name: visitor(node, depth)' },
            maxDepth: { type: 'number' },
            breadthFirst: { type: 'boolean' }
        },
        required: ['operation', 'root', 'getChildren', 'visitor']
    },
    handler: async ({ operation, root, getChildren, visitor, maxDepth = 10, breadthFirst = false }) => {
        const guard = require('./recursion');
        const gc = global[getChildren] || ((n) => n.children || []);
        const vis = global[visitor] || ((n) => n);
...
_methods.set('recursion_lookup', {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            lookup: { type: 'string', description: 'Function name: lookup(item)' },
            validator: { type: 'string', description: 'Function name: validator(item)' },
            maxDepth: { type: 'number' },
            includePath: { type: 'boolean' }
        },
        required: ['operation', 'start', 'lookup']
    },
    handler: async ({ operation, start, lookup, validator, maxDepth = 10, includePath = true }) => {
        const guard = require('./recursion');
        const lk = global[lookup] || ((item) => item.next);
        const val = global[validator] || (() => true);
...
_methods.set('recursion_retry', {
    ...
    inputSchema: {
        ...
        properties: {
            operation: { type: 'string' },
            fn: { type: 'string', description: 'Function name: fn(attempt)' },
            maxRetries: { type: 'number' },
            backoff: { type: 'number', description: 'Base backoff in ms' }
        },
        required: ['operation', 'fn']
    },
    handler: async ({ operation, fn, maxRetries = 3, backoff = 1000 }) => {
        const guard = require('./recursion');
        const retryFn = global[fn];
        if (!retryFn) {
            return { success: false, error: 'Function not found: ' + fn };
        }
