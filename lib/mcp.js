...
module.exports = {
    start: async (options = {}) => {
        // Use express for routing (in deps)
        ...
        app.post('/mcp/exec', async (req, res) => {
            const { tool, args = {} } = req.body || {};
            const result = await mcp.execute(tool, args);
            res.json({ result });
        });
        
        // Start Server (for security chain)
        ...
        // Mount express on vant server
        console.log('[MCP] Server on', srv.options.port, '-', mcp.listTools().length, 'tools');
        return { port: srv.options.port, server: srv, app };
    },
    stop,
    listTools,
    methods: _methods,
