...
// (F-1a) mood.ini lives in the brain tree — resolve PER CALL so pushBrain
// moves it, and read/write through the models FileStorage
let _vibeStore = null;
function _getStore() {
    if (!_vibeStore) {
        const Storage = require('./storage');
        _vibeStore = new Storage.FileStorage({ basePath: require('path').resolve(MODELS_PATH) });
    }
    return _vibeStore;
}
const VIBE_FILE = 'mood.ini';
...
function getMood() {
    ...
    const content = store.read(VIBE_FILE);
    if (content === null) {
...
`;
    ...
    _getStore().write(VIBE_FILE, content);
