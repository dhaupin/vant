...
class GitProvider {
    ...
    _gitExec(args, opts = {}) {
        if (!Array.isArray(args) || args.some(a => typeof a !== 'string')) {
            throw new errors.Error('git exec requires an argv array of strings', { code: errors.CODES.GIT_ERROR, retryable: false });
        }
        ...
        try {
            ...
            const err = new errors.Error('Git error: ' + (e.message || String(e)), { code: errors.CODES.GIT_ERROR, retryable: true });
            err.cause = e;
            throw err;
        }
    }
    ...
    _gitRef(name, what = 'git ref') {
        if (typeof name !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9._\/-]*$/.test(name) || name.includes('..')) {
            throw new errors.Error('Invalid ' + what + ': ' + String(name).slice(0, 40), { code: errors.CODES.VAF_PATH_BLOCKED, retryable: false });
        }
        return name;
    }
