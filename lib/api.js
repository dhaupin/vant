...
class API {
    ...
    constructor(options = {}) {
        ...
        // Authentication
        this._secret = options.secret || env.apiKey(options) || null;
     ...
     */
    authenticate(secret) {
        // Check if currently locked (duration-based)
        if (this._lockedAt && Date.now() - this._lockedAt < this._lockoutDuration) {
            return { allowed: false, reason: 'auth_locked', remaining: Math.ceil((this._lockoutDuration - (Date.now() - this._lockedAt))/1000) };
        }
        ...
        if (!this._secret) {
            this._authFailures++;
            if (this._authFailures >= this._maxAuthFailures) this._lockedAt = Date.now();
            return { allowed: false, reason: 'no_secret_configured' };
        }
        
        // Check if at max failures (but not locked yet)
        if (this._authFailures >= this._maxAuthFailures) {
            this._lockedAt = Date.now();
            return { allowed: false, reason: 'auth_locked' };
        }
        
        // Secret length check
        if (!secret || secret.length !== this._secret.length) {
            this._authFailures++;
            if (this._authFailures >= this._maxAuthFailures) this._lockedAt = Date.now();
        ...
        // Compare secret
        const valid = secret === this._secret;
        if (!valid) {
            this._authFailures++;
            if (this._authFailures >= this._maxAuthFailures) this._lockedAt = Date.now();
        }
        
        return { allowed: valid, reason: valid ? 'authorized' : 'unauthorized' };
    }
...
module.exports = {
    ...
    // ALL PROTECTED - require context.secret or env.VANT_API_KEY
    getMode(context) {
        const secret = env.apiKey(context);
