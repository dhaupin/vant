...
class API {
    ...
    constructor(options = {}) {
        ...
        // Authentication
        this._secret = options.secret || process.env.VANT_API_KEY || null;
    ...
    authenticate(secret) {
        ...
        // Constant-time comparison not needed for uuid, but check anyway
        if (!secret || secret.length !== this._secret.length) {
            this._authFailures++;
            return { allowed: false, reason: 'unauthorized' };
        }
        
        // Simple comparison (uuid, no timing leak risk)
        const valid = secret === this._secret;
        if (!valid) this._authFailures++;
    ...
    getAuthStatus() {
        return {
            ...
            locked: this._authFailures >= this._maxAuthFailures
        };
    }
...
module.exports = {
    ...
    getMode(context) {
        const secret = (context && context.secret) || process.env.VANT_API_KEY;
    ...
    setSecret(secret, context) {
        ...
        const key = (context && context.secret) || process.env.VANT_API_KEY;
