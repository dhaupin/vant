...
// Max publish depth config
function _getMaxPublishDepth() {
    return config?.get ? config.get('msg.maxPublishDepth', 10) : 10;
}
...
class Msg {
    ...
    // Publish recursion guard
    _publishDepth = new Map();
    
    /**
     * Publish (sync handlers with recursion guard)
     */
    publish(channel, message) {
        // Prevent infinite publish loops
        const maxDepth = _getMaxPublishDepth();
        const depth = this._publishDepth.get(channel) || 0;
        if (depth >= maxDepth) {
            console.warn('[Msg] Publish blocked: ' + channel + ' at depth ' + depth);
            return false;
        }
        this._publishDepth.set(channel, depth + 1);
        ...
        try {
            ...
            this._publishDepth.set(channel, Math.max(0, depth - 1));
        }
    }
