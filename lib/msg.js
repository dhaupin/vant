...
class Msg {
    ...
    post(convId, content, options = {}) {
        ...
        if (!forcePlain) {
            ...
            if (isEncrypted(content)) {
                ...
                // Paranoid mode: Encrypt then steganographize
                const encrypted = encrypt.encrypt(content, DEFAULT_ENCRYPT_KEY);
                messageContent = 'ENC:' + encrypted;
                const embedded = stego.embed(messageContent, { key: DEFAULT_STEGO_KEY, data: content });
                messageContent = 'STEGO:' + JSON.stringify(embedded);
    ...
    reveal(messageContent) {
        ...
        try {
            const stegoData = JSON.parse(messageContent.slice(6)); // Remove 'STEGO:'
            const revealed = stego.extract(stegoData, { key: DEFAULT_STEGO_KEY });
    ...
    revealAuto(messageContent) {
        if (isEncrypted(messageContent) && isStego(messageContent)) {
            // Encrypt+Stego: Extract then decrypt
            try {
                const stegoData = JSON.parse(messageContent.slice(6));
                const extracted = stego.extract(stegoData, { key: DEFAULT_STEGO_KEY });
                if (extracted.startsWith('ENC:')) {
                    const decrypted = encrypt.decrypt(extracted.slice(4), DEFAULT_ENCRYPT_KEY);
                    return { content: decrypted, layer: 'encrypt+stego' };
                }
                return { content: extracted, layer: 'stego' };
            } catch (e) {
                return { error: 'Reveal failed', details: e.message };
            }
