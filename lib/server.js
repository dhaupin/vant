    ...
    async _handleRequest(req, res) {
        ...
        req.on('end', async () => {
            try {
                ...
                } else if (url === '/brain' && req.method === 'GET') {
                    const Brain = require('./brain');
                    result = { brains: Brain.listBrains() };
                } else if (url.startsWith('/brain/') && req.method === 'GET') {
                    const name = url.split('/brain/')[1];
                    const Brain = require('./brain');
                    result = await sandbox.execute(() => Brain.loadBrain(name), { type: 'read' });
                } else if (url === '/brain/state' && req.method === 'GET') {
                    const Brain = require('./brain');
                    result = Brain.getNeuronState();
...
Server.Router = class Router {
    ...
    match(method, url) {
        return this._routes.find(r => r.method === method && this._match(r.pattern, url));
    }
    _match(pattern, url) {
