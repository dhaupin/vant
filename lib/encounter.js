...
class Encounter {
  ...
  announce() {
    ...
    const timerName = 'encounter.' + (this.id || 'default');
    const tick = async () => { await this._broadcastPresence(); };
    if (_bootMod && _bootMod.registerTimer) {
      _bootMod.registerTimer(timerName, tick, this.announcementInterval);
      this.announcementId = timerName;
    } else {
      this.announcementId = setInterval(tick, this.announcementInterval);
    }
  ...
  silence() {
    if (this.announcementId) {
      ...
      if (typeof this.announcementId === 'string') {
        const _bootMod = (() => { try { return require('./boot'); } catch (e) { return null; } })();
        if (_bootMod && _bootMod.unregisterTimer) _bootMod.unregisterTimer(this.announcementId);
      } else {
