...
async function embed(text, options = {}) {
    // RLS check (REQUIRED)
    if (!options.userCtx) {
        throw new Error('EINVAL: userCtx required for embed');
    }
    const rls = _getRLS();
    if (rls) {
        rls.checkWrite(options.userCtx, '_embed:vector', 'write');
    }

    _emit('embed:generating', { textLength: text?.length, timestamp: Date.now() });
    ...
    if (result.length !== EMBED_DIM) {
        ...
        _emit('embed:generated', { dimension: EMBED_DIM, timestamp: Date.now() });
        
        return normalized;
    }
    
    _emit('embed:generated', { dimension: result.length, timestamp: Date.now() });
...
async function embedBatch(texts) {
    ...
    _emit('embed:batch:starting', { count: texts.length, timestamp: Date.now() });
    
    if (_currentEmbedder.embedBatch) {
        const results = await _currentEmbedder.embedBatch(texts);
        _emit('embed:batch:complete', { count: texts.length, timestamp: Date.now() });
        return results;
    }
