...
function setEmbedder(name) {
    if (!_embedders.has(name)) {
        throw new errors.VantError('Unknown embedder', { code: errors.CODES.VAF_INPUT_INVALID });
    }
...
/**
 * Embed text across all brains in the stack
 * Each brain may have different embedders registered
 * @param {string} text - Text to embed
 * @param {Object} options - Options
 * @returns {Promise<Array>} Combined embeddings from all brains
 */
async function embedStack(text, options = {}) {
...
/**
 * Embed batch of texts across all brains in the stack
 * @param {Array} texts - Array of texts to embed
 * @param {Object} options - Options
 * @returns {Promise<Object>} Combined embeddings by brain
 */
async function embedBatchStack(texts, options = {}) {
    const brain = require('./brain');
    const stack = brain.getStack();
    const results = {
        source: 'stack',
