...
// v0.9.0 fs->storage migration: canvas artifact reads/writes go through
// FileStorage (containment/symlink/VAF/atomic-write) - which also closes the
// previously-unchecked artwork-name traversal hole in save/load/embed.
// Directory enumeration in list() stays on fs (storage.list is pattern-glob
// only) with paths anchored to getCanvasPath(), never caller input.
const _canvasStores = new Map();
function _getStore(isPublic = false) {
    if (!_canvasStores.has(isPublic)) {
        _canvasStores.set(isPublic, new storage.FileStorage({ basePath: getCanvasPath(isPublic) }));
    }
    return _canvasStores.get(isPublic);
}
...
async function _save(name, options = {}) {
    ...
    if (!_canvasState.current) {
        throw new errors.VantError('Nothing painted. Run paintSpiral() first.', { code: errors.CODES.STORAGE_WRITE_DENIED, retryable: false });
    }
    ...
    if (format === 'svg') {
        ...
        throw new errors.VantError('Unknown format: ' + format, { code: errors.CODES.FORMAT_INVALID, retryable: false });
    }

    // Save file through storage layer (validates name, atomic write)
    const store = _getStore(isPublic);
    store.write(name + ext, content);
...
/**
 * Vote using pipeline-secured consensus (v0.9.0-axolotl)
 * Uses unified security chain: sandbox -> vaf -> qos -> escrow
 */
async function voteSecured(topic, option, agentId) {
    return consensus.voteSecured('canvas_' + topic, option, agentId);
}
...
async function load(name, options = {}) {
    ...
    if (store.has(name + '.svg')) {
        return { format: 'svg', content: store.read(name + '.svg') };
    }

    if (store.has(name + '.md')) {
        return { format: 'md', content: store.read(name + '.md') };
    }
...
async function applyEffect(name, effect) {
    ...
    if (!art) throw new errors.VantError('Not found: ' + name, { code: errors.CODES.STORAGE_NOT_FOUND, retryable: false });
...
async function embed(name, secret, password, options = {}) {
    ...
    if (!art) throw new errors.VantError('Art not found: ' + name, { code: errors.CODES.STORAGE_NOT_FOUND, retryable: false });
    ...
    // Save the embedded art through storage layer (validates name)
    const store = _getStore(isPublic);
    store.write(name + '.svg', svg);
    const savePath = path.join(getCanvasPath(isPublic), name + '.svg');
...
/**
 * List all canvases from all brains in the stack
 * @param {Object} options - Filter options
 * @returns {Promise<Array>} Combined list from all brains
 */
async function listStack(options = {}) {
    const brain = require('./brain');
    ...
    for (const brainName of stack) {
        try {
            ...
            const canvases = await list(options);
...
/**
 * Load canvas from any brain in stack by name
 * @param {string} name - Canvas name
 * @param {Object} options - Options including brain
 * @returns {Promise<Object>} Canvas data
 */
async function loadStack(name, options = {}) {
    const brain = require('./brain');
    ...
    if (options.brain) {
        try {
            ...
            return await load(name, options);
        } catch (e) {
            // Try next brain
        } finally {
            brain.removeBrain();
    ...
    for (const brainName of stack) {
        try {
            brain.pushBrain(brainName);
            const canvas = await load(name, { ...options, brain: brainName });
            if (canvas) {
                brain.removeBrain();
                return { ...canvas, brain: brainName };
            }
        } catch (e) {
            // Try next brain
        } finally {
