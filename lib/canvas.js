...
function getCanvasPath(isPublic = false) {
    ...
    return path.join(base, 'examples');
}
...
async function save(name, options = {}) {
    ...
    if (!_canvasState.current) {
        throw new Error('Nothing painted. Run paintSpiral() first.');
    }
...
//     const barcode = geo.quasicrystal.generateBarcodeFromContent('canvas/' + name);
//     await geo.store(barcode, {
//         name,
//         format,
//         created: Date.now(),
//         options: _canvasState.current.options
//     }, geo.quasicrystal.initStorage(getCanvasPath(isPublic)));
...
async function applyEffect(name, effect) {
    ...
    if (!art) throw new Error('Not found: ' + name);
...
async function embed(name, secret, password, options = {}) {
    ...
    if (!art) throw new Error('Art not found: ' + name);
    
    const stego = require('./stego');
    const Encrypt = require('./encrypt');
    
    // Encode to buffer (adds BRN: prefix)
    let encoded = stego.encodeToBuffer(secret, password);
    
    // Embed as SVG metadata tag
    let svg = art.content;
    
    // Clean up any existing secret first (idempotent)
...
async function reveal(name, password, options = {}) {
    ...
    // Extract Base64 from metadata (simple pattern)
    const match = art.content.match(/<brn:secret>([A-Za-z0-9+\/=]+)<\/brn:secret>/);
    ...
    // Reveal the secret
    return stego.decodeFromBuffer(buf, password);
}
