...
| Area | Status | Evidence (pass 31) |
|------|--------|--------------------|
| Core (brain/storage/security/qos) | 🟢 green | brain 77, storage 40, security ✚, qos ✚ |
| Protocol layer | 🟢 green + stress 11/11 | consensus 8, market 22, msg ✚, trust 20, escrow 15 |
| Society (teams/agents/governance) | 🟢 green | teams 22, agents 17, orgflow 24 |
| Backup/horcrux | 🟢 green | backup 8, brain-storage-strict 14, stego 12 |
| CLI | 🟢 green | sync 18, sync-pull 15, build-test 15/15, test-all 17/17, eslint bin/ 0 errors |
| Crew harness | 🟢 0 fires | stress 11/11 probes green after pass-31 fixes |

Baseline result: **zero red suites** across the 25+ suites sampled. The
stability work now shifts from "fix breakage" to "keep the harness hunting
and pin what it catches."
...
| `test/` count gap | low | 115 test files, no single runner runs them all (`build-test` + `test-all` cover ~32). A `test/run-all.js` sweep is a v1.0.0-rc candidate. |

---

## Release Gate (draft, v1.0.0)

1. Full suite green (all suites that exist, via the sweep above)
2. Stress harness 0 fires across 2 consecutive runs
3. Demo genesis 9/9 ×2 (idempotency)
4. `npm run check` syntax clean + eslint 0 errors
5. `vant migrate` on a legacy tree round-trips (content-verified import)
6. Horcrux create → extract → restore round-trip in a fresh dir
7. No plaintext credential persistence anywhere in lib/ + bin/
